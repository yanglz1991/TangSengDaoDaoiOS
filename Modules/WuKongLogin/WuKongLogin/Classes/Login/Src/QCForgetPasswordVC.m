...
@interface QCForgetPasswordVC ()<UITextFieldDelegate>
...
@implementation QCForgetPasswordVC

- (instancetype)init
{
    self = [super init];
    if (self) {
        self.viewModel = [QCForgetPasswordVM new];
    }
    return self;
}
...
- (UILabel *)titleLbl {
    if(!_titleLbl) {
        ...
        _titleLbl = [[UILabel alloc] initWithFrame:CGRectMake(30, self.navigationBar.lim_bottom+10.0f, QCScreenWidth-60, 22)];
...
- (UILabel *)subtitleLbl {
    if(!_subtitleLbl) {
        _subtitleLbl = [[UILabel alloc] initWithFrame:CGRectMake(15.0f, self.titleLbl.lim_bottom + 10.0f , QCScreenWidth-15.0*2, 16.0f)];
        _subtitleLbl.font = [[QCApp shared].config appFontOfSize:14.0f];
        _subtitleLbl.text = [NSString stringWithFormat:LLang(@"%@会发送短信验证手机号码，请输入您的验证码"),[QCApp shared].config.appName];
...
- (UIButton *)okBtn {
    if(!_okBtn) {
        _okBtn = [[UIButton alloc] initWithFrame:CGRectMake(30.0f, self.passwordBoxView.lim_bottom+82.0f, QCScreenWidth - 60.0f, 40.0f)];
        [_okBtn setBackgroundColor:[QCApp shared].config.themeColor];
...
-(void) sendCode {
    ...
    [self.viewModel sendCode:[NSString stringWithFormat:@"00%@",self.country] phone:self.mobileTextField.text].then(^(NSDictionary *resultDic){
        if(resultDic && resultDic[@"exist"] && [resultDic[@"exist"] integerValue] == 1) {
            ...
            [QCAlertUtil alert:LLang(@"手机号已注册,去登录？") buttonsStatement:@[LLang(@"取消"),LLang(@"去登录")] chooseBlock:^(NSInteger buttonIdx) {
                if(buttonIdx == 1) {
                    [[QCNavigationManager shared] popViewControllerAnimated:YES];
                    return;
                }
