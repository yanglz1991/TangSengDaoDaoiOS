...
@interface QCRegisterVC ()<UITextFieldDelegate,M80AttributedLabelDelegate>
...
@implementation QCRegisterVC

- (instancetype)init
{
    self = [super init];
    if (self) {
        self.viewModel = [QCRegisterVM new];
    }
    return self;
}
...
- (UIImageView *)bgImgView {
    if(!_bgImgView) {
        _bgImgView = [[UIImageView alloc] initWithImage:[[QCApp shared] loadImage:@"Background" moduleID:@"WuKongLogin"]];
...
- (UILabel *)titleLbl {
    if(!_titleLbl) {
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:LLang(@"欢迎注册%@"),[QCApp shared].config.appName] attributes: @{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Semibold" size: 32],NSForegroundColorAttributeName: [UIColor colorWithRed:49/255.0 green:49/255.0 blue:49/255.0 alpha:1.0]}];
        _titleLbl = [[UILabel alloc] initWithFrame:CGRectMake(30, 83, QCScreenWidth-60, 50)];
...
- (UIView *)mobileBoxView {
    if(!_mobileBoxView) {
        _mobileBoxView = [[UIView alloc] initWithFrame:CGRectMake(0, 190.0f, QCScreenWidth, 40.0f)];
...
- (UIButton *)countryBtn {
    if(!_countryBtn) {
        ...
        [[_countryBtn titleLabel] setFont:QCApp.shared.config.defaultFont];
        [_countryBtn setTitleColor:QCApp.shared.config.defaultTextColor forState:UIControlStateNormal];
...
- (UIImageView *)downArrowView {
    if(!_downArrowView) {
        ...
        [_downArrowView setImage:[[QCApp shared] loadImage:@"ArrowDown" moduleID:@"WuKongLogin"]];
    }
...
- (UIView *)countrySpliteLineView {
    if(!_countrySpliteLineView) {
        ...
        _countrySpliteLineView.layer.backgroundColor = [QCApp shared].config.lineColor.CGColor;
...
-(UITextField*) mobileTextField {
    if(!_mobileTextField) {
        ...
        _mobileTextField = [[UITextField alloc] initWithFrame:CGRectMake(left, self.mobileBoxView.lim_height/2.0f - 20.0f, QCScreenWidth - left - 20.0f, 40.0f)];
...
- (UIView *)mobileBottomLineView {
    if(!_mobileBottomLineView) {
        _mobileBottomLineView = [[UIView alloc] initWithFrame:CGRectMake(20.0f, self.mobileBoxView.lim_height, QCScreenWidth-40.0f, 1)];
        _mobileBottomLineView.layer.backgroundColor = [QCApp shared].config.lineColor.CGColor;
    }
...
- (UIView *)codeBoxView {
    if(!_codeBoxView) {
        _codeBoxView = [[UIView alloc] initWithFrame:CGRectMake(0, self.mobileBoxView.lim_bottom+20.0f, QCScreenWidth, self.mobileBoxView.lim_height)];
...
- (UIView *)codeLineView {
    if(!_codeLineView) {
        _codeLineView = [[UIView alloc] initWithFrame:CGRectMake(20.0f, self.codeBoxView.lim_height, QCScreenWidth-40.0f, 1)];
        _codeLineView.layer.backgroundColor = [QCApp shared].config.lineColor.CGColor;
    }
    return _codeLineView;
}
- (UITextField *)codeTextField {
    if(!_codeTextField) {
        _codeTextField = [[UITextField alloc] initWithFrame:CGRectMake(20.0f, self.codeBoxView.lim_height/2.0f - 20.0f, QCScreenWidth-self.codeLineView.lim_left - 80.0f, 40.0f)];
...
- (UIButton *)getCodeBtn {
    if(!_getCodeBtn) {
        ...
        [_getCodeBtn setBackgroundColor:[QCApp shared].config.themeColor];
        ...
        [QCApp.shared.config setThemeStyleButton:_getCodeBtn];
    }
    return _getCodeBtn;
}
...
- (UIView *)passwordBoxView {
    if(!_passwordBoxView) {
        _passwordBoxView = [[UIView alloc] initWithFrame:CGRectMake(0, self.codeBoxView.lim_bottom+20.0f, QCScreenWidth, self.mobileBoxView.lim_height)];
       // [_passwordBoxView setBackgroundColor:[UIColor grayColor]];
    }
...
- (UIView *)passwordBottomLineView {
    if(!_passwordBottomLineView) {
        _passwordBottomLineView = [[UIView alloc] initWithFrame:CGRectMake(20.0f, self.passwordBoxView.lim_height, QCScreenWidth-40.0f, 1)];
        _passwordBottomLineView.layer.backgroundColor = [QCApp shared].config.lineColor.CGColor;
    }
...
- (UITextField *)passwordTextField {
    if(!_passwordTextField) {
        _passwordTextField = [[UITextField alloc] initWithFrame:CGRectMake(20.0f, self.mobileBoxView.lim_height/2.0f - 20.0f, QCScreenWidth-20*2 - 32.0f, 40.0f)];
...
- (UIButton *)eyeBtn {
    if(!_eyeBtn) {
        ...
        [_eyeBtn setImage:[[QCApp shared] loadImage:@"BtnEyeOff" moduleID:@"WuKongLogin"] forState:UIControlStateNormal];
        [_eyeBtn setImage:[[QCApp shared] loadImage:@"BtnEyeOn" moduleID:@"WuKongLogin"] forState:UIControlStateSelected];
...
- (UIView *)inviteCodeBoxView {
    if(!_inviteCodeBoxView) {
        _inviteCodeBoxView = [[UIView alloc] initWithFrame:CGRectMake(0, self.passwordBoxView.lim_bottom+20.0f, QCScreenWidth, self.passwordBoxView.lim_height)];
    }
...
- (UITextField *)inviteCodeTextField {
    if(!_inviteCodeTextField) {
        _inviteCodeTextField = [[UITextField alloc] initWithFrame:CGRectMake(20.0f, self.passwordBoxView.lim_height/2.0f - 20.0f, QCScreenWidth-20*2 - 32.0f, 40.0f)];
        
        _inviteCodeTextField.hidden = !QCApp.shared.remoteConfig.registerInviteOn;
...
- (UIView *)inviteCodeBottomLineView {
    if(!_inviteCodeBottomLineView) {
        _inviteCodeBottomLineView = [[UIView alloc] initWithFrame:CGRectMake(20.0f, self.inviteCodeBoxView.lim_height, QCScreenWidth-40.0f, 1)];
        _inviteCodeBottomLineView.layer.backgroundColor = [QCApp shared].config.lineColor.CGColor;
    }
...
- (UIButton *)registerBtn {
    if(!_registerBtn) {
        ...
        _registerBtn = [[UIButton alloc] initWithFrame:CGRectMake(30.0f,top+82.0f, QCScreenWidth - 60.0f, 40.0f)];
        [_registerBtn setBackgroundColor:[QCApp shared].config.themeColor];
        ...
        [QCApp.shared.config setThemeStyleButton:_registerBtn];
...
- (UIButton *)toLoginBtn {
    if(!_toLoginBtn) {
        ...
        [_toLoginBtn setTitleColor:[QCApp shared].config.themeColor forState:UIControlStateNormal];
...
-(void) countryBtnPressed {
    QCCountrySelectVC *vc = [QCCountrySelectVC new];
    ...
    [[[QCNavigationManager shared] topViewController] presentViewController:nav animated:YES completion:nil];
}
// 获取验证码
-(void) sendCode {
    ...
    [self.viewModel sendCode:[NSString stringWithFormat:@"00%@",self.country] phone:self.mobileTextField.text].then(^(NSDictionary *resultDic){
        if(resultDic && resultDic[@"exist"] && [resultDic[@"exist"] integerValue] == 1) {
            ...
            [QCAlertUtil alert:LLangW(@"手机号已注册,去登录？", weakSelf) buttonsStatement:@[LLangW(@"取消",weakSelf),LLangW(@"去登录",weakSelf)] chooseBlock:^(NSInteger buttonIdx) {
                if(buttonIdx == 1) {
                    [[QCNavigationManager shared] popViewControllerAnimated:YES];
                    return;
                }
        ...
        [QCApp shared].loginInfo.extra[@"phone"] = weakSelf.mobileTextField.text;
        [QCApp shared].loginInfo.extra[@"zone"] = [NSString stringWithFormat:@"00%@",weakSelf.country];
...
-(void) registerBtnPressed {
    ...
    __weak typeof(self) weakSelf = self;
    [self.viewModel registerByPhone:[NSString stringWithFormat:@"00%@",zone] phone:phone code:code inviteCode:inviteCode password:password].then(^(QCLoginResp*resp){
...
- (M80AttributedLabel *)privacyLbl {
    if(!_privacyLbl) {
        ...
        [_privacyLbl setTextColor:[QCApp shared].config.tipColor];
        [_privacyLbl appendText:LLang(@"点击“注册”即表示已阅读并同意")];
        NSString *uPTxt = LLang(@"用户协议");
        [_privacyLbl addCustomLink:[QCApp shared].config.userAgreementUrl forRange:NSMakeRange(_privacyLbl.text.length, uPTxt.length)];
        [_privacyLbl appendText:uPTxt];
        [_privacyLbl appendText:@"、"];
        NSString *pPTxt = LLang(@"隐私政策");
        [_privacyLbl addCustomLink:[QCApp shared].config.privacyAgreementUrl forRange:NSMakeRange(_privacyLbl.text.length, pPTxt.length)];
        [_privacyLbl appendText:pPTxt];
        
        [_privacyLbl sizeToFit];
        _privacyLbl.lim_centerX_parent = self.view;
        _privacyLbl.lim_top = QCScreenHeight - ( bottom + 30.0f);
    }
    return _privacyLbl;
}
