...
#import <WuKongIMSDK/QCMediaUtil.h>
@interface QCSmallVideoContent ()
@property(nonatomic,strong) NSData *videoData;
@property(nonatomic,strong) NSData *coverData;
@end

@implementation QCSmallVideoContent

+(QCSmallVideoContent*) smallVideoContent:(NSData*)videoData coverData:(NSData*)coverData second:(NSInteger)second {
    QCSmallVideoContent *smallVideoContent = [QCSmallVideoContent new];
...
- (NSString *)coverLocalPath {
    ...
    NSString *uid = [QCSDK shared].options.connectInfo.uid;
       return   [NSString stringWithFormat:@"%@/%@/%@",[QCSDK shared].options.messageFileRootDir,uid, [NSString stringWithFormat:@"%@/%@%@",[QCMediaUtil getChannelDir:channel],self.message.clientMsgNo,@".jpg"]];
}
