...
@implementation QCSmallVideoCell

+ (CGSize)contentSizeForMessage:(QCMessageModel *)model {
    QCSmallVideoContent *smallVideoContent = (QCSmallVideoContent*)model.content;
...
- (void)initUI {
    [super initUI];
    // cover
    self.coverImgView = [[QCImageView alloc] init];
...
//    self.trailingView.timeLbl.textColor = [UIColor whiteColor];
    ...
    if([[NSFileManager defaultManager] fileExistsAtPath:[smallVideoContent coverLocalPath]]) {
        [self.coverImgView lim_setImageWithURL:[NSURL fileURLWithPath:[smallVideoContent coverLocalPath]]];
         ...
         [self.coverImgView loadImage:[[QCApp shared] getImageFullUrl:smallVideoContent.cover] placeholderImage:[self imageName:@"DefaultVideo"]];
    }
...
-(void) updateProgress {
    ...
    if(self.uploadTask && [self.uploadTask isKindOfClass:[QCMessageFileUploadTask class]]) {
...
-(void) onTap {
    ...
    QCMessageFileDownloadTask *task =  [[QCSDK shared] getMessageDownloadTask:self.messageModel.message];
    if(!task) {
        task =  [[QCSDK shared].mediaManager download:weakSelf.messageModel.message];
    }
