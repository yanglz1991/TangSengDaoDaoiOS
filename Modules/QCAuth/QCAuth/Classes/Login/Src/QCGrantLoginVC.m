...
@interface QCGrantLoginVC ()
@property(nonatomic,strong) UIImageView *pcImgView;
@property(nonatomic,strong) UILabel *tipLbl;
@property(nonatomic,strong) UIButton *loginBtn;
@property(nonatomic,strong) UIButton *cancelLoginBtn;
@property(nonatomic,strong) UIButton *closeBtn;
@property(nonatomic,strong) QCGrantLoginVM *vm;
@end

@implementation QCGrantLoginVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.vm = [QCGrantLoginVM initWithAuthCode:self.authCode pubkeyBase64Enc:self.pubkeyBase64Enc];
...
- (UILabel *)tipLbl {
    if(!_tipLbl) {
        ...
        _tipLbl.text = [NSString stringWithFormat:LLang(@"网页版%@登录确认"),[QCApp shared].config.appName];
