...
- (UILabel *)welcomeTitleLbl {
    if(!_welcomeTitleLbl) {
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:LLang(@"欢迎登录%@"),[QCApp shared].config.appName] attributes: @{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Semibold" size: 32],NSForegroundColorAttributeName: [UIColor colorWithRed:49/255.0 green:49/255.0 blue:49/255.0 alpha:1.0]}];
        _welcomeTitleLbl = [[UILabel alloc] initWithFrame:CGRectMake(30, 93, QCScreenWidth-60, 50)];
...
- (UIView *)mobileBoxView {
    if(!_mobileBoxView) {
        _mobileBoxView = [[UIView alloc] initWithFrame:CGRectMake(0, 240.0f, QCScreenWidth, 40.0f)];
        //[_mobileBoxView setBackgroundColor:[UIColor redColor]];
        _mobileBoxView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
    }
...
- (UIView *)passwordBoxView {
    if(!_passwordBoxView) {
        _passwordBoxView = [[UIView alloc] initWithFrame:CGRectMake(0, self.mobileBoxView.lim_bottom+20.0f, QCScreenWidth, self.mobileBoxView.lim_height)];
       // [_passwordBoxView setBackgroundColor:[UIColor grayColor]];
        _passwordBoxView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
    }
...
- (UIButton *)forgetPwdBtn {
    if(!_forgetPwdBtn) {
        ...
        [_forgetPwdBtn setTitleColor:[QCApp shared].config.themeColor forState:UIControlStateNormal];
...
-(void) forgetPwdPressed {
    QCForgetPasswordVC *vc = [QCForgetPasswordVC new];
...
- (UIButton *)loginBtn {
    if(!_loginBtn) {
        _loginBtn = [[UIButton alloc] initWithFrame:CGRectMake(30.0f, self.passwordBoxView.lim_bottom+82.0f, QCScreenWidth - 60.0f, 40.0f)];
//        [_loginBtn setBackgroundColor:[QCApp shared].config.themeColor];
        [_loginBtn setBackgroundColor:[QCApp shared].config.themeColor];
        ...
        [QCApp.shared.config setThemeStyleButton:_loginBtn];
...
- (UIButton *)registerBtn {
    if(!_registerBtn) {
        ...
        [_registerBtn setTitleColor:[QCApp shared].config.themeColor forState:UIControlStateNormal];
...
-(void) toRegisterPressed{
    [[QCNavigationManager shared] pushViewController:[QCRegisterVC new] animated:YES];
}
...
-(void) giteeLoginPressed {
    ...
    [QCAPIClient.sharedClient GET:@"user/thirdlogin/authcode" parameters:nil].then(^(NSDictionary *resultDict){
        NSString *authcode = resultDict[@"authcode"];
        if(authcode) {
            QCAuthWebViewVC *vc = [[QCAuthWebViewVC alloc] init];
            vc.authcode = authcode;
            vc.url = [NSURL URLWithString:[NSString stringWithFormat:@"%@user/gitee?authcode=%@",QCAPIClient.sharedClient.config.baseUrl,authcode]];
            [QCNavigationManager.shared pushViewController:vc animated:YES];
        }
    }).catch(^(NSError *error){
...
-(void) githubLoginPressed {
    ...
    [QCAPIClient.sharedClient GET:@"user/thirdlogin/authcode" parameters:nil].then(^(NSDictionary *resultDict){
        ...
        if(authcode) {
            QCAuthWebViewVC *vc = [[QCAuthWebViewVC alloc] init];
            vc.authcode = authcode;
            vc.url = [NSURL URLWithString:[NSString stringWithFormat:@"%@user/github?authcode=%@",QCAPIClient.sharedClient.config.baseUrl,authcode]];
            [QCNavigationManager.shared pushViewController:vc animated:YES];
        }
    }).catch(^(NSError *error){
        [weakself showHUDWithHide:error.domain];
...
- (void)viewConfigChange:(QCViewConfigChangeType)type {
    self.mobileBoxView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
    self.passwordBoxView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
    self.passwordBottomLineView.layer.backgroundColor = [QCApp shared].config.lineColor.CGColor;
    self.countrySpliteLineView.layer.backgroundColor = [QCApp shared].config.lineColor.CGColor;
    self.mobileBottomLineView.layer.backgroundColor = [QCApp shared].config.lineColor.CGColor;
}


-(UIImage*) image:(NSString*)name {
    return [[QCApp shared] loadImage:name moduleID:@"QCAuth"];
}
