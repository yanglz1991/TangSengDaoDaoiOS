...
@interface QCCountrySelectVC ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>
...
@implementation QCCountrySelectVC
...
-(void) requestCountriesAndRefreshData {
    ...
    [[QCAPIClient sharedClient] GET:@"common/countries" parameters:nil].then(^(NSArray*data){
           // 缓存下来
...
- (UIView *)closeView {
    if (!_closeView) {
        ...
        [[closeBtn titleLabel] setTextColor:QCApp.shared.config.defaultTextColor];
        [closeBtn setTitleColor:QCApp.shared.config.defaultTextColor forState:UIControlStateNormal];
...
//        _searchBar.searchBarStyle = UISearchBarStyleProminent;
        ...
        [_searchBar setBackgroundColor:[QCApp shared].config.backgroundColor];
        ...
        if (@available(iOS 13.0, *)) {
            ...
            [_searchBar searchTextField].layer.backgroundColor = QCApp.shared.config.backgroundColor.CGColor;
            [_searchBar searchTextField].backgroundColor = QCApp.shared.config.backgroundColor;
        } else {
...
- (UIView *)tableView:(UITableView *)tableView
  ...
  UIView *myView = [[UIView alloc] init];
    myView.backgroundColor =QCApp.shared.config.cellBackgroundColor;
  UILabel *titleLabel =
      [[UILabel alloc] initWithFrame:CGRectMake(10, 0, 90, 22)];
  titleLabel.textColor = QCApp.shared.config.defaultTextColor;
