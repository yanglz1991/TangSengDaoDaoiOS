...
@interface QCRegisterNextVC ()<TOCropViewControllerDelegate>
...
@implementation QCRegisterNextVC
...
- (UILabel *)tipLbl {
    if(!_tipLbl) {
        ...
        [_tipLbl setFont:[[QCApp shared].config appFontOfSize:16.0f]];
        [_tipLbl setTextColor:[[QCApp shared].config defaultTextColor]];
        [_tipLbl sizeToFit];
...
- (UIView *)lineView {
    if(!_lineView) {
        ...
        [_lineView setBackgroundColor:[QCApp shared].config.backgroundColor];
    }
    return _lineView;
}
...
-(void) avatarPressed {
    ...
    [actionSheet addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"从手机相册选择") onClick:^{
        [[QCPhotoService shared] getPhotoOneFromLibrary:^(UIImage * _Nonnull image) {
            [weakSelf cropAvatar:image];
        }];
    }]];
    [actionSheet show];
...
didCropToImage:(nonnull UIImage *)image withRect:(CGRect)cropRect
    ...
    [[QCAPIClient sharedClient] fileUpload:@"users/{uid}/avatar" data:data progress:^(NSProgress * _Nonnull progress) {
        ...
        if(error) {
            ...
            [[SDImageCache sharedImageCache] removeImageForKey:[QCAvatarUtil getAvatar:[QCApp shared].loginInfo.uid] withCompletion:nil];
            
            [[NSNotificationCenter defaultCenter] postNotificationName:QCNOTIFY_USER_AVATAR_UPDATE object:@{@"uid":[QCApp shared].loginInfo.uid?:@""}];
        }
