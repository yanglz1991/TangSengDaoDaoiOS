...
@implementation QCLoginSettingVC
...
-(void) layout {
    ...
    self.schemeLbl.lim_top = QCApp.shared.config.visibleEdgeInsets.top + 80.0f;
...
-(UIButton*) resetBtn {
    if(!_resetBtn) {
        ...
        [[_resetBtn titleLabel] setFont:[QCApp.shared.config appFontOfSize:15.0f]];
        [_resetBtn setTitleColor:QCApp.shared.config.defaultTextColor forState:UIControlStateNormal];
        [_resetBtn sizeToFit];
...
+(void) setAppConfigIfNeed {
    ...
    if(serverIP && ![serverIP isEqualToString:@""]) {
        ...
        QCAppConfig *config = QCApp.shared.config;
        ...
        QCAPIClientConfig *apiConfig = QCAPIClient.sharedClient.config;
        apiConfig.baseUrl = apiAddr;
        QCAPIClient.sharedClient.config = apiConfig; // 这里目的是重新触发WKAPIClient的setConfig方法
    }
}
