...
- (UILabel *)titleLbl {
    if(!_titleLbl) {
        ...
        _titleLbl.font = [QCApp.shared.config appFontOfSizeMedium:30.0f];
        _titleLbl.text = [NSString stringWithFormat:@"欢迎登录%@",QCApp.shared.config.appName];
...
//        
//        _settingBtn.lim_top = QCApp.shared.config.visibleEdgeInsets.top + 20.0f;
...
-(void) settingPressed {
    QCLoginSettingVC *vc = [QCLoginSettingVC new];
...
-(void) titleLongPressed:(UILongPressGestureRecognizer*)gestureRecognizer {
    if(gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        QCLoginVC *vc = [[QCLoginVC alloc] init];
        [QCNavigationManager.shared pushViewController:vc animated:YES];
    }
}
