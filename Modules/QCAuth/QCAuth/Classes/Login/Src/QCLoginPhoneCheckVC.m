...
@interface QCLoginPhoneCheckVC ()<QCLoginPhoneCheckVMDelegate>
...
@implementation QCLoginPhoneCheckVC

- (instancetype)init
{
    self = [super init];
    if (self) {
        self.viewModel = [QCLoginPhoneCheckVM new];
...
#pragma mark - QCLoginPhoneCheckVMDelegate
...
- (void)loginPhoneCheckVMDidOk:(QCLoginPhoneCheckVM *)vm {
    ...
    __weak typeof(self) weakSelf = self;
    [self.view showHUD];
    [self.viewModel loginCheckPhone:self.uid code:[self.viewModel getCode]].then(^(QCLoginResp *resp){
...
-(void) reloadCodeItem {
    ...
    QCSMSCodeItemCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:1]];
    if(cell) {
