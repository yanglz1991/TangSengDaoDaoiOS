...
#import "QCConstant.h"
#import "QCSync.h"
@implementation QCSyncService

static QCSyncService *_instance = nil;
...
- (void)sync:(void (^)(NSError *))callback {
    ...
    // 创建一个 dispatch_group
       ...
       dispatch_async(dispatch_get_global_queue(0, 0), ^{
           ...
           NSArray<id<QCSync>> *syncServices = [[QCApp shared] invokes:QCPOINT_CATEGORY_SYNC param:nil];
           if(syncServices) {
               for (id<QCSync> syncService in syncServices) {
...
-(void) syncContacts:(void (^)(NSError  * __nullable error))callback {
   id<QCSync> syncService = [QCApp.shared invoke:QCPOINT_SYNC_CONTACTS param:nil];
    if(syncService) {
