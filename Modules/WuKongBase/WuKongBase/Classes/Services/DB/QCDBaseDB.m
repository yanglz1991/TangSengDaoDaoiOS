...
#import "QCModel.h"
#import "QCKitDB.h"
@implementation QCDBaseDB


-(AnyPromise*) insertModel:(QCModel*)model forTable:(NSString*)table {
    ...
    return [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        [[[QCKitDB shared] dbQueue] inDatabase:^(FMDatabase * _Nonnull db) {
            BOOL isSuccess = [[QCKitDB shared] insertTable:table dic:[model toMap:ModelMapTypeDB] db:db];
            if(isSuccess) {
                resolve(@(isSuccess));
            }else{
...
-(AnyPromise*) deleteTableAndInsertModels:(NSArray<QCModel*>*)models forTable:(NSString*)table{
    return [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        QCKitDB *atdb = [QCKitDB shared];
...
-(AnyPromise*) insertModels:(NSArray<QCModel*>*)models forTable:(NSString*)table{
    ...
    return [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        [[[QCKitDB shared] dbQueue] inDatabase:^(FMDatabase * _Nonnull db) {
            ...
            for(QCModel *model in models) {
                [dicts addObject:[model toMap:ModelMapTypeDB]];
            }
            NSArray *array = [[QCKitDB shared] insertTable:table db:db dicArray:dicts];
            if(!array || array.count<=0) {
                resolve(@(true));
            }else{
                resolve([NSError errorWithDomain:[NSString stringWithFormat:@"[%lu条数据]插入错误!",(unsigned long)array.count] code:0 userInfo:@{@"array":array}]);
...
-(AnyPromise*) insertModelsOfTransaction:(NSArray<QCModel*>*)models forTable:(NSString*)table{
    if(!models || models.count<=0) [NSException exceptionWithName:NSInternalInconsistencyException reason:@"models is nil or <=0 " userInfo:nil];
    return [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
...
-(NSArray<NSDictionary*>*) modelsToArrayDics:(NSArray<QCModel*>*)models {
...
-(AnyPromise*) queryModels:(Class)modelClass forTable:(NSString*)table whereFormat:(NSString *)format, ...{
    ...
    return [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        [[[QCKitDB shared] dbQueue] inDatabase:^(FMDatabase * _Nonnull db) {
            ...
            NSArray<NSDictionary*> *resultDic = [[QCKitDB shared] QueryTable:table db:db whereFormat:where];
            NSMutableArray *modelList = [[NSMutableArray alloc] init];
            IMP imp = [self getModelFromMapTypeSel:modelClass];
            if(imp) {
                QCModel* (*convertMap)(id, SEL,NSDictionary*,ModelMapType) = (void *)imp;
                if(resultDic&&resultDic.count>0) {
                    SEL sel = NSSelectorFromString(@"fromMap:type:");
                    for (NSDictionary *dic in resultDic) {
                        QCModel *model = convertMap(modelClass,sel,dic,ModelMapTypeDB);
...
-(NSArray<QCModel*>*) dictToModels:(NSArray<NSDictionary*>*) resultDic class:(Class)modelClass {
    NSMutableArray *modelList = [[NSMutableArray alloc] init];
    IMP imp = [self getModelFromMapTypeSel:modelClass];
    if(imp) {
        QCModel* (*convertMap)(id, SEL,NSDictionary*,ModelMapType) = (void *)imp;
...
-(QCModel*) dictToModel:(NSDictionary*) resultDic class:(Class)modelClass {
    IMP imp = [self getModelFromMapTypeSel:modelClass];
    if(imp) {
        QCModel* (*convertMap)(id, SEL,NSDictionary*,ModelMapType) = (void *)imp;
        if(resultDic) {
            SEL sel = NSSelectorFromString(@"fromMap:type:");
            QCModel *model = convertMap(modelClass,sel,resultDic,ModelMapTypeDB);
            return model;
        }
    }
    return nil;
...
-(AnyPromise*) queryCountForTable:(NSString*) table {
    return [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        [[[QCKitDB shared] dbQueue] inDatabase:^(FMDatabase * _Nonnull db) {
            NSNumber *count = [[QCKitDB shared] queryCount:table db:db];
            resolve(count);
        }];
    }];
}
-(AnyPromise*) queryCountForTable:(NSString*) table whereFormat:(NSString *)format, ...{
    return [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        [[[QCKitDB shared] dbQueue] inDatabase:^(FMDatabase * _Nonnull db) {
            NSNumber *count = [[QCKitDB shared] queryCount:table db:db whereFormat:format];
            resolve(count);
        }];
    }];
}
