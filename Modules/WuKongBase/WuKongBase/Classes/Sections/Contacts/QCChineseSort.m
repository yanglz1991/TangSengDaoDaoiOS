...
@implementation QCChineseSortSetting

+(QCChineseSortSetting*)share{
    static QCChineseSortSetting * singleton = nil ;
    if (singleton == nil) {
        singleton = [[QCChineseSortSetting alloc] init];
...
@interface QCChineseSortModel : NSObject
...
@implementation QCChineseSortModel
@end
...
- (NSComparisonResult)mySort:(NSString *)str {
    NSString*s = [QCChineseSortSetting share].specialCharSectionTitle;
    BOOL b = [QCChineseSortSetting share].specialCharPositionIsFront;
...
+(void)sortAndGroup:(NSArray*)objectArray
    ...
    if (key == nil) {
        if (![obj isKindOfClass:NSString.class]) {
            [QCChineseSort logMsg:@"数组内元素不是字符串类型,如果是对象类型，请传key"];
    ...
    if (!containKey) {
        [QCChineseSort logMsg:@"数组内元素未包含指定属性"];
        finish(NO,nil,nil,nil);
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        [QCChineseSort logMsg:@""];
        [QCChineseSort logMsg:[NSString stringWithFormat:@"转拼音用时：\t %f s", (state1-start)]];
        ...
        [QCChineseSort logMsg:[NSString stringWithFormat:@"排序用时：\t %f s", (state2-state1)]];
        ...
        for (QCChineseSortModel* object in sortModelArray) {
        ...
        [QCChineseSort logMsg:[NSString stringWithFormat:@"分组用时：\t %f s", (state3-state2)]];
        [QCChineseSort logMsg:[NSString stringWithFormat:@"QCChineseSort 排序总计用时：\t %f s", (state3-start)]];
...
//将对象 转为 QCChineseSortModel
+(QCChineseSortModel*)getModelWithObj:(id)obj key:(NSString*)key{
    QCChineseSortModel *model = [[QCChineseSortModel alloc]init];
    model.object = obj;
    if (!key) {
        model.string = obj;
    }else{
    ...
    if(model.string == nil || model.string.length == 0){
        model.string = QCChineseSortSetting.share.specialCharSectionTitle;
        model.pinYin = @"#";
    }else{
        //过滤 ignoreModelWithPrefix
        NSString *prefix = [model.string substringToIndex:1];
        if (![QCChineseSortSetting.share.ignoreModelWithPrefix containsString:prefix]) {
            //是否将字母与汉字拼音一起排序？？？？ （暂不考虑）
            //获取拼音首字母
...
+(void)logMsg:(NSString*)msg{
    if (QCChineseSortSetting.share.logEable == YES) {
        NSLog(@"------------------");
        NSLog(@"%@", msg);
        NSLog(@"------------------");
    }
...
+ (NSString *)getFirstLetter:(NSString *)chinese{
    ...
    //吧多音字先替换
    [QCChineseSortSetting.share.polyphoneMapping enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        newChinese = [newChinese stringByReplacingOccurrencesOfString:key withString:obj];
    }];

    NSMutableString *result = [NSMutableString string];

    if (QCChineseSortSetting.share.sortMode == 1) {
        //此处 对整个字符串转中文 而不单个字一次转 因为CFStringTransform太耗时 而这个时间又与字个数关系不大，所以尽量减少调用次数 以减少时间
        NSArray *wordArr = [[QCChineseSort transformChinese:newChinese] componentsSeparatedByString:@" "];
    ...
    if ([upperCaseStr characterAtIndex:0] >= 'A' && [upperCaseStr characterAtIndex:0] <= 'Z') {
        ...
        return QCChineseSortSetting.share.specialCharSectionTitle;
    }
}
