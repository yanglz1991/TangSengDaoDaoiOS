...
//底部显示的功能项
@property (nonatomic, strong) QCScanBottom *bottomView;


@end

@implementation QCScanVC
...
-(void) drawNavigationBar {
    ...
    self.navigationBar.style = QCNavigationBarStyleWhite;
...
- (QCNavigationBar *)navigationBar {
    if(!_navigationBar) {
        _navigationBar = [[QCNavigationBar alloc] initWithFrame:CGRectMake(0.0f, 0.0f,QCScreenWidth,[QCApp shared].config.navHeight)];
...
- (QCScanBottom *)bottomView {
    if(!_bottomView) {
        CGFloat safeBottom = 0.0f;
        if (@available(iOS 11.0, *)) {
            safeBottom = [[UIApplication sharedApplication].keyWindow safeAreaInsets].bottom;
        }
        _bottomView = [[QCScanBottom alloc] init];
        _bottomView.lim_top = QCScreenHeight - 200 - safeBottom;
...
//         safeBottom = [[UIApplication sharedApplication].keyWindow safeAreaInsets].bottom;
//    }
//    self.bottomItemsView = [[UIView alloc]initWithFrame:CGRectMake(0, QCScreenHeight-100 - safeBottom,
...
- (void)scanResultWithArray:(NSArray<LBXScanResult*>*)array {
    if (array.count < 1)
    {
        ...
        [QCAlertUtil alert:LLang(@"扫描失败！") buttonsStatement:@[LLang(@"知道了")] chooseBlock:^(NSInteger buttonIdx) {
    ...
    __weak typeof(self) weakSelf = self;
    [QCScanVC handleScanResult:scanResult handlers:self.handlers reStartDevice:^{
        [weakSelf reStartDevice];
    }];
...
+(BOOL) isQRCodeURL:(NSString*)scanResult {
    if([scanResult hasPrefix:@"http"]) {
        ...
        NSURL *scanPrefixURL = [NSURL URLWithString:[QCApp shared].config.scanURLPrefix];
...
+(void) handleScanResult:(LBXScanResult*)result handlers:(NSArray<QCScanHandler*>*)handlers reStartDevice:(void(^)(void))reStartDevice{
   ...
   if([self isQRCodeURL:strResult]) {
       if(handlers && handlers.count>0) {
           [[QCAPIClient sharedClient] GET:strResult parameters:nil model:QCScanResult.class].then(^(QCScanResult*result){
               for (QCScanHandler *handler in handlers) {
       ...
       [QCAlertUtil alert:strResult buttonsStatement:@[LLang(@"知道了")] chooseBlock:^(NSInteger buttonIdx) {
           if(reStartDevice) {
               reStartDevice();
           }
       }];
...
-(void) myQRCode {
    QCMeQRCodeVC *vc = [QCMeQRCodeVC new];
    [[QCNavigationManager shared] pushViewController:vc animated:YES];
}
