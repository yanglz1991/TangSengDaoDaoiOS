...
@implementation QCWebClientInfoVC
...
- (LOTAnimationView *)animationView {
    if(!_animationView) {
        _animationView = [LOTAnimationView animationNamed:@"Other/qrcode_web" inBundle:[QCApp.shared resourceBundle:@"WuKongBase"]];
...
- (UILabel *)titleLbl {
    if(!_titleLbl) {
        ...
        _titleLbl.text = [NSString stringWithFormat:LLang(@"%@网页端"),[QCApp shared].config.appName];
...
- (UIImageView *)webIconImgView {
    if(!_webIconImgView) {
        ...
        [_webIconImgView setTintColor:[QCApp shared].config.themeColor];
    }
...
- (UILabel *)webLbl {
    if(!_webLbl) {
        ...
        _webLbl.font = [[QCApp shared].config appFontOfSize:16.0f];
        [_webLbl sizeToFit];
    }
    return _webLbl;
}
...
- (UILabel *)addrLbl {
    if(!_addrLbl) {
        ...
        _addrLbl.text = [QCApp shared].remoteConfig.webURL?:@"";
        _addrLbl.textColor = [QCApp shared].config.tipColor;
        _addrLbl.font = [[QCApp shared].config appFontOfSize:15.0f];
        [_addrLbl sizeToFit];
    }
    return _addrLbl;
}
...
- (UILabel *)tipLbl {
    if(!_tipLbl) {
        ...
        _tipLbl.text = [NSString stringWithFormat:LLang(@"请使用浏览器访问%@,然后扫描二维码登录。为了您更好的体验，建议使用谷歌，火狐浏览器。"),[QCApp shared].config.appName];
        _tipLbl.lim_width = QCScreenWidth - 20.0f;
        _tipLbl.textColor = [QCApp shared].config.tipColor;
        _tipLbl.numberOfLines = 0;
        _tipLbl.lineBreakMode = NSLineBreakByWordWrapping;
        _tipLbl.font = [[QCApp shared].config appFontOfSize:12.0f];
        [_tipLbl sizeToFit];
    }
    return _tipLbl;
}
...
- (UIImageView *)qrcodeImgView {
    if(!_qrcodeImgView) {
        ...
        [_qrcodeImgView setTintColor:[QCApp shared].config.themeColor];
...
- (UILabel *)scanLoginLbl {
    if(!_scanLoginLbl) {
        ...
        _scanLoginLbl.font = [[QCApp shared].config appFontOfSize:16.0f];
...
- (UIView *)item1BoxView {
    if(!_item1BoxView) {
        _item1BoxView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, QCScreenWidth, self.addrLbl.lim_height+8.0f)];
    }
...
- (UIView *)item2BoxView {
    if(!_item2BoxView) {
        _item2BoxView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, QCScreenWidth, 50.0f)];
