...
@interface QCMeAvatarVC ()<TOCropViewControllerDelegate>
...
@implementation QCMeAvatarVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationBar.rightView = self.moreButtonItem;
    [self.view addSubview:self.avatarImgView];
    self.avatarImgView.url = [QCAvatarUtil getAvatar:[QCApp shared].loginInfo.uid];
}
...
- (QCUserAvatar *)avatarImgView {
    if(!_avatarImgView) {
        _avatarImgView = [[QCUserAvatar alloc] initWithFrame:CGRectMake(0.0f, [self visibleRect].origin.y + 100.0f, QCScreenWidth, QCScreenWidth)];
    }
