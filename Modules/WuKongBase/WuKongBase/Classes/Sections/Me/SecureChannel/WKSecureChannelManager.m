...
@interface WKSecureChannelManager ()
@property(nonatomic, assign, readwrite) BOOL enabled;
@property(nonatomic, copy, readwrite) NSString *displayName;
@end

@implementation WKSecureChannelManager

+ (instancetype)shared {
    static WKSecureChannelManager *instance;
...
- (void)refreshConfig:(void (^)(BOOL, NSString * _Nonnull))complete {
    ...
    [[WKAPIClient sharedClient] GET:@"common/secure_channel" parameters:nil].then(^(id result) {
        BOOL enabled = NO;
        NSString *name = @"";
...
- (void)verifyWithPassword:(NSString *)password
    ...
    [[WKAPIClient sharedClient] POST:@"common/secure_channel/verify" parameters:body].then(^(id result) {
        NSString *url = nil;
        if ([result isKindOfClass:[NSDictionary class]]) {
