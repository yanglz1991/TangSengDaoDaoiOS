...
#import "QCConstant.h"
@class QCGroupManager;
NS_ASSUME_NONNULL_BEGIN

// 成员数据缓存类型
typedef enum : NSUInteger {
    QCChannelMemberCacheTypeUnkown, // 未知
    QCChannelMemberCacheTypeDB, // db
    QCChannelMemberCacheTypeNetwork // 网络
} QCChannelMemberCacheType;

typedef enum : NSUInteger {
    QCGroupSettingKeyMute, // 免打扰
    QCGroupSettingKeyStick, // 置顶
    QCGroupSettingKeySave, //  保存
    QCGroupSettingKeyShowNick, // 显示昵称
    QCGroupSettingKeyInvite, // 成员邀请确认
    QCGroupSettingKeyForbidden, // 群禁言
    QCGroupSettingKeyForbiddenAddFriend, // 群内禁止互相加好友
    QCGroupSettingKeyScreenshot, // 截屏通知
    QCGroupSettingKeyRevokeRemind, // 撤回提醒
    QCGroupSettingKeyJoinGroupRemind, // 进群提醒
    QCGroupSettingKeyChatPwdOn, // 聊天密码开关
    QCGroupSettingKeyReceipt, // 回执开关
    QCGroupSettingKeyAllowViewHistoryMsg, // 允许新成员查看历史消息
    QCGroupSettingKeyRemark, // 群备注
    QCGroupSettingKeyFlame // 阅后即焚
} QCGroupSettingKey;

// 群公告
static NSString *QCGroupAttrKeyNotice = @"notice";
// 群名称
static NSString *QCGroupAttrKeyName = @"name";


@protocol QCGroupManagerDelegate <NSObject>
 ...
 */
-(void) groupManager:(QCGroupManager*)manager createGroup:(NSArray<NSString*>*)members object:(id __nullable)object complete:(void(^__nullable)(NSString*groupNo,NSError * __nullable error))complete;
...
/**
 ...
 @param members 群成员
 */
-(void) groupManager:(QCGroupManager*)manager groupNo:(NSString*)groupNo membersOfAdd:(NSArray<NSString*>*)members object:(id __nullable)object complete:(void(^__nullable)(NSError * __nullable error))complete;
 ...
 */
-(void) groupManager:(QCGroupManager*)manager groupNo:(NSString*)groupNo membersOfDelete:(NSArray<NSString*>*)members object:(id __nullable)object complete:(void(^__nullable)(NSError * __nullable error))complete;
...
/// @param complete <#complete description#>
-(void) groupManager:(QCGroupManager*)manager groupNo:(NSString*)groupNo membersToManager:(NSArray<NSString*>*) members complete:(void(^__nullable)(NSError * __nullable error))complete;
...
/// @param complete <#complete description#>
-(void) groupManager:(QCGroupManager*)manager groupNo:(NSString*)groupNo managersToMember:(NSArray<NSString*>*)managers complete:(void(^__nullable)(NSError * __nullable error))complete;
...
/**
 ...
 @param complete 同步完成
 */
-(void) groupManager:(QCGroupManager*)manager syncGroupInfo:(NSString*)groupNo complete:(void(^__nullable)(NSError * __nullable error,bool notifyBefore))complete;
...
/**
 ...
 @param complete 完成
 */
-(void) groupManager:(QCGroupManager*)manager syncMemebers:(NSString*)groupNo complete:(void(^__nullable)(NSInteger syncMemberCount,NSError * __nullable error))complete;
...
/**
 ...
 @param complete 完成回调
 */
-(void) groupManager:(QCGroupManager*)manager didMemberUpdateAtGroup:(NSString*)groupNo forMemberUID:(NSString*)memberUID withAttr:(NSDictionary*)attr complete:(void(^__nullable)(NSError * __nullable error))complete;


/// 退出群聊
/// @param manager <#manager description#>
/// @param groupNo 群编号
-(void) groupManager:(QCGroupManager*)manager didGroupExit:(NSString*)groupNo complete:(void(^__nullable)(NSError *error))complete;
...
/**
 ...
 @param on 是否开启
 */
-(void) groupManagerSetting:(QCGroupManager*)manager groupNo:(NSString*)groupNo settingKey:(QCGroupSettingKey)key on:(BOOL)on;
...
/**
 ...
 @param attrValue 属性值
 */
-(void) groupManagerUpdate:(QCGroupManager*)manager groupNo:(NSString*)groupNo attrKey:(NSString*)attrKey attrValue:(NSString*)attrValue complete:(void(^)(NSError * __nullable error))complete;

@end

@interface QCGroupManager : NSObject
+ (QCGroupManager *)shared;

@property(nonatomic,strong) id<QCGroupManagerDelegate> delegate;
 ...
 */
-(void) searchMembers:(QCChannel*)channel keyword:(NSString * __nullable)keyword limit:(NSInteger)limit complete:(void(^)(QCChannelMemberCacheType cacheType,NSArray<QCChannelMember*>*members))complete;
/**
  搜索成员
 @param keyword 根据关键字搜索
 @param page 开始页码
