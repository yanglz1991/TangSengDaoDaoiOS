//
//  QCConversationListHeaderView.m
//  WuKongBase
//
...
@interface QCConversationListHeaderView ()
...
@property(nonatomic,strong) QCPCOnlineBarView *pcOnlineBarView;



@end

@implementation QCConversationListHeaderView

- (instancetype)init
{
    self = [super initWithFrame:CGRectMake(0.0f, 0.0f, QCScreenWidth, 0.0f)];
    if (self) {
        [self setupUI];
    }
    return self;
...
- (void)viewConfigChange:(QCViewConfigChangeType)type{
    ...
    [self.pcOnlineBarView setBackgroundColor:[QCApp shared].config.backgroundColor];
}
...
- (void)setShowPCOnline:(BOOL)showPCOnline {
    ...
    if(showPCOnline) {
        self.showEmpty = false;
        self.pcOnlineBarView.tipLbl.text = [NSString stringWithFormat:LLang(@"%@%@已经登录"),[QCOnlineStatusManager.shared deviceName:self.pcDeviceFlag],[QCApp shared].config.appName];
...
- (QCSearchbarView *)searchbarView {
    if(!_searchbarView) {
        _searchbarView = [[QCSearchbarView alloc] initWithFrame:CGRectMake(15.0f, 0.0f, QCScreenWidth - 30.0f, 36.0f)];
...
- (UIView *)tableHeaderBottomEmptyView {
    if(!_tableHeaderBottomEmptyView) {
        _tableHeaderBottomEmptyView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, QCScreenWidth, 10.0f)];
...
- (QCPCOnlineBarView *)pcOnlineBarView {
    if(!_pcOnlineBarView) {
        _pcOnlineBarView = [[QCPCOnlineBarView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, QCScreenWidth, 48.0f)];
        [_pcOnlineBarView setBackgroundColor:[QCApp shared].config.backgroundColor];
...
-(void) onPCOnlineTap {
    QCPCOnlineVC *vc = [QCPCOnlineVC new];
    vc.mute = QCOnlineStatusManager.shared.muteOfApp;
...
@implementation QCPCOnlineBarView
...
- (UILabel *)tipLbl {
    if(!_tipLbl) {
        ...
        _tipLbl.font = [[QCApp shared].config appFontOfSize:14.0f];
    }
    return _tipLbl;
}
