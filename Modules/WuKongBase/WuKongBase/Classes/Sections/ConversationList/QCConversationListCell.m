...
@interface QCConversationListCell ()

@property(nonatomic,strong) UILabel *titleLbl; // 名称
@property(nonatomic,strong) QCUserAvatar *avatarImgView; // 头像
...
@property(nonatomic,strong) QCConversationWrapModel *model;

@property(nonatomic,strong) QCOnlineBadgeView *onlineBadgeView;

@property(nonatomic,strong) UIImageView *muteIcon;

@property(nonatomic,strong) QCOfficialTag *officialTag; // 官方图标

@property(nonatomic,copy) NSString *revokeTip; // 撤回消息tip

@property(nonatomic,strong) UIView *contextContainerView;

@property(nonatomic,strong) QCAutoDeleteView *autoDeleteView; // 自动删除

@end

@implementation QCConversationListCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.titleLbl setFont:[[QCApp shared].config appFontOfSizeMedium:17.0f]];
        [self.contextContainerView addSubview:self.titleLbl];
        
        
        self.avatarImgView = [[QCUserAvatar alloc] initWithFrame:CGRectMake(0, 0, [QCApp shared].config.messageListAvatarSize.width,  [QCApp shared].config.messageListAvatarSize.height)];
        [self.contextContainerView addSubview:self.avatarImgView];
        // 最后一条消息内容
        self.lastContentLbl = [[UILabel alloc] init];
        [self.lastContentLbl setFont:[[QCApp shared].config appFontOfSize:15.0f]];
        ...
        [self.lastMsgTimeLbl setFont:[[QCApp shared].config appFontOfSize:11.0f]];
        ...
        // 红点
        self.badgeView = [QCBadgeView viewWithoutBadgeTip];
...
- (QCOfficialTag *)officialTag {
    if(!_officialTag) {
        _officialTag = [QCOfficialTag new];
    }
    return _officialTag;
}
...
- (QCAutoDeleteView *)autoDeleteView {
    if(!_autoDeleteView) {
        _autoDeleteView = [[QCAutoDeleteView alloc] init];
    }
...
-(void) refreshWithModel:(QCConversationWrapModel*)model{
    ...
    // 最后一次消息时间
    self.lastMsgTimeLbl.text = [QCTimeTool getTimeStringAutoShort2:[NSDate dateWithTimeIntervalSince1970:model.lastMsgTimestamp] mustIncludeTime:true];
...
-(void) refreshAutoDeleteIfNeed:(QCConversationWrapModel*)model {
    BOOL hasChannelInfo  = model.channelInfo?true:false;
    if(!hasChannelInfo) {
...
-(void) refreshTitle:(QCConversationWrapModel*)model {
    BOOL hasChannelInfo  = model.channelInfo?true:false;
...
//        [[[QCSDK shared] channelManager] fetchChannelInfo:model.channel];
        ...
        if(model.channel.channelType == WK_PERSON) {
            if([model.channel.channelId isEqualToString:[QCApp shared].config.systemUID]) {
            ...
            }else if([model.channel.channelId isEqualToString:[QCApp shared].config.fileHelperUID]) {
...
-(void) refreshAvatar:(QCConversationWrapModel*)model {
    BOOL hasChannelInfo  = model.channelInfo?true:false;
    // 头像
    ...
    if(hasChannelInfo) {
        if([model.channelInfo.logo hasPrefix:@"http"]) {
            ...
            [self.avatarImgView.avatarImgView lim_setImageWithURL:[NSURL URLWithString:[QCAvatarUtil getFullAvatarWIthPath:model.channelInfo.logo]] placeholderImage:[self imageName:@"Common/Index/DefaultAvatar"] options:0 context:@{
                SDWebImageContextStoreCacheType: @(SDImageCacheTypeAll),
            }];
        }
    }
...
-(void) refreshOnlineStatus:(QCConversationWrapModel*)model {
...
-(void) refreshLastMessage:(QCConversationWrapModel*)model {
    // 最后一条消息
    if(model.lastMessage) {
        if(model.lastMessage.remoteExtra.revoke) {
            ...
            self.lastContentLbl.text = [QCApp shared].config.unkownMessageText;
        }else {
...
-(void) refreshTyping:(QCConversationWrapModel*)model {
    // 输入中
    self.typingIndicatorView.hidden = YES;
    [self.typingIndicatorView stopAnimating];
    QCMessage *typingMessage =  [[QCTypingManager shared] getTypingMessage:model.channel];
    if(typingMessage) {
        self.typingIndicatorView.hidden = YES;
        ...
        if(model.channel.channelType == WK_PERSON) {
            ...
            QCTypingContent *typingContent = (QCTypingContent*)typingMessage.content;
            NSString *typingName = typingContent.typingName;
            if(typingContent.typingUID) {
              QCChannelInfo *typingChannelInfo =  [[QCSDK shared].channelManager getChannelInfo:[QCChannel personWithChannelID:typingContent.typingUID]];
...
-(void) refreshUnread:(QCConversationWrapModel*)model {
    // 未读数
...
-(void) refreshSetting:(QCConversationWrapModel*)model {
    // 免打扰
    if(model.mute) { // 免打扰
        if(model.unreadCount<=0) {
    ...
    // 置顶
    if(model.stick) { // 置顶
        [self setBackgroundColor:[QCApp shared].config.backgroundColor];
    }else {
...
-(void) refreshStatus:(QCConversationWrapModel*)model {
    // 消息状态
...
-(void) refreshOfficialTag:(QCConversationWrapModel*)model {
    BOOL hasChannelInfo  = model.channelInfo?true:false;
    // 官方图标
    ...
    if(hasChannelInfo && model.channelInfo.category && ![model.channelInfo.category isEqualToString:@""]) {
        ...
        if([category isEqualToString:QCChannelCategoryService]) {
        ...
        }else if([category isEqualToString:QCChannelCategoryVisitor]) {
...
//    [self.statusImgView setBackgroundColor:[UIColor redColor]];
    QCMessage *message = self.model.lastMessage;
    if([self needLoading:message]) {
        ...
        self.statusImgView.tintColor =  [QCApp shared].config.tipColor;
        ...
        self.statusImgView.tintColor =  [QCApp shared].config.themeColor;
...
-(BOOL) needLoading:(QCMessage*)message {
...
- (NSString *)revokeTip {
    ...
    return [QCMessageRevokeCell tip:self.model.lastMessage];
}
...
-(NSMutableAttributedString*) getLastContent:(QCConversationWrapModel*)model{
    
    // 聊天密码
    BOOL chatPwdOn = model.channelInfo && [model.channelInfo settingForKey:QCChannelExtraKeyChatPwd defaultValue:false];
    if(chatPwdOn) {
    ...
    if(model.simpleReminders && model.simpleReminders.count>0) {
        for (QCReminder *reminder in model.simpleReminders) {
    ...
    QCRichTextParseOptions *options = [QCRichTextParseOptions new];
...
//    if(self.model.lastMessage.fromUid && [QCApp shared].loginInfo.extra[@"name"] && [self.model.lastMessage.fromUid isEqualToString:[QCApp shared].loginInfo.uid] ) {
//        name = [QCApp shared].loginInfo.extra[@"name"];
//    }
    // 名字显示逻辑： 个人备注>群内名字>昵称
    ...
    if(!name && self.model.lastMessage.from) {
        ...
        if([self.model.lastMessage.fromUid isEqualToString:[QCApp shared].config.systemUID]) {
            name = LLang(@"系统通知");
        }else if([self.model.lastMessage.fromUid isEqualToString:[QCApp shared].config.fileHelperUID]) {
            name = LLang(@"文件传输助手");
        }
    }
...
-(BOOL) showFromName:(QCConversationWrapModel*)model {
    return model.lastMessage && (model.lastMessage.fromUid && ![model.lastMessage.fromUid isEqualToString:@""]) && model.lastMessage.from && ![model.lastMessage.content isKindOfClass:[QCSystemContent class]];
}
