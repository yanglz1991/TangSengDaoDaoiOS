...
#import "QCApp.h"
#import "QCTimeTool.h"
@implementation QCChannelMessageModel

- (Class)cell {
    return QCChannelMessageCell.class;
}
...
@interface QCChannelMessageCell ()
...
@implementation QCChannelMessageCell
...
- (void)refresh:(QCChannelMessageModel *)model {
    [super refresh:model];
    
    self.avatarImgView.url = model.avatar;
    self.nameLbl.text = model.name;
    self.timestampLbl.text = [QCTimeTool getTimeStringAutoShort2:[NSDate dateWithTimeIntervalSince1970:model.timestamp.doubleValue] mustIncludeTime:NO];
    ...
    if(model.content && ![model.content isEqualToString:@""]) {
        ...
        if(model.keyword && ![model.keyword isEqualToString:@""]) {
            ...
            [contentAttr addAttribute:NSForegroundColorAttributeName value:[QCApp shared].config.themeColor range:colorRange];
        }
...
- (void)layoutSubviews {
    ...
    //time
    self.timestampLbl.lim_left = QCScreenWidth - self.timestampLbl.lim_width - 10.0f;
