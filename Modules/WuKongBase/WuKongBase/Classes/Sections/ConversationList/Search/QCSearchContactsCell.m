...
#import "QCApp.h"
#import "WuKongBase.h"
@implementation QCSearchContactsModel

- (Class)cell {
    return QCSearchContactsCell.class;
}
...
@interface QCSearchContactsCell ()

@property(nonatomic,strong) QCUserAvatar *avatarImgView;
@property(nonatomic,strong) UILabel *nameLbl;
@property(nonatomic,strong) UILabel *containLbl;
@property(nonatomic,strong) QCSearchContactsModel *searchModel;

@end

@implementation QCSearchContactsCell

+ (CGSize)sizeForModel:(QCFormItemModel *)model {
    return CGSizeMake(QCScreenWidth, 48.0f + 10.0f + 10.0f);
}
...
- (void)setupUI {
    ...
    // avatar
    self.avatarImgView = [[QCUserAvatar alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 48.0f, 48.0f)];
...
- (void)refresh:(QCSearchContactsModel *)model {
    [super refresh:model];
    ...
    if(model.contain && ![model.contain isEqualToString:@""]) {
        ...
        if(model.keyword) {
            ...
            [containAttr addAttribute:NSForegroundColorAttributeName value:[QCApp shared].config.themeColor range:colorRange];
        }
