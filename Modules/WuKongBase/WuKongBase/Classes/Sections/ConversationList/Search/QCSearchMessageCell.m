...
#import "QCApp.h"
#import "WuKongBase.h"
@implementation QCSearchMessageModel

- (Class)cell {
    return QCSearchMessageCell.class;
}
...
@implementation QCSearchMessageCell

+ (CGSize)sizeForModel:(QCFormItemModel *)model {
    return CGSizeMake(QCScreenWidth, QCDefaultAvatarSize.height + 10.0f + 10.0f);
}
...
- (void)setupUI {
    ...
    // avatar
    self.avatarImgView = [[QCUserAvatar alloc] init];
    ...
    [self.contentLbl setFont:[QCApp.shared.config appFontOfSize:14.0f]];
    [self.contentLbl setTextColor:[UIColor grayColor]];
    [self addSubview:self.contentLbl];
    
    // time
    self.timeLbl = [[UILabel alloc] init];
    [self.timeLbl setFont:[QCApp.shared.config appFontOfSize:12.0f]];
    [self.timeLbl setTextColor:[UIColor grayColor]];
    [self addSubview:self.timeLbl];
}

- (void)refresh:(QCSearchMessageModel *)model {
    [super refresh:model];
    
    NSString *avatar = @"";
    NSString *name = @"";
    
    QCChannelInfo *channelInfo = [QCSDK.shared.channelManager getChannelInfo:model.channel];
    if(channelInfo) {
        avatar = [QCApp.shared getImageFullUrl:channelInfo.logo].absoluteString;
    ...
    self.timeLbl.text = [QCTimeTool getTimeStringAutoShort2:[NSDate dateWithTimeIntervalSince1970:model.timestamp] mustIncludeTime:true];
...
-(NSMutableAttributedString*)  highlightText:(NSString*)text {
    ...
    for (NSTextCheckingResult* match in [matches reverseObjectEnumerator]) {
        ...
        NSAttributedString* highlightedString = [[NSAttributedString alloc] initWithString:content attributes:@{NSForegroundColorAttributeName: QCApp.shared.config.themeColor}];
        // 替换 <mark> 标签部分，并保留属性
