...
@interface QCEndToEndEncryptHitCell ()
...
@implementation QCEndToEndEncryptHitCell

+ (CGSize)sizeForMessage:(QCMessageModel *)model {
   
   CGSize size = [self getTextSize:hitText maxWidth:[QCApp shared].config.systemMessageContentMaxWidth fontSize:hitfontSize];
...
- (UILabel *)tipLbl {
    if(!_tipLbl) {
        ...
        _tipLbl.font = [[QCApp shared].config appFontOfSize:hitfontSize];
...
- (UIView *)tipBoxView {
    if(!_tipBoxView) {
        ...
        _tipBoxView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
    }
    return _tipBoxView;
}
