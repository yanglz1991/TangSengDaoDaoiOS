...
#import "WuKongBase.h"
@interface QCHistorySplitTipCell ()
...
@implementation QCHistorySplitTipCell
...
- (void)refresh:(QCMessageModel *)model {
    ...
    if([QCApp shared].config.style == QCSystemStyleDark) {
        ...
        [self.spliteLineView1 setBackgroundColor: [QCApp shared].config.tipColor];
        [self.spliteLineView2 setBackgroundColor: [QCApp shared].config.tipColor];
    }
}
...
- (UILabel *)tipLbl {
    if(!_tipLbl) {
        ...
        _tipLbl.font = [[QCApp shared].config appFontOfSize:13.0f];
