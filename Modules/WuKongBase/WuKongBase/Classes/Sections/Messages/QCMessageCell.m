...
@interface QCMessageCell ()<QCCheckBoxDelegate>
@property(nonatomic,strong) NSArray<QCMessageLongMenusItem*> *longMenusItems;
...
@property(nonatomic,strong) QCCircularProgressView *readedProgressView; // 已读进度
@property(nonatomic,strong) UIView *readedProgressViewBox; // readedProgressView的box为了增大点击面

@property(nonatomic,strong) QCReactionBaseView *reactionView;
...
//    }
    ...
    if(reason) {
        CGSize reasonSize = [self getTextSize:reason.string maxWidth:QCScreenWidth - errorTipLeftSpace - errorTipRightSpace font:[UIFont systemFontOfSize:errorTipFontSize]];
...
//    self.userInteractionEnabled = YES;
    ...
    self.tapLongTapOrDoubleTapGestureRecognizerWrap.tapActionAtPoint = ^QCTapLongTapOrDoubleTapGestureRecognizerEvent * _Nonnull(CGPoint point) {
...
//        QCTapLongTapOrDoubleTapGestureRecognizerEvent *event = [weakSelf tapActionAtPoint:point];
//        if(event.action != QCTapLongTapOrDoubleTapGestureRecognizerActionNone) {
//            return false;
//        }
...
//    self.bubbleBackgroundView.userInteractionEnabled = YES;
    ...
    // 消息正文
    self.messageContentView = [[QCMessageContentView alloc] initWithFrame:CGRectZero];
    ...
    // 头像
    self.avatarImgView = [[QCUserAvatar alloc] initWithFrame:CGRectMake(0, 0, [QCApp shared].config.messageAvatarSize.width, [QCApp shared].config.messageAvatarSize.height)];
...
//    [self.messageContentView addGestureRecognizer:longPressGesture];
    ...
    // 名字
    self.nameLbl = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, [QCApp shared].config.messageContentMaxWidth, WK_NICKNAME_HEIGHT)];
    ...
    // 回应
    self.reactionView = [[QCApp shared] invoke:QCPOINT_MESSAGEEXTEND_REACTIONVIEW param:nil];
    if(!self.reactionView) {
        self.reactionView = [[QCReactionBaseView alloc] init];
    }
...
-(QCTapLongTapOrDoubleTapGestureRecognizerEvent*) tapActionAtPoint:(CGPoint)point {
    ...
    QCTapLongTapOrDoubleTapGestureRecognizerEvent *event;
    if([self respondContentSingleTap] && CGRectContainsPoint(rectInContentView, point)) {
        event = [QCTapLongTapOrDoubleTapGestureRecognizerEvent action:(QCTapLongTapOrDoubleTapGestureRecognizerActionWaitForSingleTap)];
...
- (QCTrailingView *)trailingView {
    if(!_trailingView) {
        _trailingView = [[QCTrailingView alloc] init];
...
-(void) tappedMenuItem:(NSString*)title {
    if(self.longMenusItems) {
        for (QCMessageLongMenusItem *longItem in self.longMenusItems) {
...
- (void)refresh:(QCMessageModel *)model {
    ...
    QCBubblePostion bubblePosition = [[self class] bubblePosition:self.messageModel];
    ...
    if(!model.from && ![model.fromUid isEqualToString:@""]) { // 如果没有发送者信息则提取。
        [[QCSDK shared].channelManager fetchChannelInfo:[[QCChannel alloc] initWith:model.fromUid channelType:WK_PERSON]];
    }
    if(model.channel.channelType != WK_PERSON && !model.channelInfo) { // 没有频道信息则提取
        [[QCSDK shared].channelManager fetchChannelInfo:model.channel];
    }
    
    if([[self class] isShowName:model]) {
        self.nameLbl.hidden = NO;
        if(model.channel.channelType == WK_GROUP) {
            self.nameLbl.textColor = [QCUserColorUtil userColor:model.memberOfFrom.memberUid];
        }else {
            self.nameLbl.textColor =  [QCUserColorUtil userColor:model.from.channel.channelId];
        }
    ...
    if(model.isSend) {
        self.avatarImgView.url = [QCApp shared].loginInfo.extra[@"avatar"];
    }else {
        if(model.from) { // 如果有发送者信息
            self.avatarImgView.url = [QCAvatarUtil getFullAvatarWIthPath:model.from.logo];
        }else {
    ...
    CGSize trailingSize = [QCTrailingView size:self.messageModel];
    ...
    if(bubblePosition == QCBubblePostionMiddle || bubblePosition == QCBubblePostionFirst) {
    ...
    if(self.messageModel.content.flame && ![self hiddenFlameProgress] && !self.messageModel.flameFinished) {
        ...
        [self.messageModel startFlameIfNeed:^{
            [QCMessageManager.shared deleteMessages:@[weakMessageModel]];
        }];
    }
    if(self.messageModel.isSend) {
        self.bubbleBackgroundView.tintColor = QCApp.shared.config.themeColor;
    }else{
        self.bubbleBackgroundView.tintColor = QCApp.shared.config.cellBackgroundColor;
    }
}
// 获取发送者名字
+(NSString*) getFromName:(QCMessageModel*)messageModel {
    ...
    if([fromUID isEqualToString:[QCApp shared].config.systemUID]) {
        return LLang(@"系统通知");
    }else if([fromUID isEqualToString:[QCApp shared].config.fileHelperUID]) {
        return LLang(@"文件传输助手");
    }
    ...
    // 名字显示规则  个人备注 > 群内名字 > 昵称
    NSString *name = @"";
    QCChannelInfo *fromChannelInfo = [[QCSDK shared].channelManager getChannelInfo:[QCChannel personWithChannelID:messageModel.fromUid]];
    if(fromChannelInfo) {
...
// 获取消息发送的设备
+(NSString*) getDeviceName:(QCMessageModel*)messageModel {
...
+(CGSize) getNicknameSize:(QCMessageModel*)messageModel {
    ...
    CGSize nicknameSize =  [self getTextSize:nickname maxWidth:[QCApp shared].config.messageContentMaxWidth font:WK_NICKNAME_FONT];
    return nicknameSize;
}
...
+(QCBubblePostion) bubblePosition:(QCMessageModel*)messageModel {
    ...
    if( [self preIsSameFrom:messageModel] && [self nextIsSameFrom:messageModel] && ![self preIsSystemOrRevoke:messageModel] && ![self nextIsSystemOrRevoke:messageModel] && [self preIsSameDay:messageModel] && [self nextIsSameDay:messageModel]) {
        return QCBubblePostionMiddle;
    }else if(!nextContiue) {
        return QCBubblePostionLast;
    }else if(!preContinue) {
        return QCBubblePostionFirst;
    }
    return QCBubblePostionUnknown;
}

// 下一条消息是否连续
+(BOOL) nextIsContinue:(QCMessageModel*) messageModel {
    
    return [self nextIsSameFrom:messageModel] && ![self nextIsSystemOrRevoke:messageModel] && [self nextIsSameDay:messageModel];
}

// 上一条是否连续
+(BOOL) preIsContinue:(QCMessageModel*)messageModel {
    return [self preIsSameFrom:messageModel] && ![self preIsSystemOrRevoke:messageModel] && [self preIsSameDay:messageModel];
}

// 下一条消息是同一天
+(BOOL) nextIsSameDay:(QCMessageModel*)messageModel {
...
+(BOOL) preIsSameDay:(QCMessageModel*)messageModel {
...
// 下一条消息是否是系统消息(或者被撤回)
+(BOOL) nextIsSystemOrRevoke:(QCMessageModel*)messageModel {
    return [self isSystemOrRevoke:messageModel.nextMessageModel];
}

+(BOOL) isSystemOrRevoke:(QCMessageModel*)messageModel {
    
    return messageModel && ([[QCSDK shared] isSystemMessage:messageModel.contentType]||messageModel.revoke || messageModel.contentType == WK_HISTORY_SPLIT);
}

// 上一条消息是否是系统消息(或者被撤回) 则为第一条消息
+(BOOL) preIsSystemOrRevoke:(QCMessageModel*)messageModel {
    return [self isSystemOrRevoke:messageModel.preMessageModel];
}

// 上一条消息是否是同一个发送者
+(BOOL) preIsSameFrom:(QCMessageModel*)messageModel {
    return messageModel.preMessageModel && [messageModel.preMessageModel.fromUid isEqualToString:messageModel.fromUid];
}

// 下一条消息是否是同一个发送者
+(BOOL) nextIsSameFrom:(QCMessageModel*)messageModel {
...
-(void) startReminderAnimation:(NSInteger)count white:(BOOL)white{
    ...
    animation.toValue = (id)[QCApp shared].config.themeColor.CGColor;
...
-(BOOL) needError:(QCMessageModel*)model {
...
-(void) layoutSubviews {
    ...
    if(self.messageModel.isSend) {
        avatarLeft =  QCScreenWidth - avatarSpace - self.avatarImgView.lim_width;
    }else {
    ...
    if(self.showCheckBox) {
        ...
        if(self.messageModel.isSend) {
            //checkBoxLeft = QCScreenWidth - checkBoxSpace - self.checkBox.lim_width;
...
-(UIImage*) bubbleImage {
    ...
    NSString *imgName;
    if(self.messageModel.isSend) {
        if(bubblePosition == QCBubblePostionFirst) {
            imgName = @"Conversation/Messages/MessageSendBubbleFirst";
        }else  if(bubblePosition == QCBubblePostionMiddle){
...
-(void) layoutName {
    ...
    if(!self.nameLbl.hidden) {
        if(position == QCBubblePostionLast || position == QCBubblePostionSingle) {
            self.nameLbl.lim_left = QCLastBubbleOffsetSpace;
        }else{
...
-(void) sendFailPressed {
    __weak typeof(self) weakSelf = self;
    [QCAlertUtil alert:LLang(@"重发该消息") buttonsStatement:@[LLang(@"取消"),LLang(@"重发")] chooseBlock:^(NSInteger buttonIdx) {
        if(buttonIdx == 1) {
...
//    return UIEdgeInsetsMake(WK_CONTENT_INSETS.top, WK_CONTENT_INSETS.left, WK_CONTENT_INSETS.bottom, WK_CONTENT_INSETS.right+size.width);
    ...
    if([self hiddenBubble]) {
        ...
        if(position == QCBubblePostionLast || position == QCBubblePostionSingle) {
            if(model.isSend) {
                return UIEdgeInsetsMake(0, 0,0,QCLastBubbleOffsetSpace);
            }
            return UIEdgeInsetsMake(0, QCLastBubbleOffsetSpace,0,0);
        }
    ...
    if(model.isSend) {
        if(position == QCBubblePostionLast || position == QCBubblePostionSingle) { // 最后一条消息
            return UIEdgeInsetsMake(WK_CONTENT_INSETS.top, WK_CONTENT_INSETS.left + QCLastBubbleOffsetSpace, WK_CONTENT_INSETS.bottom, WK_CONTENT_INSETS.right+QCLastBubbleOffsetSpace);
        }
        return UIEdgeInsetsMake(WK_CONTENT_INSETS.top, WK_CONTENT_INSETS.left+QCLastBubbleOffsetSpace, WK_CONTENT_INSETS.bottom, WK_CONTENT_INSETS.right);
    }else{
        if(position == QCBubblePostionLast || position == QCBubblePostionSingle) {
            return UIEdgeInsetsMake(WK_CONTENT_INSETS.top, WK_CONTENT_INSETS.left+QCLastBubbleOffsetSpace, WK_CONTENT_INSETS.bottom, WK_CONTENT_INSETS.right);
        }
...
+(UIEdgeInsets) bubbleEdgeInsets:(QCMessageModel*) model contentSize:(CGSize)contentSize{
    ...
    if(model.contentType != WK_TYPING) {
        if(position == QCBubblePostionFirst || position == QCBubblePostionSingle ) {
    ...
    if(position == QCBubblePostionLast || position == QCBubblePostionSingle) { // 最后一条消息
        bottom +=10.0f;
        if(model.isSend) {
            right -= QCLastBubbleOffsetSpace;
        }else{
            left-= QCLastBubbleOffsetSpace;
        }
        
    }
//    if(model.reactions && model.reactions.count>0) {
//        bottom += [QCReactionView height];
//    }
...
- (UILabel *)errorLbl {
    if(!_errorLbl) {
        ...
        _errorLbl.lim_width = QCScreenWidth - errorTipLeftSpace - errorTipRightSpace;
        _errorLbl.textColor = [QCApp shared].config.tipColor;
        _errorLbl.font = [[QCApp shared].config appFontOfSize:errorTipFontSize];
        [_errorLbl onClick:^(id<QCMatchToken> token) {
            if(token.type == WKatchTokenTypeLink2) {
                QCLinkToken *linkToken = (QCLinkToken*)token;
                NSURL *url = [NSURL URLWithString:linkToken.linkContent];
                if([url.scheme hasPrefix:[QCApp shared].config.appSchemaPrefix]) {
                    [[QCSchemaManager shared]handleURL:url];
                }
            }
...
+(BOOL) isShowName:(QCMessageModel*)model {
    QCBubblePostion position = [self bubblePosition:model];
    return  !model.isSend && model.channel.channelType != WK_PERSON &&  (position == QCBubblePostionFirst || position == QCBubblePostionSingle);
}

#define mark -- QCCheckBoxDelegate
...
@interface QCBubbleBackgroundView ()
...
@implementation QCBubbleBackgroundView
