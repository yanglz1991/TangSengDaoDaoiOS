...
- (void)decodeWithJSON:(NSDictionary *)contentDic {
    ...
    if(msgDicts && msgDicts.count>0) {
        for (NSDictionary *msgDict in msgDicts) {
            [messages addObject:[QCMessageUtil toMessage:msgDict]];
        }
    }
    self.msgs = messages;
...
- (NSDictionary *)encodeWithJSON {
    ...
    if(self.msgs && self.msgs.count>0) {
        ...
        for (QCMessage *message in self.msgs) {
...
-(NSDictionary*) messageToDict:(QCMessage*)message {
