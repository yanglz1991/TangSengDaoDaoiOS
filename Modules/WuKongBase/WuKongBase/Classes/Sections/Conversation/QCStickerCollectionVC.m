...
@interface QCStickerCollectionVC () <UICollectionViewDataSource,UICollectionViewDelegate>

@property (nonatomic, strong) UIButton *rightBtn;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) QCCollectionViewGridLayout *newGridLayout;
...
@implementation QCStickerCollectionVC
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        [[_rightBtn titleLabel] setFont:[[QCApp shared].config appFontOfSize:15.0f]];
        [_rightBtn setTitleColor:[QCApp shared].config.defaultTextColor forState:UIControlStateNormal];
...
- (QCCollectionViewGridLayout *)newGridLayout {
    if (!_newGridLayout) {
        _newGridLayout = [QCCollectionViewGridLayout new];
...
- (UICollectionView *)collectionView {
    if(!_collectionView) {
        ...
        _collectionView.backgroundColor =  QCApp.shared.config.backgroundColor;
        ...
        [_collectionView registerClass:[QCStickerCollectAddCell class] forCellWithReuseIdentifier:[QCStickerCollectAddCell reuseIdentifier]];
    }
...
- (UIView *)footerView {
    if(!_footerView) {
        ...
        _footerView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, self.view.lim_height, QCScreenWidth, height)];
        _footerView.backgroundColor = QCApp.shared.config.cellBackgroundColor;
...
- (UIButton *)moveFontBtn {
    if(!_moveFontBtn) {
        ...
        [_moveFontBtn.titleLabel setFont:[QCApp.shared.config appFontOfSize:14.0f]];
        [_moveFontBtn setTitleColor:QCApp.shared.config.defaultTextColor forState:UIControlStateNormal];
...
- (UIButton *)deleteBtn {
    if(!_deleteBtn) {
        ...
        [_deleteBtn.titleLabel setFont:[QCApp.shared.config appFontOfSize:14.0f]];
...
-(BOOL) stickerHasSelected {
    for (id model in self.dataArray) {
        if([model isKindOfClass:[QCSticker class]]) {
            QCSticker *sticker = (QCSticker*)model;
...
-(void) deletePressed {
    QCActionSheetView2 *sheet = [QCActionSheetView2 initWithTip:LLang(@"删除的表情无法恢复")];
    __weak typeof(self) weakSelf = self;
    [sheet addItem:[QCActionSheetButtonItem2 initWithAlertTitle:LLang(@"删除") onClick:^{
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *identifier;
    if([cellModel isKindOfClass:[QCStickerCollectAddCellModel class]]) { // add
        identifier = [QCStickerCollectAddCell reuseIdentifier];
    }else {
        identifier = [QCStickerGIFCell reuseIdentifier];
    }
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(QCStickerGIFCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([cellModel isKindOfClass:[QCStickerCollectAddCellModel class]]) { // add
        ...
        QCSticker *sticker = (QCSticker*)cellModel;
        __weak typeof(self) weakSelf = self;
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if([cell isKindOfClass:[QCStickerGIFCell class]]) {
        [(QCStickerGIFCell*)cell onEndDisplay];
    }
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    id cellModel = _dataArray[indexPath.row];
    if([cellModel isKindOfClass:[QCStickerCollectAddCellModel class]]) {
...
//            [self deleteEmojisInStickerCollectionVC];
        ...
        for (id model in _dataArray) {
            if([model isKindOfClass:[QCSticker class]]) {
                QCSticker *resp = (QCSticker*)model;
                resp.isSelected = NO;
                resp.isEdit = _isEdit;
            }
...
- (void)openAlbumInStickerCollectionVC {
    ...
    [[QCPhotoBrowser shared] showPhotoLibraryWithSender:self selectCompressImageBlock:^(NSArray<NSData *> * _Nonnull images, NSArray<PHAsset *> * _Nonnull assets, BOOL isOriginal) {
        if(images.count>0) {
...
- (void)getNewEmojiAddressInStickerCollectionVC:(NSData *)imageData {
    ...
    __weak typeof(self) weakSelf = self;
    [[QCAPIClient sharedClient] GET:@"file/upload?type=sticker" parameters:nil].then(^(NSDictionary *resultDict) {
...
- (void)updateNewEmojiInStickerCollectionVC:(NSData *)imageData urlString:(NSString *)string {
    __weak typeof(self) weakSelf = self;
    [[QCAPIClient sharedClient] fileUpload:string data:imageData progress:^(NSProgress * _Nonnull progress) {
        QCLogDebug(@"progress:%@", @(progress.fractionCompleted));
...
- (void)getAllEmojisInStickerCollectionVC {
    ...
    [QCApp.shared loadCollectStickers].then(^(NSArray *stickerArray){
...
- (QCStickerCollectAddCellModel *)setDefaultSticker {
    QCStickerCollectAddCellModel *defaultModel = [QCStickerCollectAddCellModel new];
...
- (void)deleteEmojisInStickerCollectionVC {
    ...
    for (QCSticker *resp in _dataArray) {
        if (resp.isSelected) {
    ...
    if (deleteArray.count > 0) {
        ...
        [[QCAPIClient sharedClient] DELETE:@"sticker/user" parameters:@{@"paths":deleteArray}];
        [self refreshFootViewStatus];
    }
    return;
}
...
-(void) stickerMoveFont {
    ...
    if(moveArray.count>0) {
        ...
        [[QCAPIClient sharedClient] PUT:@"sticker/user/front" parameters:@{@"paths":movePathArray}];
    }
}
