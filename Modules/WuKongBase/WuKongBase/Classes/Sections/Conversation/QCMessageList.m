...
@interface QCMessageList ()

@property(nonatomic,strong) NSLock *messagesLock;

@property(nonatomic,strong) NSMutableDictionary<NSString*,NSMutableArray<QCMessageModel*>*> *dateMessageGroups; // 通过日期对消息分组

@end

@implementation QCMessageList
...
-(void) insertMessage:(QCMessageModel*)model {
    if(model.contentType == WK_TEXT) {
       QCTextContent *content = (QCTextContent*)model.content;
    ...
    if(messages.count>0) {
        QCMessageModel *oldMessageModel = messages.firstObject;
...
-(void) addMessages:(NSArray<QCMessageModel*>*)messages {
    for (QCMessageModel *message in messages) {
...
-(NSArray<QCMessageModel*>*) messagesAtDate:(NSString*)date {
    [_messagesLock lock];
    NSArray<QCMessageModel*> *messages =  self.dateMessageGroups[date];
...
-(void) addMessage:(QCMessageModel*)message {
    ...
    QCMessageModel *typingMessageModel;
    QCMessageModel *lastMessage = [self lastMessage];
...
-(void) _insertMessage:(QCMessageModel*)message atIndex:(NSIndexPath*)indexPath {
    if(!indexPath || indexPath.section >= self.dates.count) {
        return;
    }
    
   NSMutableArray<QCMessageModel*> *messages =  self.dateMessageGroups[self.dates[indexPath.section]];
    if(messages.count>0) {
        if (indexPath.row == 0) { // 插入到最前面
            QCMessageModel *oldFirstMessage = messages[0];
            ...
            QCMessageModel *currentMessage = messages[indexPath.row];
            ...
            QCMessageModel *oldLastMessage = messages[messages.count-1];
...
// 替换最新的消息
-(void) replaceMessageLast:(QCMessageModel*)model {
    [self handleProhibitwords:model]; // 处理违禁词
    NSString *date = self.dates.lastObject;
    NSMutableArray *messages = self.dateMessageGroups[date];
    if(messages && messages.count>0) {
        QCMessageModel *oldMessageModel = messages.lastObject;
...
-(NSIndexPath*) replaceMessage:(QCMessageModel*)newMessage atIndexPath:(NSIndexPath*)path {
    ...
    if(path) {
       NSMutableArray *messages =  self.dateMessageGroups[self.dates[path.section]];
        ...
        if(path.row < messages.count) {
            QCMessageModel *oldMessage =  messages[path.row];
...
-(void) addMessageOnly:(QCMessageModel *)message {
...
-(void) handleProhibitwords:(QCMessageModel*)messageModel {
    if(messageModel.contentType == WK_TEXT) {
        if(messageModel.remoteExtra.isEdit) {
            QCTextContent *content = (QCTextContent*)messageModel.remoteExtra.contentEdit;
            content.content =[QCProhibitwordsService.shared filter:content.content]; // 违禁词过滤
            return;
        }
        QCTextContent *content = (QCTextContent*)messageModel.content;
        content.content = [QCProhibitwordsService.shared filter:content.content]; // 违禁词过滤
    }
}
    ...
    for (NSInteger i=self.dates.count-1; i>=0; i--) {
        ...
        if(messages && messages.count>0) {
            for (NSInteger j=messages.count-1;j>=0; j--) {
                QCMessageModel *messageModel = messages[j];
...
-(NSArray<QCMessageModel*>*) messagesAtMessageReply:(uint64_t)messageID {
    ...
    [_messagesLock lock];
    NSMutableArray<QCMessageModel*> *resultMessages = [NSMutableArray array];
    for (NSInteger i=self.dates.count-1; i>=0; i--) {
        NSMutableArray *messages = self.dateMessageGroups[self.dates[i]];
        if(messages && messages.count>0) {
            for (NSInteger j=messages.count-1;j>=0; j--) {
...
-(NSIndexPath*) removeMessage:(QCMessageModel*) message {
    ...
    if(path) {
        ...
        QCMessageModel *deleteMessageModel = messages[path.row];
...
-(NSInteger) messageCount {
   ...
   __block NSInteger count = 0;
    [self.dateMessageGroups enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSMutableArray<QCMessageModel *> * _Nonnull obj, BOOL * _Nonnull stop) {
        if(obj) {
            count+=obj.count;
        }
    }];
...
-(NSIndexPath*) replaceTyping:(QCMessageModel*)messageModel {
    [self.messagesLock lock];
    NSIndexPath *indexPath = [self typingIndexPath];
    BOOL hasTyping = true;
    if(!indexPath) {
    ...
    if(hasTyping) {
        QCMessageModel *typingMessageModel = self.dateMessageGroups[self.dates[indexPath.section]][indexPath.row];
...
- (NSIndexPath*)typingIndexPath {
    ...
    if(self.dates.count>0) {
        ...
        if(messages && messages.count>0) {
            QCMessage *message = messages[messages.count-1];
...
-(NSArray<QCMessageModel*>*) getMessagesWithContentType:(NSInteger)contentType {
    __block NSMutableArray<QCMessageModel*> *filterModels = [NSMutableArray array];
     [self.messagesLock lock];
    [self.dateMessageGroups enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSMutableArray<QCMessageModel *> * _Nonnull messages, BOOL * _Nonnull stop) {
...
-(NSString*) formatMessageDate:(QCMessageModel*)model {
...
- (NSMutableDictionary<NSString *,NSMutableArray<QCMessageModel *> *> *)dateMessageGroups {
