...
class QCChatMessageContextExtractedContentSource : ContextExtractedContentSource {
    ...
    func takeView() -> ContextControllerTakeViewInfo? {
        NSLog("QCChatMessageContextExtractedContentSource---->takeView")
        guard let visibleCells = self.context.visibleCells?() else {
            return nil
        }
        var result: ContextControllerTakeViewInfo?
        visibleCells.forEach { cell in
            guard let messageCell = cell as? QCMessageCell else {
                return
            }
            if(self.message.clientMsgNo == messageCell.messageModel.clientMsgNo) {
                ...
                let topView = QCNavigationManager.shared().topViewController().view!;
                let y = QCApp.shared().config.visibleEdgeInsets.top + QCApp.shared().config.visibleEdgeInsets.bottom
    ...
    let context:QCConversationContext
    let message:QCMessageModel;
    init(message:QCMessageModel,context:QCConversationContext) {
...
@objc public class QCMessageContextController:NSObject {
    var contextController:ContextController?
    let message:QCMessageModel;
    let context:QCConversationContext
    let gesture:ContextGesture?
    let menusItems: [QCMessageLongMenusItem]

    
    @objc public var reactionSelected: ((QCReactionContextItem,Bool) -> Void)?
    
    @objc public var onDismissed: OnDismissed?
    private(set) var controllers: [UIViewController] = []
    @objc public  init(message:QCMessageModel,context:QCConversationContext,menusItems:[QCMessageLongMenusItem],gesture:ContextGesture?) {
        self.message = message
        self.context = context
    ...
    @objc public func setup() {
        ...
        // 回应表情
        let reactionItems = QCApp.shared().invoke(QCPOINT_LONGMENUS_REACTIONS, param: ["message":self.message,"context":self.context] as [String : Any]) as? [ReactionContextItem]
    ...
    @objc public func show() {
        let window = QCApp.shared().findWindow()
...
//            QCNavigationManager.shared().topViewController().addChild(contextController)
            QCNavigationManager.shared().topViewController().addChild(contextController)
    ...
    @objc public func containerLayoutUpdated(size:CGSize) {
        let visibleEdgeInsets = QCApp.shared().config.visibleEdgeInsets
