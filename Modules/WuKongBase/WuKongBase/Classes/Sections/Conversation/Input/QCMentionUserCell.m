...
@implementation QCMentionUserCellModel

+(instancetype) uid:(NSString*)uid  name:(NSString*)name avatarURL:(NSURL*)avatarURL robot:(BOOL)robot{
    QCMentionUserCellModel *model = [QCMentionUserCellModel new];
    model.uid = uid;
    model.name = name;
...
- (NSString *)name {
    QCChannelInfo *channelInfo = [QCSDK.shared.channelManager getCache:[QCChannel personWithChannelID:self.uid]];
    if(channelInfo) {
...
@implementation QCMentionUserCell
...
- (void)refresh:(QCMentionUserCellModel *)model {
    [super refresh:model];
...
- (QCUserAvatar *)avatarImgView {
    if(!_avatarImgView) {
        CGSize avatarSize = [QCApp shared].config.smallAvatarSize;
        _avatarImgView = [[QCUserAvatar alloc] initWithFrame:CGRectMake(0.0f, 0.0f, avatarSize.width, avatarSize.height)];
    }
...
- (UIImageView *)robotIdentityImgView {
    if(!_robotIdentityImgView) {
        ...
        [_robotIdentityImgView setTintColor:[QCApp shared].config.themeColor];
    }
...
- (UILabel *)nameLbl {
    if(!_nameLbl) {
        ...
        _nameLbl.textColor = [QCApp shared].config.defaultTextColor;
