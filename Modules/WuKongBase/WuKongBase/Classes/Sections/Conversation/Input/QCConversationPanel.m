...
-(BOOL) switchPanel:(QCChannel*)channel pointId:(NSString*)pointId{
...
//    [[UIDevice currentDevice] playInputClick]; // 播放input声音
    ...
    self.lim_size = CGSizeMake(QCScreenWidth, [self currentPanelHeight]);
...
-(void) initNewPanel:(QCChannel*)channel pointId:(NSString*)pointId{
    QCPanel *panel =  [self panelWithPointId:pointId context:self.conversationContext];
    if(panel) {
        _currPanelPointId = pointId;
        _currentPanel = panel;
         [panel layoutPanel:QCPanelDefaultHeight];
        _currentPanel.frame =CGRectMake(0,_currentPanel.lim_height, QCScreenWidth,_currentPanel.lim_height);
    }
}
 ...
 */
-(QCPanel*) panelWithPointId:(NSString*) pointId context:(id<QCConversationContext>) context{
   
    return  [ [QCApp shared] invoke:pointId param:@{@"context":context}];
}
