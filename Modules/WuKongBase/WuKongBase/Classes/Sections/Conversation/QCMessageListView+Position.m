...
@implementation QCMessageListView (Position)


-(void) initPosition {
    self.positionAtBottom = true;
    self.conversationPositionBarView = [[QCConversationPositionBarView alloc] init];
    __weak typeof(self) weakSelf = self;
    [self.conversationPositionBarView setOnScrollToBottom:^{
        [weakSelf pullBottom];
    }];
    [self.conversationPositionBarView setOnScrollToPosition:^(QCConversationPosition * _Nonnull position,UITableViewScrollPosition tablePosition) {
...
-(void) updatePostionReminders:(NSArray<QCReminder*>*) reminders force:(BOOL)force{
    NSMutableArray<QCReminder*> *locateReminders = [NSMutableArray array];
    ...
    for (NSInteger i = 0; i<visibleRows.count; i++) {
        ...
        CGRect rect =  [self.tableView rectForRowAtIndexPath:visibleRow];
         if([self cellIsVisible:rect]) {
            QCMessageModel *messageModel = [self.dataProvider messageAtIndexPath:visibleRow];
             if(messageModel) {
