...
#import "WuKongBase.h"
@interface QCEmojiContentView ()<UICollectionViewDataSource,UICollectionViewDelegate>

@property(nonatomic,strong) UICollectionView *collectionView;

// emoji服务
@property(nonatomic,strong) QCEmoticonService *emojiService;

@end

@implementation QCEmojiContentView

- (instancetype)init
{
    self = [super init];
    if (self) {
        self.emojiService = [QCEmoticonService new];
...
//    layout.sectionsStartOnNewLine = NO;
    ...
    CGSize itemSize = CGSizeMake((QCScreenWidth-itemSpaceing*(num+1))/num, (QCScreenWidth-itemSpaceing*(num+1))/num);
    ...
    if(self.emojiService.recentEmotions.count >0) {
        layout.headerReferenceSize = CGSizeMake(QCScreenWidth, 30.0f);
    }
...
-(UICollectionView*) collectionView {
    if(!_collectionView) {
        ...
        [_collectionView registerClass:[QCEmojiCell class] forCellWithReuseIdentifier:[QCEmojiCell reuseIdentifier]];
        [_collectionView registerClass:[QCEmojiCollectionTitleHeader class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"QCEmojiCollectionTitleHeader"];
    }
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    QCEmojiCollectionTitleHeader *headerView = (QCEmojiCollectionTitleHeader*)[collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"QCEmojiCollectionTitleHeader" forIndexPath:indexPath];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    QCEmojiCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[QCEmojiCell reuseIdentifier] forIndexPath:indexPath];
    
    return cell;
}
-(void)collectionView:(UICollectionView *)collectionView willDisplayCell:(QCEmojiCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath{
    QCEmotion *emoji ;
    if(indexPath.section == 0 && [self hasRecentEmotion]) {
...
- (UIImage *)tabIcon {
    return [QCApp.shared loadImage:@"Conversation/Panel/IconFaceEmoji" moduleID:@"WuKongBase"];
}
