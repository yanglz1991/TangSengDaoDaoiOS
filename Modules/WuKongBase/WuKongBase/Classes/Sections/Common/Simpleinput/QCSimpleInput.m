...
#define QCPanelDefaultHeight (300.0f + [self safeBottom])// 面板默认高度
...
@interface QCSimpleInput ()<QCGrowingTextViewDelegate>
...
@property(nonatomic,strong) QCSimpleEmojiPanel *emojiPanel;



@end

@implementation QCSimpleInput
...
-(void) setupUI {
    ...
    self.frame = CGRectMake(0.0f, 0.0f, QCScreenWidth, 0.0f);
...
- (void)layoutSubviews {
    ...
    self.lim_top = QCScreenHeight - [self inputTotalHeight];
...
- (UIView *)panelView {
    if(!_panelView) {
        ...
        [_panelView setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
    }
    return _panelView;
}

- (QCSimpleEmojiPanel *)emojiPanel {
    if(!_emojiPanel) {
        _emojiPanel = [[QCSimpleEmojiPanel alloc] init];
        [_emojiPanel layoutPanel:0.0f];
        __weak typeof(self) weakSelf = self;
        [_emojiPanel setOnEmoji:^(QCEmotion * _Nonnull emoji) {
...
-(void) switchPanel {
    ...
    if(selected) {
        self.panelHeight = QCPanelDefaultHeight;
