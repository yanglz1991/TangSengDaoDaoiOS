...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        [_titleLabel setTextColor:[QCApp shared].config.navBarTitleColor];
        
        CGFloat statusHeight = [UIApplication sharedApplication].statusBarFrame.size.height;
        _titleLabel.lim_top = statusHeight + 10.0f;
        [_titleLabel setFont:[[QCApp shared].config appFontOfSizeMedium:17.0f]];
    }
    return _titleLabel;
}
...
- (UILabel *)subtitleLabel {
    if(!_subtitleLabel) {
        ...
        [_subtitleLabel setTextColor:[QCApp shared].config.navBarSubtitleColor];
        [_subtitleLabel setFont:[[QCApp shared].config appFontOfSize:10.0f]];
    }
    return _subtitleLabel;
}

- (void)setStyle:(QCNavigationBarStyle)style {
    _style = style;
    [self.titleLabel setTextColor:[QCApp shared].config.navBarTitleColor];
    UIImage *img;
    if(style == QCNavigationBarStyleWhite || style == QCNavigationBarStyleDark) {
...
- (void)setLargeTitle:(BOOL)largeTitle {
    ...
    if(self.largeTitle) {
        [self.titleLabel setFont:[[QCApp shared].config appFontOfSizeMedium:25.0f]];
    }else{
        [self.titleLabel setFont:[[QCApp shared].config appFontOfSizeMedium:17.0f]];
    }
...
- (void)setSubtitle:(NSString *)subtitle {
    ...
    if(subtitle && ![subtitle isEqualToString:@""]) {
        ...
        self.subtitleLabel.lim_left = QCScreenWidth/2.0f - self.subtitleLabel.lim_width/2.0f;
    }else {
...
- (UIButton *)backButton {
    if(!_backButton) {
        ...
        [_backButton setTintColor:QCApp.shared.config.navBarButtonColor];
