...
@interface QCMediaPickerController : TZImagePickerController

@end

@implementation QCMediaPickerController
...
@interface QCMediaFetcher()<TZImagePickerControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>

@property (nonatomic,copy)   QCLibraryFetchResult libraryResultHandler;

@property (nonatomic,copy)   QCCameraFetchResult  cameraResultHandler;

@property (nonatomic,strong) UIImagePickerController  *imagePicker;

@property (nonatomic,weak) QCMediaPickerController  *assetsPicker;

@end

@implementation QCMediaFetcher
...
- (void)fetchPhotoFromLibrary:(QCLibraryFetchResult)result cancel:(void (^)(void))cancel
{
    ...
    [self setUpPhotoLibrary:^(QCMediaPickerController * _Nullable picker) {
        if (picker && weakSelf) {
                ...
                if (path) {
                    ...
                    if(isSelectOriginalPhoto) {
                        ...
                        if(format == JLImageFormatGIF)  {
                            [UIImage jl_compressWithImageGIF:imageData targetSize:image.size targetByte:[QCApp shared].config.imageMaxLimitBytes handler:^(NSData * _Nullable compressedData, CGSize gifImageSize, NSError * _Nullable error) {
                                if(error) {
                                    [[QCNavigationManager shared].topViewController.view showMsg:LLangW(@"图片压缩错误！",weakSelf)];
                                    return;
                                }
                                result(imageData,path,isSelectOriginalPhoto,type,left);
                            ...
                            NSData *imgData = [UIImage jl_compressImageSize:image toByte:[QCApp shared].config.imageMaxLimitBytes];
                            result(imgData,path,isSelectOriginalPhoto,type,left);
                        }
                    }
                    
                }else if(img) {
                    NSData *imgData  = [UIImage jl_compressImageSize:img toByte:[QCApp shared].config.imageMaxLimitBytes];
                    result(imgData,path,isSelectOriginalPhoto,type,left);
                }
                break;
            }
...
- (void)setUpPhotoLibrary:(void(^)(QCMediaPickerController * _Nullable picker)) handler
{
    __weak typeof(self) weakSelf = self;
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status){
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (status == PHAuthorizationStatusAuthorized) {
                QCMediaPickerController *vc = [[QCMediaPickerController alloc] initWithMaxImagesCount:weakSelf.limit delegate:weakSelf];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeMovie]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            NSString *outputFileName = [QCFileLocationHelper genFilenameWithExt:@"mp4"];
            NSString *outputPath = [QCFileLocationHelper filepathForVideo:outputFileName];
...
- (void)requestAssets:(NSMutableArray *)assets isSelectOriginalPhoto:(BOOL) isSelectOriginalPhoto result:(QCLibraryFetchResult)result
{
    if (!assets.count) {
        return;
    }
