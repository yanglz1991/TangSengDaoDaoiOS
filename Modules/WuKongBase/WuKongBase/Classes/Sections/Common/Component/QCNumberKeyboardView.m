...
@interface QCNumberKeyboardView ()
...
@implementation QCNumberKeyboardView
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.topViewController = [QCNavigationManager shared].topViewController;
    }
    return self;
}
...
+(instancetype) initWithTextInput:(id<UITextInput>)textInput {
    QCNumberKeyboardView *v = [[QCNumberKeyboardView alloc] init];
...
-(void) initOtherBtn {
    ...
    [self.doneBtn setBackgroundColor:[QCApp shared].config.themeColor];
...
-(UIButton*) getCommonNumBtn:(NSString*)title {
    ...
    [[btn titleLabel] setFont:[[QCApp shared].config appFontOfSizeSemibold:20.0f]];
...
- (void)layoutSubviews {
    ...
    for (NSInteger i=0; i<commKeyViews.count; i++) {
        ...
        [btn setTitleColor:[QCApp shared].config.defaultTextColor forState:UIControlStateNormal];
...
//            [self.doneBtn removeFromSuperview];
            ...
            self.doneBtn.lim_top = QCScreenHeight - (bottomSafe + eachLineKeySpace) - eachKeyHeight*3;
