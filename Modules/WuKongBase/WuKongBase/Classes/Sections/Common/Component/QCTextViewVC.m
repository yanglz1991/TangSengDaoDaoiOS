...
@interface QCTextViewVC ()<UITableViewDataSource,UITableViewDelegate,UITextViewDelegate>
...
- (UITextView *)textView {
    if(!_textView) {
        _textView = [[UITextView alloc] initWithFrame:CGRectMake(0.0f, 0, QCScreenWidth, 250)];
        [_textView setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
        _textView.delegate = self;
        _textView.returnKeyType = UIReturnKeyDone;
        _textView.font = [QCApp.shared.config appFontOfSize:16.0f];
    }
    return _textView;
}
...
- (UILabel *)limitLbl {
    if(!_limitLbl) {
        ...
        _limitLbl.font = [QCApp.shared.config appFontOfSize:14.0f];
        _limitLbl.textColor = QCApp.shared.config.tipColor;
    }
    return _limitLbl;
}
...
- (void)viewConfigChange:(QCViewConfigChangeType)type {
    ...
    if(type == QCViewConfigChangeTypeStyle) {
        [self.textView setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
    }
