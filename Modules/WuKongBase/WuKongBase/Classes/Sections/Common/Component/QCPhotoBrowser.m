...
@implementation QCPhotoBrowser

static QCPhotoBrowser *_instance;
...
//                    NSData *imgData = UIImagePNGRepresentation(img);
                    ...
                    if(format == JLImageFormatGIF) {
                        [UIImage jl_compressWithImageGIF:imgData targetSize:img.size targetByte:[QCApp shared].config.imageMaxLimitBytes handler:^(NSData * _Nullable compressedData, CGSize gifImageSize, NSError * _Nullable error) {
                            selectRetain--;
                            if(compressedData) {
                                [imageDatas addObject:compressedData];
                            } else { // 压缩失败 只能将原图发出去
                                QCLogWarn(@"压缩失败，将原图发出去！");
                        ...
                        selectRetain--;
                        NSData *imgCompressData  = [UIImage jl_compressImageSize:img toByte:[QCApp shared].config.imageMaxLimitBytes];
                        if(imgCompressData) {
...
+(void) fetchAssetFilePathWithAsset:(PHAsset*)asset completion:(void(^)(NSString* filePath))completion {
    if(asset.mediaType == PHAssetMediaTypeVideo) {
        [ZLPhotoManager fetchAVAssetForVideo:asset completion:^(AVAsset * avasset, NSDictionary * infoDict) {
            ...
            NSString *outputPath = [QCFileLocationHelper filepathForTempDir:@"video_temp" filename:outputFileName];
            ...
            session.videoComposition = [QCPhotoBrowser getVideoComposition:avasset];  //修正某些播放器不识别视频Rotation的问题
...
+ (AVMutableVideoComposition *)getVideoComposition:(AVAsset *)asset
{
    ...
    BOOL isPortrait_ = [QCPhotoBrowser isVideoPortrait:asset];
    if(isPortrait_) {
