...
#define selectedColor QCApp.shared.config.defaultTextColor // 被选中后的颜色
...
@property(nonatomic,strong) NSArray<QCTabbarItem *> *items;
...
- (id)initWithItems:(NSArray<QCTabbarItem *> *)items width:(CGFloat)width{
    ...
    if (self) {
        ...
        NSInteger index= 0;
        for (QCTabbarItem *item in items) {
...
-(UIButton*) titleView:(NSString*)title index:(NSInteger)index {
    CGSize size = [self calculateWidthWithText:title font:[QCApp.shared.config defaultFont]];
    ...
    titleLbl.text = title;
    titleLbl.font = QCApp.shared.config.defaultFont;
    titleLbl.textColor = defaultColor;
    [titleLbl sizeToFit];
...
-(void) itemClick:(UIButton*)btn {
    ...
    QCTabbarItem *item =  self.items[self.selectedIndex];
    if(item.onClick) {
        item.onClick();
    }
}
...
-(UIView *) indicatorView {
    if(!_indicatorView) {
        ...
        _indicatorView.backgroundColor = QCApp.shared.config.defaultTextColor;
