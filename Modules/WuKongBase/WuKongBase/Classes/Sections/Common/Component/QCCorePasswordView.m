...
#import "QCApp.h"
@interface QCCorePasswordTF : UITextField

@end

@implementation QCCorePasswordTF
...
@interface QCCorePasswordBtn : UIButton

@end

@implementation QCCorePasswordBtn
...
@interface QCCorePasswordView () <UITextFieldDelegate>

@property(nonatomic, strong) QCCorePasswordTF *tf;

@property(nonatomic, strong) NSMutableArray *btns;

@end

@implementation QCCorePasswordView
...
- (QCCorePasswordTF *)tf {
  ...
  if (_tf == nil) {
    ...
    _tf = [[QCCorePasswordTF alloc] init];
...
- (void)setPasswordLength:(NSInteger)passwordLength {
  ...
  if (_passwordLength == 0) { //默认
      ...
      QCCorePasswordBtn *btn = [[QCCorePasswordBtn alloc] init];
      [self addSubview:btn];
    }
    ...
    if (passwordLength > _passwordLength) { //新的更多
      ...
      [self.subviews
          enumerateObjectsUsingBlock:^(__kindof UIView *_Nonnull obj,
            ...
            if ([obj isKindOfClass:[QCCorePasswordBtn class]]) {
