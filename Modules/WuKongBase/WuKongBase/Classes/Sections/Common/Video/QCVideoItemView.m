...
@interface QCVideoItemView () <QCVideoActionBarDelegate,AVAssetResourceLoaderDelegate>
@property (nonatomic, strong) QCVideoTopBar *topBar;
@property (nonatomic, strong) QCVideoActionBar *actionBar;
@property (nonatomic, strong) UIButton *playButton;
@end

@implementation QCVideoItemView {
    AVPlayer *_player;
...
- (void)updateLayoutWithExpectOrientation:(UIDeviceOrientation)orientation containerSize:(CGSize)containerSize {
    ...
    self.topBar.frame = CGRectMake(padding.left, padding.top, width, [QCVideoTopBar defaultHeight]);
    self.actionBar.frame = CGRectMake(padding.left, height - [QCVideoTopBar defaultHeight] - padding.bottom - 10, width, [QCVideoActionBar defaultHeight]);
...
- (void)yb_videoActionBar:(QCVideoActionBar *)actionBar clickPlayButton:(UIButton *)playButton {
    [self startPlay];
}

- (void)yb_videoActionBar:(QCVideoActionBar *)actionBar clickPauseButton:(UIButton *)pauseButton {
    [self playerPause];
}

- (void)yb_videoActionBar:(QCVideoActionBar *)actionBar changeValue:(float)value {
...
- (QCVideoTopBar *)topBar {
    if (!_topBar) {
        _topBar = [QCVideoTopBar new];
...
- (QCVideoActionBar *)actionBar {
    if (!_actionBar) {
        _actionBar = [QCVideoActionBar new];
