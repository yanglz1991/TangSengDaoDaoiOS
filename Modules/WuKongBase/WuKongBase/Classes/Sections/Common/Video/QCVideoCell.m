...
@interface QCVideoCell () <QCVideoDataDelegate, QCVideoItemViewDelegate, UIGestureRecognizerDelegate>
@end

@implementation QCVideoCell {
...
- (void)prepareForReuse {
    ((QCVideoData *)self.yb_cellData).delegate = nil;
...
- (void)updateImageLayoutWithOrientation:(UIDeviceOrientation)orientation previousImageSize:(CGSize)previousImageSize {
    QCVideoData *data = self.yb_cellData;
...
- (void)setYb_cellData:(id<YBIBDataProtocol>)yb_cellData {
    ...
    QCVideoData *data = (QCVideoData *)yb_cellData;
    data.delegate = self;
    ...
    if([QCFileUtil fileIsExistOfPath:storePath]) {
    ...
    if(data.downloadTask.status == QCTaskStatusSuccess) {
    ...
    [data.downloadTask addListener:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if(weakData.downloadTask.status == QCTaskStatusProgressing) {
                [weakSelf.yb_auxiliaryViewHandler() yb_showLoadingWithContainer:weakSelf progress:weakData.downloadTask.progress];
            }else if(weakData.downloadTask.status == QCTaskStatusSuccess) {
                [self hideToolViews:NO];
                [weakSelf.yb_auxiliaryViewHandler() yb_hideLoadingWithContainer:weakSelf];
                if([weakData.downloadTask isKindOfClass:[QCDowloadTask class]]) {
                    QCDowloadTask *realTask = (QCDowloadTask*)weakData.downloadTask;
                    weakSelf.videoView.asset = [[AVURLAsset alloc] initWithURL:[NSURL fileURLWithPath:realTask.storePath] options:nil];
                }else if([weakData.downloadTask isKindOfClass:[QCMessageFileDownloadTask class]]) {
                    QCMessageFileDownloadTask *realTask = (QCMessageFileDownloadTask*)weakData.downloadTask;
                    if(realTask.message && realTask.message.content) {
                        QCMediaMessageContent *mediaContent = (QCMediaMessageContent*)realTask.message.content;
            ...
            }else if(weakData.downloadTask.status == QCTaskStatusError) {
...
-(NSString*) getTaskStorePath:(QCBaseTask*)task {
    if([task isKindOfClass:[QCDowloadTask class]]) {
        QCDowloadTask *realTask = (QCDowloadTask*)task;
        return realTask.storePath;
    }else if([task isKindOfClass:[QCMessageFileDownloadTask class]]) {
        QCMessageFileDownloadTask *realTask = (QCMessageFileDownloadTask*)task;
        if(realTask.message && realTask.message.content) {
            QCMediaMessageContent *mediaContent = (QCMediaMessageContent*)realTask.message.content;
...
- (void)yb_pageChanged {
    QCVideoData *data = (QCVideoData *)self.yb_cellData;
...
- (void)respondsToPanGesture:(UIPanGestureRecognizer *)pan {
    ...
    YBIBInteractionProfile *profile = ((QCVideoData *)self.yb_cellData).interactionProfile;
...
- (QCVideoItemView *)videoView {
    if (!_videoView) {
        _videoView = [QCVideoItemView new];
