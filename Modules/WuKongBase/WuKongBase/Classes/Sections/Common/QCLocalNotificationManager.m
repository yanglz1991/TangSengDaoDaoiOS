...
#import "WuKongBase.h"
@implementation QCLocalNotificationManager

static QCLocalNotificationManager *_instance = nil;
...
-(void) showLocalNotificationIfNeed:(QCMessage*)message {
    if(message.contentType == WK_CMD || !message.header.showUnread || ![QCMySettingManager shared].newMsgNotice) {
        return;
    }
...
-(void) showLocalNotification:(QCMessage*)message {
    QCChannelInfo *channelInfo = message.channelInfo;
    ...
    NSString *alert;
    NSString *content;
     QCChannelInfo *from = [message from];
    ...
    switch (message.contentType) {
        case WK_TEXT:
            alert = ((QCTextContent*)message.content).content;
            break;
        case WK_IMAGE:
            alert = LLang(@"[图片]");
            break;
