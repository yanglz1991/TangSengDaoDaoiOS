...
#import "QCApp.h"
@implementation QCLabelModel

- (Class)cell {
    return QCLabelCell.class;
}

- (UIFont *)font {
    if(!_font) {
        _font = [[QCApp shared].config appFontOfSize:14.0f];
    }
    return _font;
}
...
- (UIColor *)textColor {
    if(!_textColor) {
        _textColor = [QCApp shared].config.tipColor;
    }
    return _textColor;
}
...
@interface QCLabelCell ()

@property(nonatomic,strong) UILabel *lbl;

@property(nonatomic,strong) QCLabelModel *model;


@end

@implementation QCLabelCell

+ (CGSize)sizeForModel:(QCLabelModel *)model {
    CGFloat width = QCScreenWidth-  model.left.floatValue*2;
...
- (void)refresh:(QCLabelModel *)model {
    [super refresh:model];
    self.model = model;
