...
@interface QCContactsFriendRequestCell ()
@property(nonatomic,strong) QCUserAvatar *avatarImgView; // 图标
...
@property(nonatomic,strong) QCFriendRequestDBModel *model;

@end

@implementation QCContactsFriendRequestCell
...
-(void) setupUI {
    ...
    [self.okBtn setBackgroundColor:[QCApp shared].config.themeColor];
    [self.okBtn.titleLabel setFont:[[QCApp shared].config appFontOfSize:15.0f]];
...
-(void)refresh:(QCFriendRequestDBModel*)model {
    ...
    if(model.status == QCFriendRequestStatusWaitSure) {
        ...
        [self.okBtn setBackgroundColor:[[QCApp shared].config.themeColor colorWithAlphaComponent:0.5f]];
    }
}
...
+ (NSString *)cellId {
    return @"QCContactsFriendRequestCell";
}
@end
