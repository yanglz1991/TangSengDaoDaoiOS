...
@interface QCContactsHeaderItemCell ()

@property(nonatomic,strong) QCImageView *icon; // 图标

@property(nonatomic,strong) UILabel *titleLbl; // 标题

@property(nonatomic,strong) QCBadgeView *badgeView; // 红点

@property(nonatomic,strong) UIImageView *avatarImgView; // 头像
@property(nonatomic,strong) QCBadgeView *avatarReddotView;  // 头像红点

@end

@implementation QCContactsHeaderItemCell

-(void) setupUI {
    self.icon = [[QCImageView alloc] initWithFrame:CGRectMake(0, 0, 50.0f, 50.0f)];
    ...
    self.avatarReddotView = [QCBadgeView viewWithoutBadgeTip];
...
-(void)refresh:(QCContactsHeaderItem*)model {
    ...
    if( model.icon && ([model.icon hasPrefix:@"http"] || [ model.icon hasPrefix:@"https"])) {
        ...
        self.icon.image = [[QCApp shared] loadImage:model.icon moduleID:model.moduleID];
    }
...
+ (NSString *)cellId {
    return @"QCContactsHeaderItemCell";
}
