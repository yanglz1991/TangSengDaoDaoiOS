...
#import <QCCore/LBXPermissionSetting.h>
#import "QCContactsFriendDB.h"

@interface QCContactsFriendVC ()<QCContactsFriendCellDelegate,MFMessageComposeViewControllerDelegate>

@property(nonatomic,strong) QCContactsFriendVM *vm;

@property(nonatomic,strong) NSMutableArray<QCContactsFriendModel*> *contactList;

@end

@implementation QCContactsFriendVC

- (void)viewDidLoad {
    
    self.mode = QCContactsModeSingle;
    self.vm = [QCContactsFriendVM new];
    ...
    [self.tableView registerClass:QCContactsFriendCell.class forCellReuseIdentifier:[QCContactsFriendCell cellId]];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [contactStore enumerateContactsWithFetchRequest:request error:nil usingBlock:^(CNContact * _Nonnull contact, BOOL * _Nonnull stop) {
            ...
            for (CNLabeledValue *labeledValue in contact.phoneNumbers)
            {
                ...
                QCContactsFriendModel *contactModel = [QCContactsFriendModel new];
...
-(AnyPromise*) uploadContacts {
    NSMutableArray<QCContactsFriendModel*> *needUploads = [NSMutableArray array];
   NSArray<QCContactsFriendDBModel*> *dbModels = [[QCContactsFriendDB shared] queryAll];
    
        
        for (QCContactsFriendModel *friendModel in self.contactList) {
            bool exist = false;
            if(dbModels && dbModels.count>0) {
                for (QCContactsFriendDBModel *dbModel in dbModels) {
    ...
    return [self.vm requestUpload:needUploads].then(^{
        NSMutableArray<QCContactsFriendDBModel*> *dbModels = [NSMutableArray array];
        for (QCContactsFriendModel *friendModel in needUploads) {
            QCContactsFriendDBModel *dbModel = [QCContactsFriendDBModel new];
            dbModel.name = friendModel.name;
            dbModel.phone = friendModel.phone;
            [dbModels addObject:dbModel];
        }
        [[QCContactsFriendDB shared] save:dbModels];
    }).catch(^(NSError *error){
...
-(void) getContacts {
    ...
    [self uploadContacts].then(^{
         [weakSelf.vm requestMaillist].then(^(NSArray<QCContactsFriendResp*> *items){
             [weakSelf.view hideHud];
             if(items && items.count>0) {
                 for (QCContactsFriendResp *friendResp in items) {
                     for (QCContactsFriendModel *friendModel in self.contactList) {
...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    QCContactsFriendCell *cell = (QCContactsFriendCell*)[super tableView:tableView cellForRowAtIndexPath:indexPath];
    cell.delegate = self;
    return cell;
}
...
- (NSMutableArray<QCContactsFriendModel *> *)contactList {
    if(!_contactList) {
...
-(void) invite:(QCContactsFriendModel*)model {
    if( [MFMessageComposeViewController canSendText]) {
        MFMessageComposeViewController * controller = [[MFMessageComposeViewController alloc] init];
        controller.recipients = @[model.phone];//发送短信的号码，数组形式入参
        controller.navigationBar.tintColor = [UIColor redColor];
        controller.messageComposeDelegate = self;
        controller.body = [QCApp shared].config.inviteMsg; //此处的body就是短信将要发生的内容
        [self presentViewController:controller animated:YES completion:nil];
    }
}

-(void) addFriend:(QCContactsFriendModel*)model {
    // 全局群聊禁言开启时禁止添加好友
