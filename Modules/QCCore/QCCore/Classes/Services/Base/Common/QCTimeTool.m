...
+(NSString*) formatTimeByAutoAMPM:(NSDate*)dt {
    ...
    if([self is12HourFormat]) {
        NSString *lang = [QCApp shared].config.langue;
...
+(NSString*) formatDateStyle1:(NSDate*)dt {
    ...
    if(currentYear==srcYear) {
        if(currentMonth==srcMonth && currentDay == srcDay) {
            ...
            long currentTimestamp = [QCTimeTool getIOSTimeStamp_l:currentDate];
            long srcTimestamp = [QCTimeTool getIOSTimeStamp_l:dt];
            ...
            // 否则直接显示完整日期时间
                ...
                ret = [NSString stringWithFormat:@"%@", [QCTimeTool getTimeString:dt format:@"yyyy-MM-dd"]];
            }
                
        }
    }else {
        ret = [QCTimeTool getTimeString:dt format:@"yyyy-MM-dd"];
    }
...
+ (NSString *)getTimeStringAutoShort2:(NSDate *)dt mustIncludeTime:(BOOL)includeTime containToday:(BOOL)today{
    ...
    // 要额外显示的时间分钟
    NSString *timeExtraStr = (includeTime?[QCTimeTool getTimeString:dt format:@" HH:mm"]:@"");
    ...
    if(currentYear == srcYear) {
        ...
        if(currentMonth == srcMonth && currentDay == srcDay) {
            ...
            // 否则当天其它时间段的，直接显示“时:分”的形式
                ...
                ret = [QCTimeTool getTimeString:dt format:[NSString stringWithFormat:@"%@ HH:mm",LLang(@"今天")]];
        }
        // 当年 && 当天之外的时间（即昨天及以前的时间）
            ...
            // “前天”判断逻辑同上
                ...
                // 否则直接显示完整日期时间
                    ...
                    ret = [NSString stringWithFormat:@"%@%@", [QCTimeTool getTimeString:dt format:@"yyyy/M/d"], timeExtraStr];
            }
        }
    }
    // 往年
...
+ (NSString *)getTimeString:(NSDate *)dt format:(NSString *)fmt{
    ...
    return [format stringFromDate:(dt==nil?[QCTimeTool getIOSDefaultDate]:dt)];
}
...
+ (long) getIOSTimeStamp_l:(NSDate *)dat{
    return [[NSNumber numberWithDouble:[QCTimeTool getIOSTimeStamp:dat]] longValue];
}
