...
@interface QCSchemaRequest ()

@property(nonatomic,strong) NSURLComponents *urlComponents;

@end

@implementation QCSchemaRequest

+(QCSchemaRequest*) url:(NSURL*)url {
    QCSchemaRequest *request = [QCSchemaRequest new];
    request.url = url;
...
-(BOOL) isAppSchema {
    return [self.url.scheme hasPrefix:[QCApp shared].config.appSchemaPrefix];
}
...
@implementation QCSchemaManager

static QCSchemaManager *_instance = nil;
...
-(void) setup {
    ...
    // 申请加好友
    [self registerHandler:@"ui.friend.apply" handler:^BOOL(QCSchemaRequest * _Nonnull request) {
    ...
    // 用户协议
    [self registerHandler:@"ui.app.userprotocol" handler:^BOOL(QCSchemaRequest * _Nonnull request) {
        if(![request isAppSchema]) {
            return false;
        }
        NSString *host = request.url.host;
...
-(void) toApplyFriend:(NSString*)toUID {
    ...
    [[QCNavigationManager shared].topViewController presentViewController:alertController animated:true completion:nil];
}
...
-(void) registerHandler:(NSString*)sid handler:(QCSchemaHandler)handler {
    [[QCApp shared] setMethod:sid handler:^id _Nullable(id  _Nonnull param) {
...
-(void) handle:(QCSchemaRequest*)request {
    NSArray<QCEndpoint*> *endpoints = [[QCApp shared] getEndpointsWithCategory:[self category]];
    if(endpoints) {
...
-(void) handleURL:(NSURL*)url {
    [self handle:[QCSchemaRequest url:url]];
}
