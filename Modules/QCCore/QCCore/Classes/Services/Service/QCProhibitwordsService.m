...
@interface QCProhibitwordsService()
...
@implementation QCProhibitwordsService


static QCProhibitwordsService *_instance = nil;
...
- (void)sync:(void (^)(NSError *))callback {
    ...
    [QCAPIClient.sharedClient GET:@"message/prohibit_words/sync" parameters:@{@"version":@(lastVersion)}].then(^(NSArray<NSDictionary*> *results){
...
-(void) load {
    ...
    if(prohibitwordsJsonStr && ![prohibitwordsJsonStr isEqualToString:@""]) {
        NSArray *items = [QCJsonUtil toArray:prohibitwordsJsonStr];
...
-(void) save {
    NSString *filePath = [self savePath];
    NSString *jsonStr = [QCJsonUtil toJson:self.prohibitwords];
