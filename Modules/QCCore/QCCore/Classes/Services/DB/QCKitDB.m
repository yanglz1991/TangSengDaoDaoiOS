...
@property(nonatomic,strong) QCKitDBConfig *conf;

@end
@implementation QCKitDB

static QCKitDB *_instance = nil;
...
- (QCKitDBConfig *)config {
    if(!_conf) {
        _conf = [[QCKitDBConfig alloc] init];
    }
    return _conf;
}
...
-(void) switchDB:(NSString*)uid {
    ...
    [[QCDBMigration shared] resetManager];
    // im skd
     [[QCDB sharedDB] switchDB:uid];
    ...
    if(!isExist) {
        ...
        if (![fileManager createDirectoryAtPath:self.config.dbDir withIntermediateDirectories:YES attributes:nil error:&error]) {
            QCLogError(@"creat Directory Failed:%@",[error localizedDescription]);
        }
    }
    NSString *dbPath = [NSString stringWithFormat:@"%@/%@%@.db",self.config.dbDir,QCKIT_DB_Prefix,uid];
    QCLogDebug(@"数据库路径 -> %@",dbPath);
...
-(void) migrateDatabase{
    ...
    [[QCSwiftModuleManager shared] didDatabaseLoad];
}
...
- (NSBundle*) resourceBundle{
    NSURL *url = [self module_bundleUrl:[NSString stringWithFormat:@"%@_%@",@"QCCore",@"resources"]  cls:[self class]];
    if(url) {
        return [NSBundle bundleWithURL:url];
    }
    return [NSBundle bundleWithURL:[self module_bundleUrl:[NSString stringWithFormat:@"%@_%@",@"QCCore",@"images"]  cls:[self class]]];
}
