...
@implementation QCSystemMessageCell
...
+ (CGSize)sizeForMessage:(QCMessageModel *)model {
     QCSystemContent *content = (QCSystemContent*)model.content;
     NSString *text = content.displayContent;
    CGSize contentSize = CGSizeMake(0.0f, 0.0f);
    if(text) {
        contentSize =  [[self class] getTextSize:text maxWidth:QCScreenWidth - WK_SYSTEM_TEXT_SPACE];
    }
...
- (void)refresh:(QCMessageModel *)model {
    ...
    [self.tipTextBoxView setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
}
...
- (QCTipLabel *)tipTextLbl {
    if(!_tipTextLbl) {
        _tipTextLbl = [[QCTipLabel alloc] init];
        [_tipTextLbl setTextAlignment:NSTextAlignmentCenter];
        [_tipTextLbl setFont:[UIFont systemFontOfSize:[QCApp shared].config.messageTipTimeFontSize]];
...
- (UIButton *)inviteWaitSureBtn {
    if(!_inviteWaitSureBtn) {
        ...
        [[_inviteWaitSureBtn titleLabel] setFont:[UIFont systemFontOfSize:[QCApp shared].config.messageTipTimeFontSize]];
        [_inviteWaitSureBtn setTitleColor:[QCApp shared].config.themeColor forState:UIControlStateNormal];
...
-(void) onInvite {
    QCSystemContent *systemContent = (QCSystemContent*)[self.messageModel content];
      if(!systemContent.content  || !systemContent.content[@"invite_no"]) {
          [[[QCNavigationManager shared] topViewController].view showMsg:LLang(@"数据错误！")];
          return;
      }
    [[QCAPIClient sharedClient] GET:[NSString stringWithFormat:@"groups/%@/member/h5confirm?invite_no=%@",self.messageModel.channel.channelId,systemContent.content[@"invite_no"]] parameters:nil].then(^(NSDictionary *resultDic){
