...
// 上传任务
@property(nonatomic,strong) QCMessageFileUploadTask *uploadTask;
...
@implementation QCImageMessageCell

+ (CGSize)contentSizeForMessage:(QCMessageModel *)model {
    QCImageContent *imageContent = (QCImageContent*)model.content;
...
- (void)prepareForReuse {
    ...
    if(self.messageModel) {
        QCImageContent *imageContent = (QCImageContent*)self.messageModel.content;
...
-(void) initUI {
    ...
    self.imgView = [[QCImageView alloc] init];
    ...
    self.progressView = [[QCLoadProgressView alloc] initWithFrame:CGRectMake(18, 0, 44, 44)];
...
- (void)refresh:(QCMessageModel *)model {
    ...
    QCImageContent *imageContent = (QCImageContent*)model.content;
    CGSize imageSize = [QCImageMessageCell contentSizeForMessage:model];
...
-(void) updateProgress {
    ...
    // 上传进度控制
    self.uploadTask = [[QCSDK shared] getMessageFileUploadTask:self.messageModel.message];
    if(self.uploadTask) {
        [self.uploadTask addListener:^{
            if(weakSelf.uploadTask.status == QCTaskStatusProgressing) {
...
-(void) onTap {
    ...
    imageBrowser.onEditFinish = ^(UIImage *img) {
        QCImageContent *content = [QCImageContent initWithImage:img];
    ...
    if(self.messageModel.content.flame) {
        ...
        if(!weakSelf.messageModel.viewed) {
            [QCSDK.shared.flameManager didViewed:@[weakSelf.messageModel.message]];
        }
    ...
    if(dates) {
        ...
        for (NSInteger i=dates.count-1; i>=0; i--) {
            ...
            NSArray<QCMessageModel*> *messages = [self.conversationContext messagesAtDate:date];
            if(messages && messages.count>0) {
                for (NSInteger j=messages.count-1; j>=0; j--) {
                    ...
                    QCImageContent *imageContent = (QCImageContent*)messageModel.content;
                    NSData *orgData = imageContent.originalImageData;
                    if(orgData) {
                        data.image = ^UIImage * _Nullable{
                            return [YYImage imageWithData:orgData];
                    ...
                    if(cell && [cell isKindOfClass:[QCImageMessageCell class]]) {
                        UIImage *image = ((QCImageMessageCell*)cell).imgView.image;
                        if(image) {
                        ...
                        data.projectiveView = ((QCImageMessageCell*)cell).imgView;
                    }
                    if(!data.image) {
                        data.imageURL = [[QCApp shared] getImageFullUrl:imageContent.remoteUrl];
                    }
