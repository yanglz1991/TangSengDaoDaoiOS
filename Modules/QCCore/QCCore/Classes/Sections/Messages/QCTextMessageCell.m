...
@interface QCTextMessageCell ()<CNContactViewControllerDelegate,CNContactPickerDelegate>
...
// ---------- 安全提醒 ----------
@property(nonatomic,strong) QCTipLabel *securityTipLbl;

@end


@implementation QCTextMessageCell
...
+ (CGSize)sizeForMessage:(QCMessageModel *)model {
   CGSize size = [super sizeForMessage:model];
    ...
    if(model.hasSensitiveWord && !model.isSend) {
        ...
        CGSize tipSize = [[self class] getTextSize:[QCSecurityTipManager shared].tip maxWidth:[QCApp shared].config.messageContentMaxWidth fontSize:securityTipFontSize];
...
+ (CGSize)contentSizeForMessage:(QCMessageModel *)model {
    ...
    CGSize trailingSize = [QCTrailingView size:model];

    CGFloat lastlineWidth = [[self class] textLastlineWidth:attrStr messageModel:model];

    CGFloat lastLineWithTrailingWidth = lastlineWidth + trailingSize.width + QCTrailingLeft;
    if(lastLineWithTrailingWidth>[QCApp shared].config.messageContentMaxWidth) {
        size.height += QCTimeHeight;
    }else{
...
//    self.textLbl.delegate = self;
    ...
    [self.textLbl setFont:[[QCApp shared].config appFontOfSize:[QCApp shared].config.messageTextFontSize]];
...
+(NSMutableAttributedString*) parseAndCacheTextMessage:(QCMessageModel*)message {
    
    
    if(message.streamOn && message.streamFlag!=QCStreamFlagEnd) { // 流式消息不缓存
    ...
    if([textContent.format isEqualToString:@"html"]) {
        key = [NSString stringWithFormat:@"%@-%lu",key,(unsigned long)QCApp.shared.config.style]; // 如果是html需要加上主题
    }
...
+(NSMutableAttributedString*) getContentAttrStr:(QCMessageModel*)message {
    QCTextContent *textContent =  (QCTextContent*)[message content];
    if(message.remoteExtra.contentEdit) {
        textContent = (QCTextContent*)message.remoteExtra.contentEdit;
    }
    ...
    if(message.streams && message.streams.count>0) {
        for (QCStream *stream in message.streams) {
            if([stream.content isKindOfClass:QCTextContent.class]) {
                QCTextContent *textContent = (QCTextContent*)stream.content;
                [content appendString:textContent.content];
            }
        }
    }
    
    NSArray<id<QCMatchToken>> *tokens = [self getTokens:message text:content];
    
    NSMutableAttributedString *attrStr = [[NSMutableAttributedString alloc] init];
    attrStr.font = [[QCApp shared].config appFontOfSize:[QCApp shared].config.messageTextFontSize];
...
+(NSMutableAttributedString*) parseText:(QCTextContent*)content isSend:(BOOL)isSend parseBefore:(void(^)(NSMutableAttributedString *attr))parseBeforeBlock{
    ...
    if(content.content) {
        if(content.format && [content.format isEqualToString:@"html"]) {
            UIColor *textColor;
            if(isSend) {
                textColor =  [QCApp shared].config.messageSendTextColor;
            }else {
                textColor = [QCApp shared].config.messageRecvTextColor;
            }
            NSString *temp = [NSString stringWithFormat:@"<style>body{font-size:%0.0fpx;color:%@}</style>%@",[QCApp shared].config.messageTextFontSize,[textColor toHexRGB],content.content];
...
+(NSArray<id<QCMatchToken>>*) getTokens:(QCMessageModel*)message text:(NSString*)text{
    NSMutableArray<id<QCMatchToken>> *tokens = [NSMutableArray array];
    @try {
        
        NSArray<QCMessageEntity*> *entities = message.content.entities;
        if(message.remoteExtra.contentEdit) {
        ...
        if(entities && entities.count>0) {
            ...
            for (QCMessageEntity *messageEntiy in entities) {
                if(!messageEntiy.type) {
                    continue;
                }
                if(messageEntiy.type && [messageEntiy.type isEqualToString:QCMentionRichTextStyle]) {
                ...
                }else if([messageEntiy.type isEqualToString:QCLinkRichTextStyle]) {
        ...
        QCLogDebug(@"解析文本消息的 token失败！->%@ %@",text,exception);
    } @finally {
...
+(CGSize) textSize:(NSMutableAttributedString*)attrStr messageModel:(QCMessageModel*)model{
    
    if(model.streamOn && model.streamFlag!=QCStreamFlagEnd) { // 流式消息不缓存
        return [attrStr size:[QCApp shared].config.messageContentMaxWidth];
    }
    ...
    CGSize size = [attrStr size:[QCApp shared].config.messageContentMaxWidth];
    [memoryCache setCache:NSStringFromCGSize(size) forKey:key];
    return size;
}

+(CGFloat) textLastlineWidth:(NSMutableAttributedString*)attrStr messageModel:(QCMessageModel*)model{
    
    if(model.streamOn && model.streamFlag!=QCStreamFlagEnd) { // 流式消息不缓存
        return [attrStr lastlineWidth:[QCApp shared].config.messageContentMaxWidth];
    }
    ...
    CGFloat lastLineWidthF = [attrStr lastlineWidth:[QCApp shared].config.messageContentMaxWidth];
    [memoryCache setCache:@(lastLineWidthF) forKey:key];
    return lastLineWidthF;
}

+(BOOL) hasReply:(QCMessageModel*)messageModel {
...
//    NSString *text = textContent.content;
    ...
    if(model.isSend) {
        attrStr.textColor =  [QCApp shared].config.messageSendTextColor;
        attrStr.linkColor = [UIColor whiteColor];
    }else {
        attrStr.textColor = [QCApp shared].config.messageRecvTextColor;
    ...
    if([[self class] hasReply:model]) {
        ...
        self.replyAvatarIcon.url = [QCAvatarUtil getAvatar:model.content.reply.fromUID];
    ...
    if([self.messageModel isSend]) {
        self.replyContentLbl.textColor =[QCApp shared].config.messageTipColor;
        self.replyNameLbl.textColor = [QCApp shared].config.messageTipColor;
    }else{
        self.replyContentLbl.textColor =[QCApp shared].config.tipColor;
        self.replyNameLbl.textColor = [QCApp shared].config.tipColor;
    }
...
-(void) onTapWithGestureRecognizer:(TapLongTapOrDoubleTapGestureRecognizerWrap*)gesture {
   ...
   id<QCMatchToken> token = [self.textLbl matchDidTapAttributedTextInLabelWithPoint:point];
    if(token) {
        if(token.type == WKatchTokenTypeMetion) {
            ...
            QCLinkToken *linToken = (QCLinkToken*)token;
...
-(void) layoutName {
    QCBubblePostion position = [[self class] bubblePosition:self.messageModel];
    if(!self.nameLbl.hidden) {
        if(position == QCBubblePostionLast || position == QCBubblePostionSingle) {
            self.nameLbl.lim_left =  WK_CONTENT_INSETS.left+QCLastBubbleOffsetSpace;
        }else{
...
//+ (UIEdgeInsets)bubbleEdgeInsets:(QCMessageModel *)model contentSize:(CGSize)contentSize {
//    QCBubblePostion position = [self bubblePosition:model];
//    if(position == QCBubblePostionLast) { // 最后一条消息
...
- (QCUserAvatar *)replyAvatarIcon {
    if(!_replyAvatarIcon) {
        _replyAvatarIcon = [[QCUserAvatar alloc] initWithFrame:CGRectMake(0.0f, 0.0f, replyAvatarSize, replyAvatarSize)];
    }
...
- (UILabel *)replyNameLbl {
    if(!_replyNameLbl) {
        _replyNameLbl = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 0.0f, [QCApp shared].config.messageContentMaxWidth - 20*2, 0.0f)];
        _replyNameLbl.font = [[QCApp shared].config appFontOfSize:replyNameFontSize];
    }
...
- (UILabel *)replyContentLbl {
    if(!_replyContentLbl) {
        _replyContentLbl = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 0.0f, [QCApp shared].config.messageContentMaxWidth - 20*2, 0.0f)];
        _replyContentLbl.font = [[QCApp shared].config appFontOfSize:replyContentFontSize];
        _replyContentLbl.numberOfLines = 1;
        [_replyContentLbl setTextColor:[QCApp shared].config.messageTipColor];
    }
    return _replyContentLbl;
}

- (QCTipLabel *)securityTipLbl {
    if(!_securityTipLbl) {
        _securityTipLbl = [[QCTipLabel alloc] init];
        _securityTipLbl.text = [QCSecurityTipManager shared].tip;
        _securityTipLbl.lim_width = [QCApp shared].config.messageContentMaxWidth;
        _securityTipLbl.font = [[QCApp shared].config appFontOfSize:securityTipFontSize];
        ...
        _securityTipLbl.textColor = [QCApp shared].config.defaultTextColor;
...
+(CGSize) getReplyNameSize:(QCMessageModel *)message {
    return [self getTextSize:message.content.reply.fromName?:@"" maxWidth:[QCApp shared].config.messageContentMaxWidth - 20*2 fontSize:replyNameFontSize];
}

+(CGSize) getReplyContentSize:(QCMessageModel *)message {
    return [self getTextSize:[message.content.reply.content conversationDigest] maxWidth:[QCApp shared].config.messageContentMaxWidth - 20*2 fontSize:replyContentFontSize];
}
...
-(void) didMetionClick:(QCMetionToken*)token {
    NSString *atUID = token.uid;
    if(!atUID || [atUID isEqualToString:@""]) {
        return;
    }
    QCChannelMember *member = [[QCSDK shared].channelManager getMember:self.messageModel.channel uid:atUID];
    NSString *vercode = @"";
    if(member) {
        vercode = member.extra[QCChannelExtraKeyVercode];
    }
...
//    NSString *link = token.text;
    if([link containsString:@"."]) { // 网站
        ...
        QCActionSheetView2 *sheetView = [QCActionSheetView2 initWithTip:[NSString stringWithFormat:LLang(@"%@可能是一个电话号码，你可以"),link]];
        [sheetView addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"呼叫") onClick:^{
        ...
        [sheetView addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"复制号码") onClick:^{
            UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
            [pasteboard setString:link];
        }]];
        [sheetView addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"添加到手机通讯录") onClick:^{
...
-(void) toSaveContacts:(NSString*)phone {
    __weak typeof(self) weakSelf = self;
    QCActionSheetView2 *sheetView = [QCActionSheetView2 initWithTip:[NSString stringWithFormat:LLang(@"%@可能是一个电话号码，你可以"),phone]];
    [sheetView addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"创建新联系人") onClick:^{
        [weakSelf saveNewContact:phone];
    }]];
    [sheetView addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"添加到现有联系人") onClick:^{
...
-(void) saveNewContact:(NSString*)phone {
    if (@available(iOS 9.0, *)) {
        ...
        [[QCNavigationManager shared].topViewController presentViewController:navigation animated:YES completion:nil];
    }
}
...
-(void) saveExistContact:(NSString*)phone {
    if (@available(iOS 9.0, *)) {
        ...
        controller.delegate = self;
           [[QCNavigationManager shared].topViewController presentViewController:controller
             animated:YES
           completion:^{
...
- (void)contactPicker:(CNContactPickerViewController *)picker
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        UINavigationController *navigation =
                                  ...
                                  [[QCNavigationManager shared].topViewController presentViewController:navigation
                                                        animated:YES
                                                      completion:^{
