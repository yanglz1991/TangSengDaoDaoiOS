...
#import "QCMeVC.h"
#import "QCMeInfoVC.h"
@interface QCMeVC ()<QCChannelManagerDelegate>
@property(nonatomic,strong) WKeHeader *meHeader;
@end

@implementation QCMeVC

- (instancetype)init
{
    self = [super init];
    if (self) {
        self.viewModel = [QCMeVM new];
    }
    return self;
}
...
-(WKeHeader*) meHeader {
    if (!_meHeader) {
        _meHeader = [[WKeHeader alloc] initWithFrame:CGRectMake(0, 0, QCScreenWidth, 240.0f)];
...
- (void)channelInfoUpdate:(QCChannelInfo *)channelInfo {
    ...
    [[SDImageCache sharedImageCache] removeImageForKey:QCApp.shared.loginInfo.uid withCompletion:nil];
...
-(void) reloadData {
    ...
    self.nameLbl.text = [QCApp shared].loginInfo.extra[@"name"];
...
-(QCUserAvatar*) avatarImgView {
    if (!_avatarImgView) {
        ...
        _avatarImgView = [[QCUserAvatar alloc] initWithFrame:CGRectMake(0.0f, 0.0f, size, size)];
...
- (UIView *)avatarBox {
    if(!_avatarBox) {
        ...
        [_avatarBox setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
    }
    return _avatarBox;
}
...
-(UILabel*) nameLbl {
    if(!_nameLbl) {
        ...
        [_nameLbl setFont:[[QCApp shared].config appFontOfSizeSemibold:20.0f]];
    }
    return _nameLbl;
}
