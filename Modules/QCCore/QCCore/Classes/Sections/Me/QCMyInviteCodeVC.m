...
@implementation QCMyInviteCodeVC
...
-(void) loadInviteCode {
    ...
    // 获取邀请码
    [QCAPIClient.sharedClient GET:@"invite" parameters:nil].then(^(NSDictionary *resultDict){
...
- (UIView *)boxView {
    if(!_boxView) {
        ...
        _boxView.backgroundColor = QCApp.shared.config.cellBackgroundColor;
    }
    return _boxView;
}
...
- (QCUserAvatar *)avatar {
    if(!_avatar) {
        ...
        _avatar.url =  [QCAvatarUtil getAvatar:QCApp.shared.loginInfo.uid];
    }
    return _avatar;
}
...
- (UILabel *)nicknameLbl {
    if(!_nicknameLbl) {
        ...
        _nicknameLbl.text = QCApp.shared.loginInfo.extra[@"name"]?:@"";
...
- (UILabel *)inviteCodeLbl {
    if(!_inviteCodeLbl) {
        ...
        _inviteCodeLbl.text = @"-------";
        _inviteCodeLbl.font = [QCApp.shared.config appFontOfSizeSemibold:40.0f];
...
- (UIButton *)cpBtn {
    if(!_cpBtn) {
        ...
        [_cpBtn setTitleColor:QCApp.shared.config.themeColor forState:UIControlStateNormal];
        [_cpBtn sizeToFit];
...
- (UIButton *)disableBtn {
    if(!_disableBtn) {
        ...
        [_disableBtn setTitleColor:QCApp.shared.config.themeColor forState:UIControlStateNormal];
...
-(void) disablePressed {
    __weak typeof(self) weakSelf = self;
    [self.view showHUD];
    [QCAPIClient.sharedClient PUT:@"invite/status" parameters:nil].then(^{
