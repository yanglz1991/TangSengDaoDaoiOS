...
@interface QCScreenPasswordVC ()

@property(nonatomic,strong) QCUserAvatar *avatarImgView;
@property(nonatomic,strong) UILabel *tipLbl;
@property(nonatomic,strong) QCCorePasswordView *passwordView;
...
@implementation QCScreenPasswordVC

- (instancetype)init
{
    self = [super init];
    if (self) {
        self.viewModel = [QCScreenPasswordVM new];
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    NSNumber *tryCount = [QCApp shared].loginInfo.extra[@"lock_screen_try"];
    if(tryCount) {
...
- (QCUserAvatar *)avatarImgView {
    if(!_avatarImgView) {
        _avatarImgView = [[QCUserAvatar alloc] initWithFrame:CGRectMake(0.0f, self.navigationBar.lim_bottom+10.0f, 100.0f, 100.0f)];
...
- (QCCorePasswordView *)passwordView {
    if(!_passwordView) {
        __weak typeof(self) weakSelf = self;
        NSString *lockScreenPwd = [QCApp shared].loginInfo.extra[@"lock_screen_pwd"];
        _passwordView = [[QCCorePasswordView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.view.lim_width - 60.0f, 40.0f)];
        __weak typeof(_passwordView) weakPasswordView = _passwordView;
        [_passwordView setPasswordCompeleteBlock:^(NSString *password) {
            if([[QCScreenPasswordSetVM digestLockScreenPwd:password] isEqualToString:lockScreenPwd]) {
                ...
                [QCApp shared].loginInfo.extra[@"lock_screen_try"] = @(weakSelf.tryCount);
...
- (UIButton *)forgetPwdBtn {
    if(!_forgetPwdBtn) {
        ...
        [[_forgetPwdBtn titleLabel] setFont:[[QCApp shared].config appFontOfSize:14.0f]];
...
-(void) closeScreenPassword {
    [self.viewModel requestCloseLock].then(^{
        [QCApp shared].loginInfo.extra[@"lock_screen_try"] = @(0);
...
- (UILabel *)warnLbl {
    if(!_warnLbl) {
        ...
        _warnLbl.font = [[QCApp shared].config appFontOfSize:16.0f];
