...
- (void)viewDidLoad {
    ...
    [[QCAPIClient sharedClient] GET:@"user/qrcode" parameters:nil].then(^(NSDictionary *result){
    ...
    [self updateRemark: [NSString stringWithFormat:LLang(@"扫一扫上面的二维码图案，加我%@"),[QCApp shared].config.appName]];
}
...
-(UILabel*) titleLbl {
    if(!_titleLbl) {
        ...
        _titleLbl.text = [QCApp shared].loginInfo.extra[@"name"];
