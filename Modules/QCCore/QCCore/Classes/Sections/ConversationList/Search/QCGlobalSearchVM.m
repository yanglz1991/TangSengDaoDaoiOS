...
#define QCSearchMaxCount 4

@interface QCGlobalSearchVM ()
...
@implementation QCGlobalSearchVM
...
- (NSArray<QCFormSection *> *)tableSections {
    ...
    return  [QCTableSectionUtil toSections:items];
}
...
-(void) search:(void(^)(NSError * _Nullable))complete {
    ...
    // 频道内查找聊天记录（all/media/file 三个 tab）改为基于本地 SDK QCMessageDB 数据库查询，
    // 避免依赖未启用的 QCIM 服务端搜索插件 /v1/search/global。
...
-(NSMutableArray<NSDictionary*>*) handleSearchResult:(NSDictionary * )result {
    ...
    if(friends&&friends.count>0) {
        ...
        for (NSInteger i=0; i<friends.count; i++) {
            ...
            [friendItems addObject:@{
                      @"class":QCSearchContactsModel.class,
                      @"name":name,
                      @"avatar":[QCAvatarUtil getAvatar:uid],
                      @"keyword": @"",
                      @"showBottomLine":@(NO),
                      @"showTopLine":@(NO),
                      @"onClick":^{
                        [[QCApp shared] invoke:QCPOINT_USER_INFO param:@{@"uid":uid}];
                      }
                   }];
        }
        [items addObject:@{
    ...
    if(groups && groups.count>0) {
        ...
        for (NSInteger i=0; i<groups.count; i++) {
            ...
            [groupsItems addObject:@{
               @"class":QCSearchContactsModel.class,
               @"name":name?:@"",
               @"avatar":[QCAvatarUtil getGroupAvatar:groupNo],
               @"keyword": @"",
               @"showBottomLine":@(NO),
               @"showTopLine":@(NO),
               @"onClick":^{
                [[QCApp shared] pushConversation:[QCChannel groupWithChannelID:groupNo]];
            }
            }];
        }
    ...
    if(messages && messages.count>0 && ![self.tabType isEqualToString:@"media"]) {
        ...
        for (NSInteger i=0; i<messages.count; i++) {
            ...
            QCMessageContent *messageContent = [QCSDK.shared.chatManager getMessageContent:contentType.intValue];
            if(payload) {
                NSString *payloadStr = [QCJsonUtil toJson:payload];
            ...
            if([self.tabType isEqualToString:@"file"]) {
                ...
                NSDictionary *itemDict = [QCApp.shared invoke:QCPOINT_SEARCH_ITEM_FILE param:param];
                if(itemDict) {
            ...
            QCChannel *channel = [QCChannel channelID:channelId channelType:channelType.integerValue];
            
            QCChannel *requestChannel = channel;
            if([self searchInChannel]) {
                requestChannel = [QCChannel personWithChannelID:fromUid];
            }
            
            if([QCSDK.shared isSystemMessage:contentType.integerValue]) {
                content = LLang(@"[系统消息]");
    ...
    if(messages && messages.count>0 && [self.tabType isEqualToString:@"media"]) {
        ...
        for (NSMutableArray<NSDictionary*> *rows in messageGroups) {
            ...
            NSMutableArray<QCSearchMediaItem*> *items = [NSMutableArray array];
            for (NSDictionary *message in rows) {
                ...
                NSURL *url = [[QCApp shared] getImageFullUrl:thumbPath];

                QCSearchMediaItem *item = [[QCSearchMediaItem alloc] init];
                ...
                // 改为点击图片后跳转到聊天窗口并定位到该消息（替代直接弹 QCImageBrowser），
                // 规避 YBImageBrowser/QCImageBrowser 内部路径带来的闪退风险，体验也与其他 tab 一致。
                ...
                item.onClick = ^{
                    ...
                    QCConversationVC *vc = [[QCConversationVC alloc] init];
                    vc.channel = [QCChannel channelID:mediaChannelId channelType:mediaChannelType.integerValue];
                    vc.locationAtOrderSeq = [QCSDK.shared.chatManager getOrderSeq:mediaMessageSeq.unsignedLongLongValue];
                    [[QCNavigationManager shared] pushViewController:vc animated:YES];
                };
            ...
            [mediaItems addObject:@{
                @"class":QCSearchMediaModel.class,
                @"items": items,
                @"numOfRow": @(numOfRow),
            }];
        }
...
// MARK: - 频道内本地搜索（基于 QCMessageDB 本地数据库）
...
- (NSSet<NSNumber *> *)targetContentTypesForLocalSearch {
    ...
    if ([self.tabType isEqualToString:@"media"]) {
        // 始终包含图片和小视频；小视频的点击跳转逻辑在本地处理，不依赖 SmallVideo 模块注册 QCPOINT_SEARCH_ITEM_VIDEO。
...
- (NSDictionary *)convertWKMessageToDict:(QCMessage *)msg {
    ...
    return @{
        ...
        // 额外补充字段，供下游（如小视频点击播放）凭这些唯一标识反查本地 QCMessage
...
- (void)localSearchInChannel:(void (^)(NSError * _Nullable))complete {
    ...
    while (result.count < (NSUInteger)pageLimit && safety++ < 30) {
        ...
        NSArray<QCMessage *> *batch = [[QCMessageDB shared] getMessages:self.channel
                                                          startOrderSeq:cursor
                                                            endOrderSeq:0
                                                                  limit:fetchBatch
                                                               pullMode:QCPullModeDown];
        if (!batch || batch.count == 0) {
            reachedEnd = YES;
            break;
        }
        for (QCMessage *msg in batch) {
            if (msg == nil) continue;
...
-(void) requestSearch:(NSDictionary*)param callback:(void (^)(NSError * _Nullable error,NSDictionary * _Nullable))callback{
    ...
    [QCAPIClient.sharedClient POST:@"search/global" parameters:request].then(^(NSDictionary*result){
        if(callback) {
            callback(nil,result);
        }
    }).catch(^(NSError *error){
