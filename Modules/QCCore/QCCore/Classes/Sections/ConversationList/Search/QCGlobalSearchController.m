...
@interface QCGlobalSearchController ()<UISearchControllerDelegate>

@property(nonatomic,strong) QCGlobalSearchResultController *searchResultController;
@end

@implementation QCGlobalSearchController

-(instancetype) initWithSearchResultsController:(QCGlobalSearchResultController *)searchResultsController{
...
-(void) defaultStyle {
    self.searchBar.frame = CGRectMake(10.0f, 0.0f, QCScreenWidth-20.0f, 44.0f);
    [self.searchBar setBackgroundImage:[UIImage new]];
    self.searchBar.backgroundColor =[QCApp shared].config.backgroundColor;// 设置搜索框背景颜色
    self.searchBar.barTintColor=[QCApp shared].config.backgroundColor;
...
+(instancetype) searchController{
    QCGlobalSearchController *controller = [[QCGlobalSearchController alloc] initWithSearchResultsController:[QCGlobalSearchResultController new]];
    return controller;
}
