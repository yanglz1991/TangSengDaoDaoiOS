...
@implementation QCPCOnlineVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    
    self.view.backgroundColor = [QCApp shared].config.backgroundColor;
...
- (void)setMute:(BOOL)mute {
    ...
    if(mute) {
        ...
        self.muteBoxView.backgroundColor = [QCApp shared].config.themeColor;
        ...
        self.muteBoxView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
...
-(void) layout {
    ...
    self.pcIconImgView.lim_top = [[QCApp shared].config visibleEdgeInsets].top + 120.0f;
    ...
    self.quitPCBtn.lim_top = self.view.lim_height - [[QCApp shared].config visibleEdgeInsets].bottom - 60.0f;
...
- (UILabel *)titleLbl {
    if(!_titleLbl) {
        ...
        _titleLbl.font = [[QCApp shared].config appFontOfSize:24.0f];
        _titleLbl.text = [NSString stringWithFormat:LLang(@"网页%@已登录"),[QCApp shared].config.appName];
        [_titleLbl sizeToFit];
    }
    return _titleLbl;
}
...
- (UILabel *)subtitleLbl {
    if(!_subtitleLbl) {
        ...
        _subtitleLbl.font = [[QCApp shared].config appFontOfSize:16.0f];
...
- (UIButton *)quitPCBtn {
    if(!_quitPCBtn) {
        ...
        [_quitPCBtn setTitle:[NSString stringWithFormat:LLang(@"退出网页%@"),[QCApp shared].config.appName] forState:UIControlStateNormal];
        [_quitPCBtn sizeToFit];
        [_quitPCBtn setTitleColor:[QCApp shared].config.themeColor forState:UIControlStateNormal];
        _quitPCBtn.layer.masksToBounds = YES;
        _quitPCBtn.layer.cornerRadius = 8.0f;
        [_quitPCBtn.titleLabel setFont:[[QCApp shared].config appFontOfSize:16.0f]];
        _quitPCBtn.backgroundColor = [QCApp shared].config.cellBackgroundColor;
...
-(void) quitPressed {
    QCActionSheetView2 *sheet = [QCActionSheetView2 initWithTip:[NSString stringWithFormat:LLang(@"退出网页%@？"),[QCApp shared].config.appName]];
    
    __weak typeof(self) weakSelf = self;
    [sheet addItem:[QCActionSheetButtonItem2 initWithAlertTitle:LLang(@"退出") onClick:^{
        [weakSelf.view showHUD];
        [[QCAPIClient sharedClient] POST:@"user/pc/quit" parameters:nil].then(^{
...
- (UIView *)muteBoxView {
    if(!_muteBoxView) {
        ...
        _muteBoxView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
...
-(void) mutePressed {
    self.mute = !self.mute;
    QCOnlineStatusManager.shared.muteOfApp = self.mute;
    [QCMySettingManager.shared muteOfApp:self.mute];
}
...
- (UILabel *)muteLbl {
    if(!_muteLbl) {
        ...
        _muteLbl.font = [[QCApp shared].config appFontOfSize:14.0f];
        _muteLbl.textColor = [QCApp shared].config.tipColor;
        [_muteLbl sizeToFit];
    }
    return _muteLbl;
}
...
- (UIView *)fileBoxView {
    if(!_fileBoxView) {
        ...
        _fileBoxView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
...
-(void) filePressed {
    ...
    vc.channel = QCFileHelperChannel;
...
- (UILabel *)fileLbl {
    if(!_fileLbl) {
        ...
        _fileLbl.font = [[QCApp shared].config appFontOfSize:14.0f];
        _fileLbl.textColor = [QCApp shared].config.tipColor;
        [_fileLbl sizeToFit];
    }
    return _fileLbl;
}
