...
@implementation QCFriendRequestDB

static QCFriendRequestDB *_instance = nil;
...
-(BOOL) addFriendRequest:(QCFriendRequestDBModel*)model {
    __weak typeof(self) weakSelf = self;
    __block BOOL isNewRequest = true; // 是否是新的请求
    [[QCKitDB shared].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        QCFriendRequestDBModel *existFriendRequest = [self getFriendRequest:model.uid db:db];
        if(existFriendRequest) {
            [weakSelf deleteFriendRequest:model.uid db:db];
            if(existFriendRequest.status == QCFriendRequestStatusWaitSure) {
                isNewRequest = false;
            }else {
                isNewRequest = true;
            }
...
-(QCFriendRequestDBModel*) getFriendRequest:(NSString*)uid db:(FMDatabase*) db {
    FMResultSet *resultSet = [db executeQuery:WK_GET_FRIENDREQUEST_SQL,uid];
    if(!resultSet.next) {
        [resultSet close];
        return nil;
    }
    QCFriendRequestDBModel *model = [self to:resultSet];
    [resultSet close];
    return model;
}
...
-(QCFriendRequestDBModel*) to:(FMResultSet*)resultSet {
    QCFriendRequestDBModel *model = [QCFriendRequestDBModel new];
...
@implementation QCFriendRequestDBModel
