...
@implementation QCVideoRecordUtil

+(void) videoRecord:(void(^)(NSString *coverPath,NSString *videoPath))callback imgCallback:(void(^)(UIImage*img))imgCallback{
    
    UIViewController *vc = [[QCNavigationManager shared] topViewController];
    
    [[QCPhotoBrowser shared] takePhoto:vc doneBlock:^(UIImage * _Nonnull img, NSURL * _Nonnull videoURL) {
        if(img) {
            if(imgCallback) {
                imgCallback(img);
            }
            ...
            if(callback) {
                ...
                UIImage *coverImg =  [QCVideoRecordUtil getVideoPreViewImage:asset];
