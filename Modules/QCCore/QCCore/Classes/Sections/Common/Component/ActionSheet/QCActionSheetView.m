...
- (instancetype)initWithCancleTitle:(NSString *)cancleTitle otherTitleArray:(NSArray *)otherTitleArray{
    self = [super init];
       if (self) {
           ...
           //首先添加取消按钮
           QCActionSheetItem *cancleItem = [QCActionSheetItem itemWithTitle:LLang(@"取消") index:0];
           [_items addObject:cancleItem];
           
           tag ++;
           
           for (NSString *otherTitle in otherTitleArray) {
               QCActionSheetItem *item = [QCActionSheetItem itemWithTitle:otherTitle index:tag];
...
- (instancetype)initWithCancleTitle:(NSString *)cancleTitle otherTitles:(NSString *)otherTitles, ...
{
    ...
    if (self) {
        ...
        if(otherTitles)
        {
            QCActionSheetItem *item = [QCActionSheetItem itemWithTitle:otherTitles index:tag];
            [_items addObject:item];
            tag ++;
            
            va_start(list, otherTitles);
            while ((curStr = va_arg(list, NSString*))) {
                QCActionSheetItem *item = [QCActionSheetItem itemWithTitle:curStr index:tag];
                [_items addObject:item];
                tag ++;
            }
            va_end(list);
...
- (void)setupBtnWithTitles:(NSString*)msgTitle {
    ...
    for (QCActionSheetItem *item in _items) {
        UIButton *btn = nil;
...
- (void)sheetBtnClick:(UIButton *)btn{
    QCActionSheetItem *item = _items[btn.tag];
