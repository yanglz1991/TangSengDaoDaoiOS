 ...
 */
@interface QCCheckBox ()

@property (strong, nonatomic, nullable) QCCheckBoxGroup *group;

- (void)_setOn:(BOOL)on animated:(BOOL)animated notifyGroup:(BOOL)notifyGroup;

@end

@implementation QCCheckBoxGroup
...
+ (nonnull instancetype)groupWithCheckBoxes:(nullable NSArray<QCCheckBox *> *)checkBoxes {
    QCCheckBoxGroup *group = [[QCCheckBoxGroup alloc] init];
    for (QCCheckBox *checkbox in checkBoxes) {
...
- (void)addCheckBoxToGroup:(nonnull QCCheckBox *)checkBox {
    if (checkBox.group) {
...
- (void)removeCheckBoxFromGroup:(nonnull QCCheckBox *)checkBox {
...
- (QCCheckBox *)selectedCheckBox {
    QCCheckBox *selected = nil;
    for (QCCheckBox *checkBox in self.checkBoxes) {
        if(checkBox.on){
            selected = checkBox;
            break;
        }
...
- (void)setSelectedCheckBox:(QCCheckBox *)selectedCheckBox {
    if (selectedCheckBox) {
        for (QCCheckBox *checkBox in self.checkBoxes) {
...
#pragma mark Private methods called by QCCheckBox

- (void)_checkBoxSelectionChanged:(QCCheckBox *)checkBox {
    if ([checkBox on]) {
