...
#import "QCApp.h"
@implementation QCSMSCodeItemModel

- (Class)cell {
    return QCSMSCodeItemCell.class;
}
...
@property(nonatomic,strong) QCSMSCodeItemModel *model;

@end

@implementation QCSMSCodeItemCell
...
- (void)refresh:(QCSMSCodeItemModel *)model {
    [super refresh:model];
    self.model = model;
    [self.sendBtn setTitle:model.sendBtnTitle forState:UIControlStateNormal];
    if(model.disable) {
        self.sendBtn.enabled = NO;
        [self.sendBtn setTitleColor:[QCApp shared].config.tipColor forState:UIControlStateNormal];
    }else{
        self.sendBtn.enabled = YES;
        [self.sendBtn setTitleColor:[QCApp shared].config.themeColor forState:UIControlStateNormal];
    }
}
...
- (UIButton *)sendBtn {
    if(!_sendBtn) {
        ...
        [_sendBtn setTitleColor:[QCApp shared].config.themeColor forState:UIControlStateNormal];
...
- (UIView *)splitView {
    if(!_splitView) {
        ...
        _splitView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
    }
    return _splitView;
}
