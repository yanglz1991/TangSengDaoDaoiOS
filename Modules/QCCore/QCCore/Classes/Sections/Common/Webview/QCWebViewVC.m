...
@interface QCWebViewVC ()<WKUIDelegate,WKNavigationDelegate,UIScrollViewDelegate>
...
@property(nonatomic,strong) QCWebViewService *webViewService;

@end

@implementation QCWebViewVC
...
- (void)viewDidLoad {
    ...
    [request setValue:[QCApp shared].config.langue forHTTPHeaderField:@"Accept-Language"];
...
- (QCWebViewService *)webViewService {
    if(!_webViewService) {
        _webViewService = [[QCWebViewService alloc] init];
    }
...
- (UIButton *)moreBtn {
    if(!_moreBtn) {
        ...
        [_moreBtn setTintColor:QCApp.shared.config.navBarButtonColor];
    }
    return _moreBtn;
}
...
- (UIView *)bottomView {
    if(!_bottomView) {
        ...
        _bottomView.backgroundColor = QCApp.shared.config.navBackgroudColor;
...
- (UIButton *)gobackBtn {
    if(!_gobackBtn) {
        ...
        [gobackBtn setTintColor:QCApp.shared.config.navBarButtonColor];
...
- (UIButton *)goBtn {
    if(!_goBtn) {
        ...
        [goBtn setTintColor:QCApp.shared.config.navBarButtonColor];
...
-(void) morePressed {
    ...
    [sheetView addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"转发") onClick:^{
        QCTextContent *textContent = [[QCTextContent alloc] initWithContent:weakSelf.currentUrl.absoluteString];
        [[QCMessageActionManager shared] forwardContent:textContent complete:nil];
    }]];
    [sheetView addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"复制") onClick:^{
    ...
    [sheetView addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"在浏览器中打开") onClick:^{
...
//        preferences.minimumFontSize = 40.0;
        ...
        [_webView addObserver:self forKeyPath:@"URL" options:NSKeyValueObservingOptionNew context:nil];
           /***/
        self.bridge = [QCWebViewJavascriptBridge bridgeForWebView:_webView];
...
-(void) checkGoAndGobackBtn {
    if([self.webView canGoBack]) {
        ...
        [self.gobackBtn setTintColor:QCApp.shared.config.navBarButtonColor];
    }else {
    ...
    if([self.webView canGoForward]) {
        ...
        [self.goBtn setTintColor:QCApp.shared.config.navBarButtonColor];
    }else{
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler{
    ...
    NSMutableURLRequest *newRequest = [QCWebViewCookieMgr newRequest:navigationAction.request];
...
//    NSString *js = [QCWebViewCookieMgr clientCookieScripts];
//    if (!js) return;
//    WKUserScript *jsscript = [[WKUserScript alloc]initWithSource:js
//                                                   injectionTime:QCUserScriptInjectionTimeAtDocumentStart
//                                                forMainFrameOnly:NO];
...
-(void) showBottomView {
    [UIView animateWithDuration:QCApp.shared.config.defaultAnimationDuration animations:^{
        if(self.scrollIsUp) {
            self.bottomView.lim_top = self.view.lim_height - self.bottomView.lim_height;
        }else{
            self.bottomView.lim_top = self.view.lim_height;
