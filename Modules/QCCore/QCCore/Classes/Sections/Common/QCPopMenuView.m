...
@interface QCPopMenuView ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
...
+ (void)showWithItems:(NSArray <NSDictionary *>*)array
     ...
     triangleLocation:(CGPoint)point
               action:(void(^)(NSInteger index))action
{
    QCPopMenuView *view = [[QCPopMenuView alloc] initWithItems:array width:width triangleLocation:point action:action];
    [view show];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    // 设置填充色
    [[QCApp shared].config.cellBackgroundColor setFill];
    // 设置边框颜色
    [[QCApp shared].config.cellBackgroundColor setStroke];
    // 绘制路径
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _titleLabel.font = [[QCApp shared].config appFontOfSize:16.0f];
...
- (void)layoutSubviews {
    ...
    _titleLabel.textColor = [QCApp shared].config.defaultTextColor;
