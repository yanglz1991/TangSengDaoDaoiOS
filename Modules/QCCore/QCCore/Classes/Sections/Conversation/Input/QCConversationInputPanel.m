...
#define  WKiPhoneX (QCScreenWidth == 375.f && QCScreenHeight == 812.f ? YES : NO)

#define QCConversationInputHeight 36.0f // 输入框高度
#define QCConversationFuncGroupViewHeight 50.0f // 输入框下面的功能组的视图高度

@interface QCConversationInputPanel()<QCGrowingTextViewDelegate>{
...
// 工具栏中间视图
@property(nonatomic,strong) QCFuncGroupView *funcGroupView;
// 消息工具栏
...
@property(nonatomic,strong) NSArray<id<QCInputChangeTextRespondProto>> *inputChangeTextResponds; // 输入框文本改变响应链
...
@property(nonatomic,strong) QCSendButton *sendButton;
...
@implementation QCConversationInputPanel

-(QCConversationInputPanel*) initWithConversationContext:(id<QCConversationContext>)context {
    self = [super init];
    if(!self) return nil;
...
-(void) setupUI {
    ...
    self.currentInputHeight = QCConversationInputHeight;
    
     //获取输入框改变响应链
    _inputChangeTextResponds = [[QCApp shared] invokes:QCPOINT_CATEGORY_CONVERSATION_INPUT_TEXT_RESPOND param:nil];
    if(_inputChangeTextResponds) {
        [_inputChangeTextResponds enumerateObjectsUsingBlock:^(id<QCInputChangeTextRespondProto>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            obj.conversationContext = self.conversationContext;
        }];
    }
    
    
    _contentViewMinHeight = QCConversationInputHeight + QCConversationFuncGroupViewHeight +10.0f;
...
-(void) updateAndLayoutTextViewRightView {
    self.textViewRights = [[QCApp shared] invokes:QCPOINT_CATEGORY_TEXTVIEW_RIGHTVIEW param:@{@"context":self.conversationContext}];
...
-(void) resetCurrentInputHeight {
   ...
   CGFloat mHeight = [self.textView measureHeight];
    ...
    self.currentInputHeight = MAX(QCConversationInputHeight, currentHeight);
}
//
...
#pragma mark -- 面板相关
-(QCConversationPanel*) conversationPanel {
    if(!_conversationPanel) {
        _conversationPanel = [[QCConversationPanel alloc] init];
    }
...
-(void) layoutSubviews{
    ...
    [self.messageToolBar setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
    [self setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
    [self.conversationPanel setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
...
//    UIEdgeInsets inputFieldInsets = [self inputFieldInsets];
    ...
    if(self.funcGroupView.startScroll) {
        self.funcGroupView.lim_top = self.textView.lim_bottom - (self.funcGroupView.lim_height - QCConversationFuncGroupViewHeight);
    }else {
        self.funcGroupView.lim_top = self.textView.lim_bottom;
    }
...
//    self.funcGroupView.lim_height = QCConversationFuncGroupViewHeight;
//    self.funcGroupView.lim_top = self.textView.lim_bottom;
...
-(QCFuncGroupView*) funcGroupView {
    if(!_funcGroupView) {
        CGFloat scaleZoom = 1.8f;
        _funcGroupView = [[QCFuncGroupView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, QCScreenWidth, QCConversationFuncGroupViewHeight) inputPanel:self];
...
- (QCMenusBtn *)menusBtn {
    if(!_menusBtn) {
        _menusBtn = [[QCMenusBtn alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 30.0f, 25.0f)];
//        _menusBtn.backgroundColor = [QCApp shared].config.themeColor;
...
- (QCSendButton *)sendButton {
    if(!_sendButton) {
        CGSize size = CGSizeMake(32.0f, 32.0f);
        _sendButton = [[QCSendButton alloc] initWithFrame:CGRectMake(self.messageToolBar.lim_width, 0.0f, size.width, size.height)];
        __weak typeof(self) weakSelf = self;
...
-(QCGrowingTextView*) textView {
    if(!_textView) {
        _textView = [[QCGrowingTextView alloc] init];
        _textView.lim_height =QCConversationInputHeight;
...
-(void) reloadInputPanelFrame {
    self.lim_size = CGSizeMake(QCScreenWidth, [self currentContentHeight]+[self currentPanelHeight]);
...
//    CGFloat y =  QCScreenHeight -navHeight - statusRect.size.height -[self currentInputPanelHeight];
     CGFloat y =  QCScreenHeight -[self currentInputPanelHeight];
...
-(CGFloat) currentContentHeight{
    ...
    CGFloat height = MAX(self.currentInputHeight + QCConversationFuncGroupViewHeight +10.0f,_contentViewMinHeight);
    ...
    if(hidden) {
        [self animateInputWithBlock:^{
            if(!weakSelf.disableAutoTop) {
                weakSelf.lim_top = QCScreenHeight;
            }
...
-(void) inputSendFinished {
    ...
    if([QCApp shared].config.messageTextMaxBytes !=0) {
        if(content && [self convertToByte:content]>[QCApp shared].config.messageTextMaxBytes) {
            [[QCNavigationManager shared].topViewController.view showHUDWithHide:LLang(@"发送的内容太长！")];
            return;
        }
    }
...
//    }else
    if ([self isMention:text]) { // @功能
        ...
        if([willDeleteStr isEqualToString:QCInputAtStartChar]) { /// @被删除了 说明@结束了
    ...
    if(_inputChangeTextResponds && _inputChangeTextResponds.count>0){
        ...
        for(id<QCInputChangeTextRespondProto> inputChangeTextRespond in _inputChangeTextResponds){
            id<QCInputChangeRespondResult> result =  [inputChangeTextRespond shouldChangeTextInRange:range replacementText:text];
...
-(void) handleTextViewContentDidChange {
    ...
    if([text hasSuffix:QCInputAtStartChar]) {
        [self triggerMentionStartIfNeed];
    }
    ...
    if(![text containsString:@"@"]) {
        ...
        if([text hasSuffix:QCInputAtEndChar]) {
            [self triggerMentionEndIfNeed];
        }
    }
    if(self.mentionStart) {
    ...
    [[QCApp shared] invokes:QCPOINT_CATEGORY_CONVERSATION_INPUT_TEXT_CHANGE param:@{@"input":self}];
...
-(BOOL) isMention:(NSString*)text {
    return [text isEqualToString:QCInputAtStartChar];
}

// 是否删除提及
- (NSRange)delRangeForMention {
    NSRange range = [self rangeForPrefix:QCInputAtStartChar suffix:QCInputAtEndChar];
    return range;
}
...
//    NSString *selectedText = range.length ? [text substringWithRange:range] : text;
    ...
    for (NSInteger i = endLocation; i >= endLocation - p && i-1 >= 0 ; i--) {
        NSRange subRange = NSMakeRange(i - 1, 1);
        NSString *subString = [text substringWithRange:subRange];
        if([subString compare:QCInputAtEndChar] == NSOrderedSame) {
            return NSMakeRange(NSNotFound, 0);
        }
        if ([subString compare:QCInputAtStartChar] == NSOrderedSame) {
            index = i - 1;
            break;
        }
    }
...
#pragma mark - QCGrowingTextViewDelegate


- (void)growingTextView:(QCGrowingTextView *)growingTextView willChangeHeight:(CGFloat)height duration:(NSTimeInterval)duration animationCurve:(int)animationCurve{
//    UIEdgeInsets inputFieldInsets = [self inputFieldInsets];
//    CGFloat inputContainerHeight = MAX(_messageToolBarMinHeight, height);
    if(height < QCConversationInputHeight) {
        height = QCConversationInputHeight;
    }
...
#pragma mark - QCInputProto

- (void)sendMessage:(QCMessageContent *)content {
