...
@interface QCUserHandleVC ()

@property(nonatomic,strong) QCUserHandleTableHeaderView *tableHeaderView;
@property(nonatomic,strong) QCUserHandleTableFooterView *tableFooterView;

@property(nonatomic,copy) void(^onScrollingAnimationEnd)(void);

@end

@implementation QCUserHandleVC

- (void)viewDidLoad {
    
    [self setTableView:[[QCTableHeaderBypassTableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain]];
...
-(void) show:(NSArray<QCFormItemModel *> *)items {
    self.view.hidden = NO;
...
- (QCUserHandleTableHeaderView *)tableHeaderView {
    if(!_tableHeaderView) {
        _tableHeaderView = [[QCUserHandleTableHeaderView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, QCScreenWidth, 100.0f)];
...
- (QCUserHandleTableFooterView *)tableFooterView {
    if(!_tableFooterView) {
        _tableFooterView = [[QCUserHandleTableFooterView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, QCScreenWidth, 7.0f)];
...
-(CGFloat) getContentHeight:(NSArray<QCFormItemModel*>*)items {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    QCFormItemCell *cell = (QCFormItemCell*)[tableView dequeueReusableCellWithIdentifier:reuseIdentifier forIndexPath:indexPath];
    QCFormItemModel *model = [self.items objectAtIndex:indexPath.row];
    [cell refresh:model];
    return cell;
}
...
@interface QCUserHandleTableHeaderView ()

@property(nonatomic,strong) QCTopShadowView *shadowView;
@property(nonatomic,strong) UIView *decorationView;

@end

@implementation QCUserHandleTableHeaderView
...
- (QCTopShadowView *)shadowView {
    if(!_shadowView) {
        _shadowView = [[QCTopShadowView alloc] init];
    }
    return _shadowView;
}
...
@interface QCUserHandleTableFooterView ()

@property(nonatomic,strong) UIView *bottomFillingBackgroundView;
@end

@implementation QCUserHandleTableFooterView
