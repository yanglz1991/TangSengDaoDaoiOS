...
@interface QCFuncGroupEditVC ()<UITableViewDelegate,UITableViewDataSource>
...
@property(nonatomic,strong) NSMutableArray<NSMutableArray<QCFuncGroupEditItemModel*>*> *items;

@property(nonatomic,strong) NSDictionary<NSString*,QCAPMSortInfo*> *apmSortDict;

@end

@implementation QCFuncGroupEditVC

- (void)viewDidLoad {
    [super viewDidLoad];

    self.navigationBar.lim_top = 0.0f;
    self.navigationBar.lim_height = [QCApp shared].config.navHeight - [UIApplication sharedApplication].statusBarFrame.size.height;
...
- (NSArray<QCFuncGroupEditItemModel*> *)funcItems {
    NSArray<id<QCPanelFuncItemProto>> *funcItems = [[QCApp shared] invokes:QCPOINT_CATEGORY_PANELFUNCITEM param:@{@"context":self.conversationContext}];
    ...
    [newFuncItems sortUsingComparator:^NSComparisonResult(QCFuncGroupEditItemModel  *obj1, QCFuncGroupEditItemModel *obj2) {
...
- (NSMutableArray<NSMutableArray<QCFuncGroupEditItemModel*> *> *)items {
    if(!_items) {
...
//        _tableView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        ...
        [_tableView registerClass:QCFuncGroupEditItemCell.class forCellReuseIdentifier:NSStringFromClass(QCFuncGroupEditItemCell.class)];
    }
    return _tableView;
}
...
- (UIButton *)editBtn {
    if(!_editBtn) {
        ...
        [_editBtn setTitleColor:[QCApp shared].config.navBarButtonColor forState:UIControlStateNormal];
        [_editBtn sizeToFit];
...
-(void) editPressed {
    ...
    if(isComplete) {
        NSMutableArray<QCAPMSortInfo*> *sortInfos = [NSMutableArray array];
        NSInteger k = 0;
        for (NSInteger i=0; i<self.items[favoriteSectionIndex].count; i++) {
            QCFuncGroupEditItemModel *model = self.items[favoriteSectionIndex][i];
            QCAPMSortInfo *sortInfo = [QCAPMSortInfo new];
            sortInfo.type = QCFuncGroupEditItemTypeFavorite;
        ...
        for (NSInteger i=0; i<self.items[moreSectionIndex].count; i++) {
            QCFuncGroupEditItemModel *model = self.items[moreSectionIndex][i];
            QCAPMSortInfo *sortInfo = [QCAPMSortInfo new];
            sortInfo.type = QCFuncGroupEditItemTypeMore;
            sortInfo.apmID = model.sid;
            sortInfo.disable = model.disable;
            sortInfo.sort = k+1;
            [sortInfos addObject:sortInfo];
        ...
        [QCAPMManager shared].apmSorts = sortInfos;
        [[QCAPMManager shared] saveAPMSorts];
    }
}
...
- (UIButton *)completeBtn {
    if(!_completeBtn) {
        ...
        [_completeBtn setTitleColor:[QCApp shared].config.navBarButtonColor forState:UIControlStateNormal];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    QCFuncGroupEditItemModel *item = self.items[indexPath.section][indexPath.row];
    QCFuncGroupEditItemCell *cell =  (QCFuncGroupEditItemCell*)[tableView dequeueReusableCellWithIdentifier:NSStringFromClass(QCFuncGroupEditItemCell.class) forIndexPath:indexPath];
    [cell refresh:item];
    if(indexPath.section == moreSectionIndex) {
...
- (BOOL)tableView:(UITableView *)tableView canMoveRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.section == favoriteSectionIndex) {
        ...
        QCFuncGroupEditItemModel *item = items[indexPath.row];
...
- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath {
    ...
    QCFuncGroupEditItemModel *destItem = self.items[destinationIndexPath.section][destinationIndexPath.row];
    ...
    QCFuncGroupEditItemModel *obj = [items objectAtIndex:sourceIndexPath.row];
    [items removeObjectAtIndex:sourceIndexPath.row];
    [items insertObject:obj atIndex:destinationIndexPath.row];
    if(destinationIndexPath.section == moreSectionIndex) {
        obj.type = QCFuncGroupEditItemTypeMore;
    }
    [tableView reloadData];
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.section == favoriteSectionIndex) {
        UITableViewRowAction *deleteAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleNormal title:LLang(@"从个人收藏中移除") handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
            ...
            QCFuncGroupEditItemModel *item = [firstItems objectAtIndex:indexPath.row];
            [firstItems removeObjectAtIndex:indexPath.row];
            [lastItems insertObject:item atIndex:0];
            item.type = QCFuncGroupEditItemTypeMore;
            [tableView reloadData];
        }];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if(editingStyle == UITableViewCellEditingStyleInsert) {
        NSMutableArray *firstItems = self.items[favoriteSectionIndex];
        NSMutableArray *lastItems = self.items[moreSectionIndex];
        QCFuncGroupEditItemModel *item = [lastItems objectAtIndex:indexPath.row];
        [lastItems removeObjectAtIndex:indexPath.row];
        [firstItems addObject:item];
        item.type = QCFuncGroupEditItemTypeFavorite;
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    titleLbl.text = title;
    titleLbl.font = [[QCApp shared].config appFontOfSizeSemibold:20.0f];
    [titleLbl sizeToFit];
    titleLbl.lim_left = 10.0f;
    titleLbl.textColor = [QCApp shared].config.tipColor;
    titleLbl.lim_centerY_parent = headerView;
    [headerView addSubview:titleLbl];
    [headerView setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
    return headerView;
}
