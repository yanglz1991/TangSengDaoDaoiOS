...
@implementation QCConversationView (Robot)


-(void) initRobot {
    __weak typeof(self) weakSelf = self;
    
    [[QCApp shared] setMethod:QCPOINT_ROBOT_INPUT_TEXT_CHANGE handler:^id _Nullable(id  _Nonnull param) {
        [weakSelf inputTextChange];
        return nil;
    } category:QCPOINT_CATEGORY_CONVERSATION_INPUT_TEXT_CHANGE];
...
-(void) requestAndShowRobotInlineQuery:(NSString*)query username:(NSString*)username offset:(NSString*)offset{
    ...
    [self requestInlineQuery:query username:username offset:offset].then(^(QCInlineQueryResult *result){
        [weakSelf showInlineQueryView:result query:query username:username];
    }).catch(^(NSError *error){
        QCLogError(@"提交robot查询数据失败！->%@",error);
    });
}

-(AnyPromise*) requestInlineQuery:(NSString*)query username:(NSString*)username offset:(NSString*)offset {
    return [[QCAPIClient sharedClient] POST:@"robot/inline_query" parameters:@{
        @"username":username,
        @"query": query,
        @"offset":offset?:@"",
        @"channel_id": self.channel.channelId,
        @"channel_type": @(self.channel.channelType),
    } model:QCInlineQueryResult.class];
}

-(void) showInlineQueryView:(QCInlineQueryResult*)result query:(NSString*)query username:(NSString*)username{
    if(!self.robotInlineOn) {
        return;
    }
    if(result.results && result.results.count>0) {
        __weak typeof(self) weakSelf = self;
        QCResultPanel *panel = [[QCInlineQueryManager shared] createResultPanel:result context:self.conversationContext];
        [panel setLoadMore:^(NSString *nextOffset,QCLoadMoreCallback callback) {
            [weakSelf requestInlineQuery:query username:username offset:nextOffset].then(^(QCInlineQueryResult *result){
                callback(result,nil);
            }).catch(^(NSError *error){
                callback(nil,error);
            });
...
-(void) triggerRobotInlineSearchIfNeed {
   ...
   NSString *text = self.input.textView.text;
    ...
    __weak typeof(self) weakSelf = self;
   QCRobot *robot = [[QCRobotManager shared] getRobotWithUsername:robotUsername];
    if(!robot) {
        [[QCRobotManager shared] syncWithUsernames:@[robotUsername] complete:^(BOOL hasData, NSError * _Nonnull error) {
            if(error) {
                QCLogError(@"同步机器人[%@]数据失败！->%@",robotUsername,error);
                return;
            }
            if(hasData) {
                QCRobot *robot = [[QCRobotManager shared] getRobotWithUsername:robotUsername];
...
-(void) syncRobot:(NSArray<NSString*>*) robotIDs {
    ...
    __weak typeof(self) weakSelf = self;
    [[QCSDK shared].robotManager sync:robotIDs complete:^(BOOL hasData,NSError * _Nonnull error) {
        if(error) {
            return;
        }
        if(hasData) {
...
-(void) initRobotMenus:(NSArray<NSString*>*) robotIDs {
    NSArray<QCRobot*> *robots = [[QCRobotDB shared] queryRobots:robotIDs];
    NSMutableArray *menus = [NSMutableArray array];
    for (QCRobot *robot in robots) {
...
- (QCRobotMenusListView *)newRobotMenusModalView {
    QCRobotMenusListView *robotMenusModalView = [QCRobotMenusListView initItems:[self getRobotMenus]];
    robotMenusModalView.targetView = self.input;
    return robotMenusModalView;
}

-(NSArray<QCRobotMenusItem*>*) getRobotMenus {
    
    NSMutableArray *items = [NSMutableArray array];
    
    __weak typeof(self) weakSelf = self;
    if(self.robotMenus) {
        for (QCRobotMenus *m in self.robotMenus) {
            [items addObject:[QCRobotMenusItem cmd:m.cmd iconURL:[QCAvatarUtil getAvatar:m.robotID] remark:m.remark onClick:^{
                [weakSelf.conversationContext sendTextMessage:m.cmd entities:@[[QCMessageEntity type:QCEntityTypeRobotCommand range:NSMakeRange(0, m.cmd.length)]] robotID:m.robotID];
