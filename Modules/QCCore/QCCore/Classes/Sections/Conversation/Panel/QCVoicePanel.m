...
@interface QCVoicePanel ()<CWTalkBackViewDelegate,CWAudioPlayViewDelegate,CWRecordViewDelegate,CWVoiceChangePlayViewDelegate>
@property(nonatomic,strong) CWVoiceView *voiceView;
@end

@implementation QCVoicePanel
...
-(void) layoutPanel:(CGFloat)height {
    ...
    if(!_voiceView) {
        _voiceView = [[CWVoiceView alloc] initWithFrame:CGRectMake(0, 0,QCScreenWidth, height)];
        ...
        [_voiceView setBackgroundColor:[QCApp shared].config.backgroundColor];
...
-(void) sendVoiceMessage:(NSData*)voiceData second:(NSInteger)second waveform:(NSArray<NSNumber*>*)waveform{
    if(second<=0) {
        [[QCNavigationManager shared].topViewController.view showHUDWithHide:LLang(@"说话时间太短")];
        return;
    }
    NSData *waveforms = [self cutAudioWaveform:waveform];
    [self.context sendMessage:[QCVoiceContent initWithData:voiceData second:(int)second waveform:waveforms]];
}
