...
#import <GZIP/GZIP.h>
@interface QCStickerBigViewModal ()
...
@implementation QCStickerBigViewModal

+(QCStickerBigViewModal*) focusedView:(UIView*)focusedView sticker:(QCSticker*)sticker{
    QCStickerBigViewModal *modal = [QCStickerBigViewModal new];
...
- (UIView *)view {
    if(!_view) {
        _view = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, QCScreenWidth, QCScreenHeight)];
...
-(void) downSticker:(void(^)(NSString *jsonPath))compeletion {
    NSString *storePath = [NSString stringWithFormat:@"%@/%@_%@",[QCSDK shared].options.messageFileRootDir,self.sticker.category,[self.sticker.path lastPathComponent]];
    
    [QCFileUtil createDirectoryIfNotExist:[storePath stringByDeletingLastPathComponent]];
    ...
    if([[NSFileManager defaultManager] fileExistsAtPath:jsonStorePath]) {
        ...
        NSURLSessionDownloadTask *task = [[QCAPIClient sharedClient] createDownloadTask:[[QCApp shared] getFileFullUrl:self.sticker.path].absoluteString storePath:storePath progress:^(NSProgress * _Nullable downloadProgress) {
            ...
            if(copyError) {
                QCLogError(@"复制sticker路径失败->%@",error);
                [[QCNavigationManager shared].topViewController.view showHUDWithHide:copyError.domain];
                return;
            }
            if(compeletion) {
                compeletion(jsonStorePath);
...
-(void) addSticker {
    ...
    if(weakSelf.animationBoxView.lim_right> QCScreenWidth) {
        weakSelf.animationBoxView.lim_left = QCScreenWidth - weakSelf.animationBoxView.lim_width;
    }
...
//    [self.animationView setAnimationFromJSON:[QCJsonUtil toDic:jsonStr]];
}
...
- (UIView *)animationBoxView {
    if(!_animationBoxView) {
        ...
        self.animationBoxView.backgroundColor = [QCApp shared].config.cellBackgroundColor;
