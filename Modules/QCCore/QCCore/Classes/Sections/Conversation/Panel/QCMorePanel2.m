...
@interface QCMorePanel2 ()<UIScrollViewDelegate>
...
-(instancetype) initWithContext:(id<QCConversationContext>)context {
    ...
    if (self) {
        ...
        self.scrollView.backgroundColor = [QCApp shared].config.backgroundColor;
...
-(void) layoutItems {
    ...
    for (NSInteger i=0; i<self.moreItems.count; i++) {
        QCMoreItemModel *model = self.moreItems[i];
        ...
        QCMoreItemView *moreView = [[QCMoreItemView alloc] initWithFrame:CGRectMake((columnIndex)*itemSpacing + columnIndex*itemWidth + contentInsets.left + pageIndex*self.scrollView.lim_width, (rowIndex)*lineSpacing+rowIndex*itemHeight+contentInsets.top, itemWidth, itemHeight)];
...
-(void) morePressed:(UIView*)view {
    QCMoreItemModel *model = self.moreItems[view.tag];
    if(model) {
...
-(UIScrollView*) scrollView {
    if(!_scrollView) {
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, QCScreenWidth, 0)];
...
- (UIPageControl *)pageControl {
    if(!_pageControl) {
        ...
        _pageControl.currentPageIndicatorTintColor = [QCApp shared].config.defaultTextColor;
...
@interface QCMoreItemView ()


@end

@implementation QCMoreItemView
