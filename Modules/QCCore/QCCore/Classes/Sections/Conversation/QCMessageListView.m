...
@interface QCMessageListView ()<UITableViewDelegate,UITableViewDataSource,QCConversationTableViewDelegate,QCChannelManagerDelegate,QCChatManagerDelegate,QCReactionManagerDelegate,QCConnectionManagerDelegate,QCTypingManagerDelegate,QCReminderManagerDelegate>
...
@property(nonatomic,strong,nullable) QCMessageModel *lastMessageInner;

@end

@implementation QCMessageListView
...
- (void)viewDidLoad {
    ...
    // 同步扩展数据
    [[QCSDK shared].chatManager syncMessageExtra:self.channel complete:nil];
    
    // 同步回应数据
    [[QCSDK shared].reactionManager sync:self.channel];
}
...
-(void) addDelegates {
    ...
    [[QCSDK shared].chatManager addDelegate:self]; // 消息监听
    [[QCSDK shared].reactionManager addDelegate:self]; // 回应监听
    [[QCSDK shared].connectionManager addDelegate:self]; // 连接状态监听
    [[QCTypingManager shared] addDelegate:self]; // 正在输入...
    [[QCReminderManager shared] addDelegate:self]; // 提醒项监听
...
-(void) removeDelegates {
    ...
    [[QCSDK shared].reactionManager removeDelegate:self];
    [[QCSDK shared].connectionManager  removeDelegate:self];
    [[QCTypingManager shared] removeDelegate:self];
    [[QCReminderManager shared] removeDelegate:self];
...
-(void) markReminderDoneIfNeed {
    NSArray<QCReminder*> *reminders = self.reminders;
    if(reminders && reminders.count>0) {
        ...
        [[QCSDK shared].reminderManager done:ids];
    }
...
-(void) handleLoadMessages:(BOOL)animation firstLoad:(BOOL)firstLoad hasMore:(bool)hasMore complete:(void(^)(void))complete {
    ...
    if(firstLoad) {
        QCMessage *typingMessage =  [[QCTypingManager shared] getTypingMessage:self.channel];
         if(typingMessage) {
             [self.dataProvider addTypingMessageIfNeed:[[QCMessageModel alloc] initWithMessage:typingMessage]];
         }
    }
    [self.tableView reloadData];
    ...
    if(!self.keepPosition) {
        ...
        if(self.needPositionReminder) {
            QCMessageBaseCell *cell =  (QCMessageBaseCell*) [self.tableView cellForRowAtIndexPath:indexPath];
            if(cell && [cell isKindOfClass:[QCMessageCell class]]) {
                [(QCMessageCell*)cell startReminderAnimation];
            }
...
-(void) updateOrInsertHistoryMsgSplitIfNeed {
    ...
    QCMessageModel *oldHistorySplitMsgModel;
...
-(void) insertHistoryMsgSplitUI {
    NSIndexPath *browseToIndex =  [self.dataProvider indexPathAtOrderSeq:self.browseToOrderSeq];
     if(browseToIndex) {
         QCMessageModel *browseMessageModel = [self.dataProvider messageAtIndexPath:browseToIndex];
...
//    QCMessageModel *browseToMessageModel = [self.conversationVM messageAtIndex:index];
    QCMessage *historySplitMessage = [[QCSDK shared].chatManager contentToMessage:QCHistorySplitTipContent.new channel:self.channel fromUid:@""];
    self.insertedHistoryClientMsgNo = historySplitMessage.clientMsgNo;
    [self.dataProvider insertMessage:[[QCMessageModel alloc] initWithMessage:historySplitMessage] atIndex:[NSIndexPath indexPathForRow:indexPath.row+1 inSection:indexPath.section]];
    [self.tableView reloadData];
}
...
-(QCConversationTableView*) tableView {
    if(!_tableView){
        _tableView = [[QCConversationTableView alloc] initWithFrame:CGRectMake(0.0f,0.0f,self.lim_width, self.lim_height) style:UITableViewStylePlain];
...
//        _tableView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        ...
        [_tableView registerClass:[QCTimeHeaderView class] forHeaderFooterViewReuseIdentifier:[QCTimeHeaderView reuseId]];
...
-(void) pulldown {
    QCMessageModel *firstMessage = [self.dataProvider firstMessage];
    __weak typeof(self) weakSelf = self;
...
-(BOOL) pullupHasMore {
    ...
    QCMessageModel *currentLastMsgModel = [self.dataProvider lastMessage];
...
-(void) updateBrowseToOrderSeq {
    ...
    if(lastVisibleIndexPath) {
        QCMessageModel *lastVisibleMessageModel = [self.dataProvider messageAtIndexPath:lastVisibleIndexPath];
...
-(void) updatePosition {
    ...
    QCMessageModel *firstMessageModel = [self.dataProvider messageAtIndexPath:firstIndexPath];
    ...
    self.keepPosition = [QCConversationPosition orderSeq:firstVisibleOrderSeq offset:offset];
}
...
-(BOOL) messageIsVisible:(NSString*)clientMsgNo {
    ...
    if(visibleRows && visibleRows.count>0) {
        for (NSIndexPath *indexPath in visibleRows) {
           QCMessageModel *visibleMessage = [self.dataProvider messageAtIndexPath:indexPath];
...
-(void) refreshNewMsgCount {
    ...
    if(self.browseToOrderSeq == 0) {
        ...
        uint32_t browseToMessageSeq = [[QCSDK shared].chatManager getOrNearbyMessageSeq:self.browseToOrderSeq];
...
-(void) refreshConversationListNewCount {
   QCConversationWrapModel *model = [[QCConversationListVM shared] modelAtChannel:self.channel];
    if(model) {
        ...
        [[QCSDK shared].conversationManager callOnConversationUpdateDelegate:[model getConversation]];
    }
}
...
// 获取消息cell的class
-(Class) getMessageCellClass:(QCMessageModel*) messageModel {
    if(messageModel.revoke) {
        return QCMessageRevokeCell.class;
    }
    Class  messageCellClass =  [[QCApp shared].messageRegitry getMessageCell:messageModel.contentType];
...
- (void)didReadedAndViewed {
    NSArray<NSIndexPath*> *visibleRows = [self.tableView indexPathsForVisibleRows];
    if(visibleRows && visibleRows.count>0) {
        NSMutableArray<QCMessageModel*> *messagesOfReaded = [NSMutableArray array];
        NSMutableArray<QCMessage*> *messagesOfViewed = [NSMutableArray array];
        // 仅私聊上报已读，群聊不开启该功能
        BOOL canReceipt = self.channel.channelType == WK_PERSON;
        for (NSIndexPath *indexPath in visibleRows) {
            CGRect rect = [self.tableView rectForRowAtIndexPath:indexPath];
            if([self cellIsVisible:rect]) {
                QCMessageModel *messageModel = [self.dataProvider messageAtIndexPath:indexPath];
        ...
        if(messagesOfViewed.count>0) {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [QCSDK.shared.flameManager didViewed:messagesOfViewed];
            });
...
-(void) locateMessageCellWithOrderSeqForReminder:(uint32_t)orderSeq tablePosition:(UITableViewScrollPosition)tablePosition{
    ...
    QCMessageModel *locatMessageModel; // 定位到的消息对象
    if(locatMessagePath) {
    ...
    [[QCConversationPositionManager shared] removePositions:self.channel type:QCConversationPositionTypeUnreadFirst];
    [[QCConversationPositionManager shared] channel:self.channel position:[QCConversationPosition orderSeq:orderSeq offset:0]];
    
    self.keepPosition = [QCConversationPosition orderSeq:orderSeq offset:0];
...
-(void) startReminderAnimation {
    ...
    if(reminders && reminders.count>0) {
        ...
        [self.tableView performBatchUpdates:^{
            ...
            for (QCReminder *reminder in reminders) {
                if(reminder.isLocate && !reminder.done) {
                    uint32_t orderSeq = [[QCSDK shared].chatManager getOrderSeq:reminder.messageSeq];
            ...
            if(indexPaths.count>0) {
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    for (NSIndexPath *indexPath in indexPaths) {
                        QCMessageBaseCell *cell =  (QCMessageBaseCell*) [weakSelf.tableView cellForRowAtIndexPath:indexPath];
                        if(cell && [cell isKindOfClass:[QCMessageCell class]]) {
                            [(QCMessageCell*)cell startReminderAnimation];
                        }
                    }
...
-(void) cancelAllSelected {
    ...
    if(visibleCells) {
        for (QCMessageBaseCell *baseCell in visibleCells) {
            if([baseCell isKindOfClass:[QCMessageCell class]]) {
                QCMessageCell *messageCell = (QCMessageCell*)baseCell;
...
-(void) locateMessageCellWithMessageSeq:(uint32_t )messageSeq {
    ...
    locatMessagePath = [self.dataProvider indexPathAtOrderSeq:[QCSDK.shared.chatManager getOrderSeq:messageSeq]];
    if(locatMessagePath) {
        locatMessageModel = [self.dataProvider messageAtIndexPath:locatMessagePath];
    }
    ...
    self.keepPosition = [QCConversationPosition orderSeq:[[QCSDK shared].chatManager getOrderSeq:messageSeq] offset:0];
    
    __weak typeof(self) weakSelf = self;
    [self loadMessages:true firstLoad:false complete:^{
        NSIndexPath *browseToIndex = [self.dataProvider indexPathAtOrderSeq:[QCSDK.shared.chatManager getOrderSeq:messageSeq]];
        if(browseToIndex) {
             QCMessageModel *messageModel =  [weakSelf.dataProvider messageAtIndexPath:browseToIndex];
 ...
 //
 //        // 判断是否启用pullup
 //        QCMessageModel *lastMessageModel = [self.conversationVM lastMessageModel];
...
// 是否需要处理消息
-(BOOL) needHandle:(QCMessage*)message {
...
-(void) handleRecvMessage:(QCMessage*) message  {
...
//通过删除然后插入的方式更新cell
-(void) refreshCellForDeleteAndInsert:(QCMessageModel*) messageModel {
...
- (void)setLastMessage:(QCMessageModel *)lastMessage {
...
-(BOOL) updateLastMsgIfNeed:(QCMessageModel*)messageModel {
    bool change = false;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    QCMessageBaseCell *cell = [self.tableView dequeueReusableCellWithIdentifier:identifier];
    if(!cell) {
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    QCMessageModel *messageModel = [self.dataProvider messageAtIndexPath:indexPath];
    QCMessageBaseCell *baseCell = (QCMessageBaseCell*)cell;
    baseCell.conversationContext = [self.dataProvider conversationContext];
    messageModel.checkboxOn =self.multipleOn;
    if([baseCell isKindOfClass:[QCMessageCell class]]) {
        ((QCMessageCell*)baseCell).showNavigateToMessage = self.showNavigateToMessage;
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NSArray<QCMessageModel*> *messages = [self.dataProvider messagesAtSection:section];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
   QCTimeHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:[QCTimeHeaderView reuseId]];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    NSDate *date = [dateFormatter dateFromString:[self.dataProvider dateWithSection:section]];
    headerView.dateLbl.text = [QCTimeTool formatDateStyle1:date];
//    [headerView.dateLbl sizeToFit];
    return headerView;
}

- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return [QCTimeHeaderView height];
}
...
// 消息更新
-(void) onMessageUpdate:(QCMessage*) message left:(NSInteger)left total:(NSInteger)total{
    QCLogDebug(@"onMessageUpdate-->%u",message.messageSeq);
    ...
    NSIndexPath *indexPath = [self.dataProvider indexPathAtClientMsgNo:message.clientMsgNo];
    if(indexPath) {
//        QCMessageModel *newMessageModel = [[QCMessageModel alloc]  initWithMessage:message];
        QCMessageModel *newMessageModel = [self.dataProvider messageAtIndexPath:indexPath];
        if(!newMessageModel) {
            newMessageModel =  [[QCMessageModel alloc]  initWithMessage:message];
        }
...
//            [self refreshCellForDeleteAndInsert:newMessageModel];
            ...
            if(message.remoteExtra.revoke) {
               NSArray<QCMessageModel*> *containReplyMessages = [self.dataProvider messagesAtMessageReply:message.messageId];
                if(containReplyMessages&&containReplyMessages.count>0) {
                    for (QCMessageModel *model in containReplyMessages) {
...
- (void)onMessageStream:(QCStream *)stream {
    ...
    CGFloat tableOffset = 0.0f;
     if(visibleCells && visibleCells.count>0) {
         for (UITableViewCell *tableCell in visibleCells) {
             if([tableCell isKindOfClass:QCMessageCell.class]) {
                 QCMessageCell *messageCell = (QCMessageCell*)tableCell;
                 QCMessageModel *messageModel = messageCell.messageModel;
...
// 消息已删除
-(void) onMessageDeleted:(QCMessage *)message {
    if(![self needHandle:message]) {
        return;
    }
...
-(void) deleteMessageUI:(QCMessage*)message {
    NSIndexPath *indexPath  = [self.dataProvider indexPathAtClientMsgNo:message.clientMsgNo];
    if(indexPath) {
        QCMessageModel *deleteMessageModel = [self.dataProvider messageAtIndexPath:indexPath];
...
// 清除所有消息
- (void)onMessageCleared:(QCChannel *)channel {
...
#pragma mark -- QCReactionManagerDelegate

- (void)reactionManagerChange:(QCReactionManager *)reactionManager reactions:(NSArray<QCReaction *> *)reactions channel:(QCChannel *)channel {
    ...
    for (QCReaction *reaction in reactions) {
        ...
        if(indexPath) {
            ...
            QCMessageCell *messageCell = [self.tableView cellForRowAtIndexPath:indexPath];
            [messageCell refresh:messageModel];
...
-(void) onConnectStatus:(QCConnectStatus)status reasonCode:(QCReason)reasonCode {
    if(status == QCConnected) { // 如果已连接，则重新请求消息
        __weak typeof(self) weakSelf = self;
        [self pullup:^(bool more) {
            QCMessageModel *localLastMsg = [weakSelf.dataProvider lastMessage];
            if(!localLastMsg) {
                return;
            }
            ...
            QCMessage *newLastMsg = [[QCSDK shared].chatManager getLastMessage:weakSelf.channel];
            
            if(newLastMsg) {
                [weakSelf updateLastMsgIfNeed:[[QCMessageModel alloc] initWithMessage:newLastMsg]];
            }
        }];
    }
}
...
- (void)typingAdd:(QCTypingManager *)manager message:(QCMessage *)message {
    if(![self.channel isEqual:message.channel] || [message.fromUid isEqualToString:[QCApp shared].loginInfo.uid]) {
        return;
    }
    if([self pullupHasMore]) {
        return;
    }
    QCMessageModel *model = [[QCMessageModel alloc] initWithMessage:message];
...
- (void)typingRemove:(QCTypingManager *)manager message:(QCMessage *)message  newMessage:(QCMessage *)newMessage{
    ...
    QCLogDebug(@"typingRemove------%@",newMessage);
...
#pragma mark -- QCReminderManagerDelegate

- (void)reminderManager:(QCReminderManager *)manager didChange:(QCChannel *)channel reminders:(NSArray<QCReminder *> *)reminders {
    if(![channel isEqual:self.channel]) {
        return;
    }
    NSArray<QCReminder*> *oldReminders = self.reminders;
    if(!oldReminders||oldReminders.count ==0) {
        self.reminders = reminders;
    }else {
        NSMutableArray<QCReminder*> *newReminders = [NSMutableArray array];
        for (QCReminder *reminder in reminders) {
            if(reminder.publisher && [reminder.publisher isEqualToString:QCApp.shared.loginInfo.uid]) {
                continue;
            }
            QCReminder *newReminder = [reminder copy];
            for (QCReminder *oldReminder in oldReminders) {
