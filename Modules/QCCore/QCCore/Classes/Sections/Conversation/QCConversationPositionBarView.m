//
//  QCConversationPositionBarView.m
//  QCCore
//
...
@interface QCConversationPositionBarView ()

@property(nonatomic,strong) NSArray<QCReminder*>*reminders;

@property(nonatomic,strong) NSMutableDictionary<NSString*,NSMutableArray<QCReminder*>*> *remindersDict;
...
@implementation QCConversationPositionBarView

- (instancetype)init
{
    self = [super init];
    if (self) {
        self.frame = CGRectMake(0.0f, 0.0f, QCPositionBarWidth, QCPositionBarHeight);
        [self addSubview:self.barBox];
    }
    return self;
}

-(void) updateReminders:(NSArray<QCReminder*>*)reminderList {
    
    NSMutableArray<QCReminder*> *reminders = [NSMutableArray array];
    if(reminderList && reminderList.count>0) {
        for (QCReminder *reminder in reminderList) {
    ...
    for (NSNumber *type in self.types) {
        ...
        NSArray<QCReminder*> *typeReminders =  self.remindersDict[key];
        QCPositionBar *bar = [[QCPositionBar alloc] initWithType:(QCConversationPositionType)type.integerValue];
        NSArray<QCConversationPosition*> *positions = [self toPositions:typeReminders];
        positions = [positions sortedArrayUsingComparator:^NSComparisonResult(QCConversationPosition*  _Nonnull obj1, QCConversationPosition * _Nonnull obj2) {
        ...
        [bar setOnClick:^ {
            ...
            if(weakSelf.onScrollToPosition) {
                QCConversationPosition *lastPosition = positions.lastObject;
                QCConversationPosition *firstPosition = positions.firstObject;
...
-(void) showScrollBottom:(BOOL)showScrollBottom animateComplete:(void(^__nullable)(void))animateComplete{
    ...
    if(showScrollBottom) {
        ...
        QCPositionBar *bar = [[QCPositionBar alloc] initWithType:QCConversationPositionTypeScrollToBottom];
...
-(void) updateScrollToBottomBarBadge:(NSInteger)value {
    QCPositionBar *scrollToBottomBar = (QCPositionBar*)[self.barBox viewWithTag:bottomBarViewTag];
    if(!scrollToBottomBar) {
        return;
    }
...
-(NSArray<QCConversationPosition*>*) toPositions:(NSArray<QCReminder*>*)reminders {
    NSMutableArray *positions = [NSMutableArray array];
    for (QCReminder *reminder in reminders) {
        [positions addObject:[QCConversationPosition orderSeq:[[QCSDK shared].chatManager getOrderSeq:reminder.messageSeq] offset:0 type:(QCConversationPositionType)reminder.type]];
    }
    return positions;
}
...
- (void)layoutSubviews{
    ...
    if(self.barBox.subviews.count>0) {
        self.hidden = NO;
        CGFloat totalHeight = self.barBox.subviews.count * QCPositionBarHeight + (self.barBox.subviews.count -1)*barSpace;
        self.barBox.frame =  CGRectMake(0.0f, 0.0f, QCPositionBarWidth, totalHeight);
...
- (NSMutableDictionary<NSString *,NSMutableArray<QCReminder *> *> *)remindersDict {
    if(!_remindersDict) {
...
@interface QCPositionBar ()

@property(nonatomic,assign) QCConversationPositionType type;
...
@implementation QCPositionBar

- (instancetype)initWithType:(QCConversationPositionType)type
{
    self = [super init];
    if (self) {
        self.type = type;
        ...
        [self.boxView setBackgroundColor:[QCApp shared].config.cellBackgroundColor];
//        self.backgroundColor = [UIColor greenColor];
        
        self.boxView.layer.cornerRadius = self.boxView.lim_height/2.0f;
        
        UIImage *icon;
        if(type == QCConversationPositionTypeScrollToBottom) {
            icon = [self imageName:@"Conversation/Index/MessageDown"];
        }else if(type == QCConversationPositionTypeMention) {
            icon = [self imageName:@"Conversation/Reminder/Mention"];
        }else if(type == QCConversationPositionTypeApplyJoinGroup) {
...
- (void)setPositions:(NSArray<QCConversationPosition *> *)positions {
...
- (QCBadgeView *)badgeView {
    if(!_badgeView) {
        _badgeView = [QCBadgeView viewWithBadgeTip:@""];
