...
@interface QCSettingMemberGridView ()
...
@implementation QCSettingMemberGridView
...
+(instancetype) initWithMaxWidth:(CGFloat) maxWidth numberOfLine:(NSInteger)number hasMore:(BOOL)hasMore {
    QCSettingMemberGridView *view = [QCSettingMemberGridView new];
...
- (UIButton *)moreBtn {
    if(!_moreBtn) {
        ...
        [[_moreBtn titleLabel] setFont:[[QCApp shared].config appFontOfSize:15.0f]];
        [_moreBtn setTitle:LLang(@"查看更多群成员") forState:UIControlStateNormal];
        [_moreBtn setTitleColor:[QCApp shared].config.themeColor forState:UIControlStateNormal];
        [_moreBtn sizeToFit];
