...
@interface QCConversationChannelHeader ()
...
@property(nonatomic,strong) QCAutoDeleteView *autoDeleteView;
...
@implementation QCConversationChannelHeader
...
//    [self setBackgroundColor:[UIColor redColor]];
    ...
    [QCApp.shared addChannelAvatarUpdateNotify:self selector:@selector(channelAvatarUpdate:)];
    
}

-(void) channelAvatarUpdate:(NSNotification*)notify {
    QCChannel *channel = notify.object;
...
- (void)setChannelInfo:(QCChannelInfo *)channelInfo {
    ...
    QCChannel *channel = channelInfo.channel;
    self.titleLbl.text = channelInfo.displayName;
    if(channel.channelType == WK_PERSON) {
        if([channel.channelId isEqualToString:[QCApp shared].config.systemUID]) {
        ...
        }else if([channelInfo.channel.channelId isEqualToString:[QCApp shared].config.fileHelperUID]) {
    ...
    if(channelInfo && (channelInfo.channel.channelType == WK_PERSON || channelInfo.channel.channelType == WK_CustomerService)) {
        ...
        NSString *onlineTip = [QCOnlineStatusManager.shared onlineStatusDetailTip:channelInfo];
        if(onlineTip) {
        ...
        if(channelInfo.logo && ![channelInfo.logo isEqualToString:@""]) {
            self.avatarImgView.url = [QCAvatarUtil getFullAvatarWIthPath:channelInfo.logo];
        }else{
            self.avatarImgView.url = [QCAvatarUtil getAvatar:channelInfo.channel.channelId];
        }
        ...
        if(channelInfo.logo && ![channelInfo.logo isEqualToString:@""]) {
            ...
            self.avatarImgView.url = [QCAvatarUtil getGroupAvatar:channelInfo.channel.channelId];
        }
    }
...
- (QCUserAvatar *)avatarImgView {
    if(!_avatarImgView) {
        _avatarImgView = [[QCUserAvatar alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 38.0f, 38.0f)];
...
- (UILabel *)titleLbl {
    if(!_titleLbl) {
        ...
        _titleLbl.font = [[QCApp shared].config appFontOfSizeSemibold:17.0f];
        _titleLbl.lim_height = 19.0f;
        _titleLbl.textColor = [QCApp shared].config.navBarTitleColor;
...
- (UILabel *)subtitleLbl {
    if(!_subtitleLbl) {
        ...
        _subtitleLbl.font = [[QCApp shared].config appFontOfSizeMedium:12.0f];
        _subtitleLbl.textColor = [QCApp shared].config.navBarSubtitleColor;
    }
    return _subtitleLbl;
}
...
- (UIButton *)voiceCallBtn {
    if(!_voiceCallBtn) {
        ...
        if (@available(iOS 13.0, *)) {
            img =  [[self imageName:@"Conversation/Index/VoiceCall"] imageWithTintColor:[QCApp shared].config.navBarButtonColor renderingMode:UIImageRenderingModeAlwaysTemplate];
        ...
        [_voiceCallBtn setTintColor:[QCApp shared].config.navBarButtonColor];
    }
...
- (UIButton *)videoCallBtn {
    if(!_videoCallBtn) {
        ...
        UIImage *img;
        if (@available(iOS 13.0, *)) {
            img =  [[self imageName:@"Conversation/Index/VideoCall"] imageWithTintColor:[QCApp shared].config.navBarButtonColor renderingMode:UIImageRenderingModeAlwaysTemplate];
        ...
        [_videoCallBtn setTintColor:[QCApp shared].config.navBarButtonColor];
    }
...
- (void)viewConfigChange:(QCViewConfigChangeType)type {
    if(type == QCViewConfigChangeTypeStyle) {
        ...
        self.subtitleLbl.textColor = [QCApp shared].config.tipColor;
    }
}
