...
@property(nonatomic,strong) QCUserAvatar *avatarImgView; // 群头像

@property(nonatomic,strong) UILabel *titleLbl; // 群标题

@property(nonatomic,strong) UILabel *remarkLbl; // 二维码备注

@property(nonatomic,strong) QCGroupQRCodeVM *vm;
...
@implementation QCGroupQRCodeVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.vm = [[QCGroupQRCodeVM alloc] initWithChannel:self.channel];
    ...
    [self.vm requestGetQRCodeInfo].then(^(QCGroupQRCodeInfoModel *model){
...
-(void) moreBtnPressed {
    QCActionSheetView2 *actionSheetView = [QCActionSheetView2 initWithCancel:nil];
    [actionSheetView addItem:[QCActionSheetButtonItem2 initWithTitle:LLang(@"保存图片") onClick:^{
...
-(QCUserAvatar*) avatarImgView {
    if(!_avatarImgView) {
        ...
        [_avatarImgView.avatarImgView lim_setImageWithURL:[NSURL URLWithString:[QCAvatarUtil getGroupAvatar:self.channel.channelId]]];
    }
...
-(UILabel*) titleLbl {
    if(!_titleLbl) {
       ...
       [_titleLbl setFont:[[QCApp shared].config appFontOfSizeMedium:16.0f]];
