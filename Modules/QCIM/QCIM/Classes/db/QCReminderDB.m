...
@implementation QCReminderDB

static QCReminderDB *_instance;
...
-(NSArray<QCReminder*>*) getWaitDoneReminders:(QCChannel*)channel type:(QCReminderType)type {
    ...
    __block NSMutableArray<QCReminder*> *reminders = [NSMutableArray array];
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        FMResultSet *resultSet = [db executeQuery:SQL_LIST_WAIT_DONE_WITH_TYPE_CHANNEL,channel.channelId,@(channel.channelType),@(type),QCSDK.shared.options.connectInfo.uid?:@""];
...
-(NSDictionary<QCChannel*,NSArray<QCReminder*>*>*) getAllWaitDoneReminders {
    ...
    NSMutableDictionary<QCChannel*,NSMutableArray<QCReminder*>*> *reminderDicts = [NSMutableDictionary dictionary];
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        FMResultSet *resultSet =  [db executeQuery:SQL_GET_ALL_WAIT_DONE,QCSDK.shared.options.connectInfo.uid?:@""];
        while (resultSet.next) {
            QCReminder *reminder = [self toReminder:resultSet];
            NSMutableArray<QCReminder*> *reminders = reminderDicts[reminder.channel];
            if(!reminders) {
...
-(NSArray<QCReminder*>*) getWaitDoneReminder:(QCChannel*) channel {
    __block NSMutableArray<QCReminder*> *reminders = [NSMutableArray array];
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        FMResultSet *resultSet = [db executeQuery:SQL_GET_NO_DONE_WITH_CHANNEL,channel.channelId,@(channel.channelType),QCSDK.shared.options.connectInfo.uid?:@""];
        while (resultSet.next) {
            [reminders addObject:[self toReminder:resultSet]];
        }
        [resultSet close];
...
-(NSDictionary<QCChannel*,NSArray<QCReminder*>*>*)  getWaitDoneReminders:(NSArray<QCChannel*>*) channels {
    ...
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        FMResultSet *resultSet =  [db executeQuery:[NSString stringWithFormat:@"%@ (%@)",SQL_GET_NO_DONE_WITH_CHANNELS,inquery],QCSDK.shared.options.connectInfo.uid?:@""];
        while (resultSet.next) {
            QCReminder *reminder = [self toReminder:resultSet];
            [reminders addObject:reminder];
        }
...
-(QCReminder*) toReminder:(FMResultSet*)resultSet {
    ...
    QCChannel *channel = [[QCChannel alloc] initWith:channelID channelType:channelType];
