...
static QCDB *_instance;

static QCDBMigrationManager *_manager;
...
-(BOOL) needSwitchDB:(NSString*)uid {
    NSString *dbPrefix = [QCSDK shared].options.dbPrefix;
...
-(void) switchDB:(NSString*)uid {
   ...
   BOOL isExist = [fileManager fileExistsAtPath: [QCSDK shared].options.dbDir];
    if(!isExist) {
        NSError *error;
        if (![fileManager createDirectoryAtPath:[QCSDK shared].options.dbDir withIntermediateDirectories:YES attributes:nil error:&error]) {
            NSLog(@"creat Directory Failed:%@",[error localizedDescription]);
        }
    }
    NSString *dbPath = [NSString stringWithFormat:@"%@/%@%@.db",[QCSDK shared].options.dbDir,[QCSDK shared].options.dbPrefix,uid];
...
//    [QCFMEncryptHelper shared].encryptKey = dbKey;
//    [[QCFMEncryptHelper shared] unEncryptDatabase:dbPath targetPath:[NSString stringWithFormat:@"%@.tmp.db",dbPath]];
//
    
    FMDatabaseQueue *fmdb = [FMDatabaseQueue databaseQueueWithPath:dbPath];
  
    self.dbQueue = [QCFMDatabaseQueue databaseQueue:fmdb] ;
...
-(void) createMigrationsTable{
    [[self dbQueue] inDatabase:^(FMDatabase * _Nonnull db) {
        _manager = [QCDBMigrationManager managerWithDatabase:db];
...
- (NSBundle*) resourceBundle{
    NSURL *url = [self module_bundleUrl:[NSString stringWithFormat:@"QCIM"]  cls:[self class]];
