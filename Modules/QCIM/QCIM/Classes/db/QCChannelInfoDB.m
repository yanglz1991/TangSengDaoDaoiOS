...
@implementation QCChannelInfoDB

static QCChannelInfoDB *_instance;
...
-(BOOL) saveChannelInfo:(QCChannelInfo*)channelInfo db:(FMDatabase*) db{
...
-(void) updateChannelOnlineStatus:(QCChannel*)channel status:(QCOnlineStatus)status lastOffline:(NSTimeInterval)lastOffline {
    [self updateChannelOnlineStatus:channel status:status lastOffline:lastOffline mainDeviceFlag:QCDeviceFlagEnumUnknown];
}
...
-(void) updateChannelOnlineStatus:(QCChannel*)channel status:(QCOnlineStatus)status lastOffline:(NSTimeInterval)lastOffline mainDeviceFlag:(QCDeviceFlagEnum)mainDeviceFlag{
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        if(status == QCOnlineStatusOnline) {
...
-(void) updateChannelInfo:(QCChannelInfo*)channelInfo db:(FMDatabase*)db {
    NSString *parentChannelID = channelInfo.parentChannel?channelInfo.parentChannel.channelId:@"";
    NSInteger parentChannelType = channelInfo.parentChannel?channelInfo.parentChannel.channelType:0;
    [db executeUpdate:SQL_CHANNEL_UPDATE,parentChannelID,@(parentChannelType),channelInfo.name?:@"",@(channelInfo.follow),channelInfo.notice?:@"",channelInfo.logo?:@"",channelInfo.remark?:@"",@(channelInfo.stick),@(channelInfo.mute),@(channelInfo.showNick),@(channelInfo.save),@(channelInfo.forbidden),@(channelInfo.invite),[self extraToStr:channelInfo.extra],@(channelInfo.status),@(channelInfo.online),@(channelInfo.receipt),@(channelInfo.robot),@(channelInfo.lastOffline),@(channelInfo.deviceFlag),channelInfo.category?:@"",@(channelInfo.beDeleted),@(channelInfo.beBlacklist),@(channelInfo.flame),@(channelInfo.flameSecond),channelInfo.channel.channelId?:@"",@(channelInfo.channel.channelType)];
}
...
-(QCChannelInfo*) queryChannelInfo:(QCChannel*)channel {
    ...
    __block QCChannelInfo *channelInfo;
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
...
-(NSArray<QCChannelInfo*>*) queryChannelInfoWithFriend:(NSString*)keyword limit:(NSInteger)limit{
    ...
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        ...
        FMResultSet * result = [db executeQuery:@"select * from channel where ( lower(name) like ? or lower(remark) like ?) and follow=? limit ?",likeKeyword,likeKeyword,@(QCChannelInfoFollowFriend),@(limit)];
        while(result.next) {
...
-(NSArray<QCChannelInfo*>*) queryChannelInfosWithStatusAndFollow:(QCChannelStatus)status follow:(QCChannelInfoFollow)follow {
    __block NSMutableArray *channelInfos = [NSMutableArray array];
       [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
           FMResultSet * result;
           if(follow == QCChannelInfoFollowAll) {
...
-(NSArray<QCChannelMessageSearchResult*>*) searchChannelMessageWithKeyword:(NSString*)keyword  limit:(NSInteger)limit {
    ...
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        FMResultSet *result =  [db executeQuery:SQL_CHANNEL_MESSAGE_SEARCH,likeKeyword,@(limit)];
         while(result.next) {
//             QCChannelInfo *channelInfo = [self toChannelInfo:result.resultDictionary];
             QCChannelMessageSearchResult *searchResult = [QCChannelMessageSearchResult new];
             NSString *channelID = [result stringForColumn:@"channel_id"];
             NSInteger channelType = [result intForColumn:@"channel_type"];
             searchResult.channel = [[QCChannel alloc] initWith:channelID channelType:channelType];
...
-(NSArray<QCChannelInfoSearchResult*>*) searchChannelInfoWithKeyword:(NSString*)keyword channelType:(uint8_t)channelType limit:(NSInteger)limit {
    ...
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
       FMResultSet * result =  [db executeQuery:SQL_CHANNEL_SEARCH,likeKeyword,likeKeyword,likeKeyword,likeKeyword,@(channelType),@(limit)];
        while(result.next) {
           QCChannelInfo *channelInfo = [self toChannelInfo:result.resultDictionary];
            QCChannelInfoSearchResult *searchResult = [QCChannelInfoSearchResult new];
            searchResult.channelInfo = channelInfo;
...
-(BOOL) existChannelInfo:(QCChannel*)channel db:(FMDatabase*) db {
    ...
    if(!channelInfos || channelInfos.count<=0) {
     ...
     [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
         for (QCChannelInfo *channelInfo in channelInfos) {
             QCChannelInfo *oldChannelInfo = [self queryChannelInfo:channelInfo.channel db:db];
             if(oldChannelInfo) {
...
-(QCChannelInfo*) toChannelInfo:(NSDictionary*)dict {
    QCChannelInfo *channelInfo = [QCChannelInfo new];
    channelInfo.channel = [[QCChannel alloc] initWith:dict[@"channel_id"] channelType:[dict[@"channel_type"] integerValue]];
    if(dict[@"parent_channel_id"] && ![dict[@"parent_channel_id"] isEqualToString:@""]) {
        channelInfo.parentChannel = [QCChannel channelID:dict[@"parent_channel_id"] channelType:[dict[@"parent_channel_type"] integerValue]];
    }
