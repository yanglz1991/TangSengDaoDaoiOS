...
@implementation QCMessageDB

static QCMessageDB *_instance;
...
-(NSArray<QCMessage*>*) saveMessages:(NSArray<QCMessage*>*)messages {
    __block NSMutableArray<QCMessage*> *newMessages = [NSMutableArray array];
    @synchronized(self) {
        [[QCDB sharedDB].dbQueue inTransaction:^(FMDatabase * _Nonnull db, BOOL * _Nonnull rollback) {
            for(QCMessage *message in messages) {
                QCMessage *existMessage = [self getMessageWithMessageIdOrClientMsgNo:message.messageId clientMsgNo:message.clientMsgNo db:db];
                if(existMessage) {
                ...
                if(message.messageSeq!=0) {
                    orderSeq = message.messageSeq*QCOrderSeqFactor;
                }else{
                ...
                if(message.hasRemoteExtra) { // 添加扩展消息
                    [[QCMessageExtraDB shared] addOrUpdateMessageExtra:message.remoteExtra db:db];
                }
                if(message.reactions && message.reactions.count>0) {
                    [[QCReactionDB shared] insertOrUpdateReactions:message.reactions db:db];
                }
    ...
    [[QCDB sharedDB].dbQueue inTransaction:^(FMDatabase * _Nonnull db, BOOL * _Nonnull rollback) {
        for (QCMessage *message in messages) {
            ...
            if(success) {
                ...
                QCMessage *dupMessage = [self getMessageWithClientMsgNo:message.clientMsgNo db:db]; // 查询重复的clientMsgNo消息
...
-(BOOL) insertMessage:(QCMessage*)message db:(FMDatabase*)db clientMsgNo:(NSString*)clientMsgNo isDeleted:(NSInteger)isDeleted{
    NSString *searchableWord = @"";
    if(message.content && message.content.searchableWord) {
        searchableWord =message.content.searchableWord;
    }
...
-(NSArray<QCMessage*>*) getMessagesWithClientMsgNos:(NSArray*)clientMsgNos {
    ...
    __block NSArray<QCMessage*> *messages = [NSArray new];
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
...
-(NSArray<QCMessage*>*) getMessagesWithClientMsgNos:(NSArray*)clientMsgNos db:(FMDatabase*)db{
    if(!clientMsgNos || clientMsgNos.count<=0) {
        return nil;
    }
    __block NSMutableArray *messages = [NSMutableArray new];
...
-(QCMessage*) getMessageWithClientMsgNo:(NSString*)clientMsgNo db:(FMDatabase*)db{
    __block QCMessage *message;
    FMResultSet *result = [db executeQuery:SQL_MESSAGE_WITH_CLIENTMSGNO,clientMsgNo];
    if(result.next) {
        message = [self toMessage:result.resultDictionary db:db];
...
-(QCMessage*) getMessage:(uint32_t)clientSeq db:(FMDatabase*)db{
    __block QCMessage *message;
    FMResultSet *resultSet = [db executeQuery:SQL_MESSAGE_WITH_ID,@(clientSeq)];
    if(resultSet.next) {
        message = [self toMessage:resultSet.resultDictionary db:db];
...
-(QCMessage*) getMessageWithMessageId:(uint64_t)messageId db:(FMDatabase*)db {
    QCMessage *message;
    FMResultSet *resultSet = [db executeQuery:SQL_MESSAGE_WITH_MESSAGEID,@(messageId)];
    if(resultSet.next) {
        message = [self toMessage:resultSet.resultDictionary db:db];
...
-(QCMessage*) getMessageWithMessageIdOrClientMsgNo:(uint64_t)messageId clientMsgNo:(NSString*)clientMsgNo db:(FMDatabase*)db {
...
-(void) updateMessageMaxSeqWithMessageIDOrClientMsgNo:(uint32_t)messageSeq messageID:(uint64_t)messageID clientMsgNo:(NSString*)clientMsgNo db:(FMDatabase*)db{
    [db executeUpdate:SQL_MESSAGE_UPDATE_MAX_SEQ_WITH_MESSAGEID_OR_CLIENTMSGNO,@(messageSeq),@(messageSeq*QCOrderSeqFactor),@(messageID),clientMsgNo?:@""];
}
...
-(void) updateMessageWithSendackPackets:(NSArray<QCSendackPacket*> *)sendackPackets {
    ...
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        for (QCSendackPacket *sendackPacket in sendackPackets) {
            ...
            if(sendackPacket.messageSeq!=0) {
                orderSeq = sendackPacket.messageSeq*QCOrderSeqFactor;
            }else {
                QCMessage *message=  [self getMessage:sendackPacket.clientSeq db:db];
                if(message) {
                    orderSeq = [self getMaxOrderSeqWithChannel:db channel:message.channel]+1;
                }
...
-(uint32_t) getMaxMessageSeqWithChannel:(FMDatabase*)db channel:(QCChannel*)channel {
...
-(uint32_t) getMaxOrderSeqWithChannel:(FMDatabase*) db channel:(QCChannel*)channel {
...
- (void)deleteMessage:(QCMessage *)message db:(FMDatabase*)db{
...
-(void) saveOrUpdateStreams:(NSArray<QCStream*>*)streams db:(FMDatabase*)db{
    for (NSInteger i=0; i<streams.count; i++) {
        QCStream *stream = streams[i];
...
-(QCStream*) toStream:(FMResultSet*)resultSet db:(FMDatabase*)db {
    ...
    stream.channel = [QCChannel channelID:channelID channelType:channelType];
...
-(QCMessage*) toMessage:(NSDictionary*)dict db:(FMDatabase*)db{
    ...
    message.channel = [[QCChannel alloc] initWith:dict[@"channel_id"] channelType:[dict[@"channel_type"] integerValue]];
    if(dict[@"parent_channel_id"] && ![dict[@"parent_channel_id"] isEqualToString:@""]) {
        message.parentChannel = [[QCChannel alloc] initWith:dict[@"parent_channel_id"] channelType:[dict[@"parent_channel_type"] integerValue]];
    }
    ...
    if(dict[@"content"] && [dict[@"content"] isKindOfClass:[NSData class]]) {
        ...
        message.content = [[QCUnknownContent alloc] init];
    ...
    if(dict[@"setting"]) {
        message.setting = [QCSetting fromUint8:[dict[@"setting"] integerValue]];
    }
...
//    if([QCSDK shared].options.mosConvertOn) {
//        contentType = [[QCMOSContentConvertManager shared] convertTypeToLM:contentType];
//    }
     Class contentCls = [[QCSDK shared] getMessageContent:contentType];
    QCMessageContent *messageContent = [contentCls new];
