...
-(uint8_t) readUint8 {
    uint8_t v;
     [QCDataRead numberHNMemcpy:&v src:[[self.buffData subdataWithRange:NSMakeRange(self.offset, 1)] bytes] count:1];
     self.offset+=1;
    return v;
}
...
-(uint16_t) readUint16 {
    uint16_t v;
     [QCDataRead numberHNMemcpy:&v src:[[self.buffData subdataWithRange:NSMakeRange(self.offset, 2)] bytes] count:2];
    self.offset+=2;
    return v;
}
...
-(uint32_t) readUint32 {
    uint32_t v;
     [QCDataRead numberHNMemcpy:&v src:[[self.buffData subdataWithRange:NSMakeRange(self.offset, 4)] bytes] count:4];
    self.offset+=4;
    return v;
}
...
-(uint64_t) readUint64 {
    uint64_t v;
    [QCDataRead numberHNMemcpy:&v src:[[self.buffData subdataWithRange:NSMakeRange(self.offset, 8)] bytes] count:8];
    self.offset+=8;
    return v;
}
...
-(BOOL) lengthFull {
    ...
    while (multiplier < 27) {
        ...
        uint8_t digit;
        [QCDataRead numberHNMemcpy:&digit src:[[self.buffData subdataWithRange:NSMakeRange(i, 1)] bytes] count:1];
