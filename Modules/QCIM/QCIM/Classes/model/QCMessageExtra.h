...
typedef enum : NSUInteger {
    QCContentEditUploadStatusSuccess, // 成功
    QCContentEditUploadStatusWait, // 等待上传
    QCContentEditUploadStatusError, // 上传错误
} QCContentEditUploadStatus; // 编辑正文上传状态

@interface QCMessageExtra : NSObject
...
@property(nonatomic,strong,nullable) QCMessageContent *contentEdit; // 消息编辑后的正文
@property(nonatomic,assign) NSInteger editedAt; // 消息编辑时间 （0表示消息未被编辑）
@property(nonatomic,assign) BOOL isEdit; //  是否编辑
@property(nonatomic,assign) QCContentEditUploadStatus uploadStatus; // 上传状态
