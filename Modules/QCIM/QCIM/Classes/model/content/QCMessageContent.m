...
NSString * const QCEntityTypeRobotCommand = @"bot_command";
...
@implementation QCMessageEntity


+(QCMessageEntity*) type:(NSString*)type range:(NSRange)range{
    QCMessageEntity *entity = [QCMessageEntity new];
    entity.type = type;
    entity.range = range;
    return entity;
}

+(QCMessageEntity*) type:(NSString*)type range:(NSRange)range value:(id)value {
    QCMessageEntity *entity = [QCMessageEntity new];
    entity.type = type;
    entity.range = range;
    entity.value = value;
...
@implementation QCMentionedInfo

- (instancetype)initWithMentionedType:(QCMentionedType)type {
...
- (BOOL)isMentionedMe {
    if([self.uids containsObject:[QCSDK shared].options.connectInfo.uid]||self.type == WK_Mentioned_All) {
        return true;
    }
    return false;
}
...
@implementation QCReply


@end

@interface QCMessageContent ()

@property(nonatomic,strong) FMDatabase *db;

@end

@implementation QCMessageContent
...
- (void)decodeSenderUserInfo:(NSDictionary *)dict {
    if(dict[@"from_uid"]) {
         self.senderUserInfo = [[QCUserInfo alloc] initWithUid:dict[@"from_uid"] name:dict[@"from_name"] avatar:@""];
    }
...
-(void) decodeMentionInfo:(NSMutableDictionary*)dict {
    ...
    if(mentionDict && [mentionDict isKindOfClass:[NSDictionary class]]) {
        QCMentionedType type = WK_Mentioned_Users;
        ...
        if([mentionDict[@"uids"] isKindOfClass:[NSArray class]]) {
         ...
         self.mentionedInfo = [[QCMentionedInfo alloc] initWithMentionedType:type uids:uids];
    }
}
...
-(void) decodeReply:(NSDictionary*)dict {
    ...
    if(replyDict) {
        ...
        if(!reply.fromName||[reply.fromName isEqualToString:@""]) {
            QCChannel *fromChannel = [QCChannel personWithChannelID:reply.fromUID];
            QCChannelInfo *fromChannelInfo;
            if(self.db) {
                fromChannelInfo = [[QCSDK shared].channelManager getCache:fromChannel];
                if(!fromChannelInfo) {
                    fromChannelInfo = [[QCChannelInfoDB shared] queryChannelInfo:fromChannel db:self.db];
                }
                
            }else{
                fromChannelInfo = [[QCSDK shared].channelManager getChannelInfo:fromChannel];
            }
            if(fromChannelInfo) {
        ...
        if(replyDict[@"payload"]) {
           ...
           reply.content =  [[QCSDK shared].chatManager getMessageContent:contentType];
...
-(QCMessageEntity*) toEntity:(NSDictionary*)entityDict {
    ...
    return [QCMessageEntity type:type range:NSMakeRange(offset, length) value:value];
}
...
-(void) encodeEntities:(NSMutableDictionary*)dict {
    if(self.entities && self.entities.count>0) {
        ...
        for (QCMessageEntity *entity in self.entities) {
...
- (NSArray<QCMessageEntity *> *)entities {
    if(!_entities) {
...
- (nonnull id)copyWithZone:(nullable NSZone *)zone {
    QCMessageContent *messageContent = [[[self class] allocWithZone:zone] init];
