...
typedef enum : NSUInteger {
    QCChannelInfoFollowStrange = 0, // 未关注
    QCChannelInfoFollowFriend = 1, // 已关注
     QCChannelInfoFollowAll = 2,
} QCChannelInfoFollow;

// 扩展字段的key
typedef NSString *QCChannelExtraKey NS_STRING_ENUM;

@interface QCChannelInfo : NSObject<NSCopying>

// 频道
@property(nonatomic,strong) QCChannel *channel;

// 父类频道
@property(nonatomic,strong,nullable) QCChannel *parentChannel;
/**
 是否已关注 0.未关注（陌生人） 1.已关注（好友）
 */
@property(nonatomic,assign) QCChannelInfoFollow follow;
...
/**
 扩展字段
 */
@property(nonatomic,strong) NSMutableDictionary<QCChannelExtraKey,id> *extra;

/// 获取扩展字段内的值
/// @param key <#key description#>
-(id) extraValueForKey:(QCChannelExtraKey)key;

-(id) extraValueForKey:(QCChannelExtraKey)key defaultValue:(id _Nullable)value;
