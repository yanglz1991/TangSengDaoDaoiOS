...
@protocol QCChatManagerDelegate;

NS_ASSUME_NONNULL_BEGIN
typedef BOOL(^MessageStoreBeforeIntercept)(QCMessage*message);



@interface QCChatManager : NSObject
...
/**
 ...
 @param setting 消息设置
 */
-(QCMessage*) sendMessage:(QCMessageContent*)content channel:(QCChannel*)channel setting:(QCSetting * __nullable)setting;

-(QCMessage*) sendMessage:(QCMessageContent*)content channel:(QCChannel*)channel setting:(QCSetting * __nullable)setting topic:(NSString*)topic;
...
/**
 ...
 @return <#return value description#>
 */
-(QCMessage*) saveMessage:(QCMessageContent*)content channel:(QCChannel*)channel fromUid:( NSString* __nullable)fromUid;

-(QCMessage*) saveMessage:(QCMessageContent*)content channel:(QCChannel*)channel fromUid:(NSString* __nullable)fromUid  status:(QCMessageStatus)status;
...
-(void) pullUp:(QCChannel*)channel startOrderSeq:(uint32_t)startOrderSeq endOrderSeq:(uint32_t)endOrderSeq limit:(int)limit complete:(void(^)(NSArray<QCMessage*> *messages,NSError *error))complete;
 ...
 */
-(void) pullAround:(QCChannel*)channel orderSeq:(uint32_t)orderSeq  limit:(int)limit complete:(void(^)(NSArray<QCMessage*> *messages,NSError *error))complete;
 ...
 */
-(void) pullAround:(QCChannel*)channel orderSeq:(uint32_t)orderSeq maxMessageSeq:(uint32_t)maxMessageSeq  limit:(int)limit complete:(void(^)(NSArray<QCMessage*> *messages,NSError *error))complete;
 ...
 */
-(void) getMessages:(QCChannel*)channel aroundOrderSeq:(uint32_t)aroundOrderSeq  limit:(int)limit complete:(void(^)(NSArray<QCMessage*> *messages,NSError *error))complete DEPRECATED_MSG_ATTRIBUTE("use pullAround");;
...
/// @param channel 频道
/// @param fromUid 发送者UID
-(QCMessage*) contentToMessage:(QCMessageContent*)content channel:(QCChannel*)channel fromUid:(NSString * __nullable)fromUid;
...
/**
  标记消息为已读
 */
//-(void) markReaded:(QCChannel*)channel messages:(NSArray<QCMessage*>*)messages;
...
/// 同步频道消息提供者（由第三方设置）
@property(nonatomic,copy) QCSyncChannelMessageProvider syncChannelMessageProvider;

@property(nonatomic,copy) QCSyncMessageExtraProvider syncMessageExtraProvider; // 同步消息扩展
@property(nonatomic,copy) QCUpdateMessageExtraProvider updateMessageExtraProvider; // 更新消息扩展

// 消息编辑提供者
@property(nonatomic,copy) QCMessageEditProvider messageEditProvider;
...
/**
 聊天管理委托
 */
@protocol QCChatManagerDelegate <NSObject>
