...
#import "QCRobotDB.h"
@implementation QCRobotManager


static QCRobotManager *_instance;
...
-(void) sync:(NSArray<NSString*>*)robotIDs complete:(void(^)(BOOL hasData,NSError *error))complete{
    ...
    self.syncRobotProvider(robotVersions, ^(NSArray<QCRobot *> * _Nullable robots, NSError * _Nullable error) {
        if(error) {
            NSLog(@"同步机器人失败！->%@",error);
            if(complete) {
        ...
        [[QCRobotDB shared] addOrUpdateRobots:robots];
        if(complete) {
...
-(void) syncWithUsernames:(NSArray<NSString*>*)usernames complete:(void(^)(BOOL hasData,NSError *error))complete {
    ...
    NSArray<QCRobot*> *robots = [[QCRobotDB shared] queryRobotsWithUsernames:usernames];
    NSMutableArray *robotVersions = [NSMutableArray array];
...
-(QCRobot*) getRobotWithUsername:(NSString*)username {
    return [[QCRobotDB shared] queryRobotWithUsername:username];
}
