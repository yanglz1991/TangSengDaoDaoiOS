...
- (void)refresh:(QCMessageModel *)model {
    ...
    QCFileContent *content = (QCFileContent*)model.content;
    self.fileNameLbl.text =content.name;
    self.sizeLbl.text = [[QCFileCommon shared] sizeFormat:content.size];
    
   QCFileInfoModel *fileModel = [[QCFileCommon shared] fileInfoWithName:content.name];
   [self.fileHeaderView setBackgroundColor:fileModel.fileColor];
    ...
    if(model.isSend) {
        self.fileNameLbl.textColor = [QCApp shared].config.messageSendTextColor;
        self.sizeLbl.textColor = [QCApp shared].config.messageTipColor;
    }else{
        self.fileNameLbl.textColor = [QCApp shared].config.messageRecvTextColor;
...
- (void)onTap {
    [super onTap];
    QCFileInfoVC *fileInfo = [QCFileInfoVC new];
    fileInfo.fileMessage = self.messageModel;
    [[QCNavigationManager shared] pushViewController:fileInfo animated:YES];
}
