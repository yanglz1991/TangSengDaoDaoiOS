...
@interface QCContactsSearchVC ()<UITextFieldDelegate>
@property(nonatomic,strong) UIView *searchBarView;
@property(nonatomic,strong) UITextField *searchBarInput;
@end

@implementation QCContactsSearchVC
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    [self.viewModel searchFriend:textField.text].then(^(QCUserSearchResp *resp){
        if(!resp.exist) {
            [weakSelf.view showMsg:LLang(@"用户不存在！")];
            return;
        }
        [[QCApp shared] invoke:QCPOINT_USER_INFO param:@{@"uid":resp.user.uid,@"vercode":resp.user.vercode?:@""}];
    });
    return YES;
}
// 重写返回事件
