...
@implementation QCContactsFriendModel



@end

@interface QCContactsFriendCell ()

@property(nonatomic,strong) UILabel *firstNameLbl;

@property(nonatomic,strong) UILabel *phoneLbl;

@property(nonatomic,strong) QCContactsFriendModel *contactsFriendModel;
...
@implementation QCContactsFriendCell
...
-(void) refreshWithModel:(id)cellModel {
    ...
    if(self.contactSelectModel.uid && ![self.contactSelectModel.uid isEqualToString:@""]) {
        ...
        self.avatarImgView.url = [QCAvatarUtil getAvatar:self.contactSelectModel.uid];
    }else{
        self.firstNameLbl.hidden = NO;
        self.avatarImgView.avatarImgView.image = nil;
        [self.avatarImgView setBackgroundColor:[QCUserColorUtil userColor:self.contactSelectModel.name]];
        self.firstNameLbl.text = [self.contactSelectModel.name substringToIndex:1];
        [self.firstNameLbl sizeToFit];
    }
    
    [self.actionBtn setBackgroundColor:[QCApp shared].config.themeColor];
    ...
    if(self.contactsFriendModel.isFriend) {
        ...
        [self.actionBtn setBackgroundColor:[QCApp shared].config.tipColor];
    ...
    if([self.contactsFriendModel.uid isEqualToString:QCApp.shared.loginInfo.uid]) {
...
- (UILabel *)firstNameLbl {
    if(!_firstNameLbl) {
        ...
        _firstNameLbl.font = [[QCApp shared].config appFontOfSizeSemibold:25.0f];
...
- (UILabel *)phoneLbl {
    if(!_phoneLbl) {
        ...
        _phoneLbl.textColor = [QCApp shared].config.tipColor;
        _phoneLbl.font = [[QCApp shared].config appFontOfSize:14.0f];
    }
    return _phoneLbl;
}
...
- (UIButton *)actionBtn {
    if(!_actionBtn) {
        ...
        [[_actionBtn titleLabel] setFont:[[QCApp shared].config appFontOfSize:14.0f]];
