...
// 编码(lmproto)
-(NSData*) encodeLM:(QCPacket*)packet {
    ...
    // 编码body
    NSData *bodyBytes =  [[[QCSDK shared].bodyCoderManager getBodyCoder:packet.header.packetType] encode:packet];
    // 编码剩余长度并组合
...
// 解码(lmproto)
-(QCPacket*) decodeLM:(NSData*)data {
    QCDataRead *reader = [[QCDataRead alloc] initWithData:data];
    uint8_t flag =  [reader readUint8];
    uint8_t packetType = flag >> 4;
    if(packetType == WK_PONG) {
        return [QCPongPacket new];
    }
   uint32_t remainingLength =  [reader readLenth];
    
    QCHeader *header = [QCHeader new];
   ...
   return  [[[QCSDK shared].bodyCoderManager getBodyCoder:packetType] decode:reader.remainingData header:header];
}
