...
@interface QCMessageHeader : NSObject
// 是否红点
// 是否显示未读
...
@property(nonatomic,strong) QCMessageHeader *header; // 消息头

@property(nonatomic,strong) QCSetting *setting; // 消息设置
...
@property(nonatomic,assign) QCStreamFlag streamFlag; // 流式标记
...
@property(nonatomic,strong) QCChannelInfo *from; // 发送者
...
@property(nonatomic,strong,readonly) QCChannelMember *memberOfFrom; // 发送者在频道里的成员信息

@property(nonatomic,strong) QCChannel *channel; // 频道

@property(nonatomic,strong) QCChannel *parentChannel; // 父类频道

@property(nullable,nonatomic,strong,readonly) QCChannelInfo *channelInfo; //  频道资料，可能为空，如果为空可以调用WKChannelManager fetchChannelInfo:completion 触发频道信息变更委托
...
@property(nonatomic,assign) QCMessageStatus status; // 消息状态
@property(nonatomic,assign) QCReason reasonCode; // 原因代码,当status为WK_MESSAGE_FAIL时 应该有相应的原因代号
@property(nonatomic,weak,readonly) id<QCTaskProto> task; // 消息关联的任务（例如：下载图片任务，上传图片任务等等）

- (BOOL)isSend; // 是否是发送消息

@property(nonatomic,strong) NSMutableDictionary *extra; // 消息本地扩展数据
@property(nonatomic,strong,nullable) NSArray<QCReaction*> *reactions; // 消息回应集合
...
@property(nonatomic,strong) QCMessageExtra *remoteExtra; // 消息远程扩展

@property(nonatomic,assign) BOOL  syncStreamsFromDB; // 是否从db同步流数据
@property(nonatomic,strong) NSMutableArray<QCStream*> *streams; // 流式消息内容
