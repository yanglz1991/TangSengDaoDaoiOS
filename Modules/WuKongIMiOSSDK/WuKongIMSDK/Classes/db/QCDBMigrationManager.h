...
@protocol QCDBMigrating;

/**
 @abstract The `QCDBMigrationManager` class provides a simple, flexible interface for managing migrations for
 a SQLite database that is accessed via FMDB.
 */
@interface QCDBMigrationManager : NSObject
...
/**
 ...
 @discussion When `YES` all classes will be enumerated to search for any that conform to the `QCDBMigrating` protocol.
 */
...
/**
 @abstract Returns all migrations discovered by the receiver. Each object returned conforms to the `QCDBMigrating` protocol. The
 ...
 and accumulating all classes that conform to the `QCDBMigrating` protocol. These migrations can then be sorted and applied
...
/**
 ...
 @param migrations An array of objects conforming to `QCDBMigrating` protocol.
 */
...
/**
 @abstract Creates the `schema_migrations` table used by `QCDBMigrationManager` to maintain an index of applied migrations.
...
/**
 @abstract The `QCDBMigrating` protocol is adopted by classes that wish to provide migration of SQLite databases accessed via FMDB. The
 `QCDBMigrationManager` project ships with a single concrete implementation in the `FMDBFileMigration` class. Non file backed
 migrations can be implemented by conforming to the `QCDBMigrating` protocol.
 */
@protocol QCDBMigrating <NSObject>
...
/**
 ...
 @discussion The `QCDBMigrationManager` manages a transaction while migrations are being applied. Should any call to `migrateDatabase:error` return `NO`,
...
/**
 @abstract The `FMDBFileMigration` class provides a concrete implementation of the `QCDBMigrating` protocol that models
 ...
 */
@interface QCDBFileMigration : NSObject <QCDBMigrating>
...
/**
 @abstract The domain for errors created by `QCDBMigrationManager`.
 */
extern NSString *const QCDBMigrationManagerErrorDomain;
 ...
 */
extern NSString *const QCDBMigrationManagerProgressVersionUserInfoKey;

/**
 @abstract A key for an `id<QCDBMigrating>` object in the `userInfo` of an `NSProgress` object that identifies
 the migration that was just applied to the database.
 @see `migrateDatabase:error:`
 */
extern NSString *const QCDBMigrationManagerProgressMigrationUserInfoKey;

/**
 @abstract Enumerates the errors returned by QCDBMigrationManager
 */
typedef NS_ENUM(NSUInteger, QCDBMigrationManagerError) {
    /// Indicates that migration was halted due to cancellation
    QCDBMigrationManagerErrorMigrationCancelled  = 1
};
