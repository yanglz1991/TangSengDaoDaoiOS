...
@implementation QCChannelMember
...
-(BOOL) isEqual:(id)obj{
    ...
    QCChannelMember *cm = (QCChannelMember*)obj;
...
@interface QCChannelMemberDB ()

@property(nonatomic,strong) NSLock *memberLock;

@end

@implementation QCChannelMemberDB

static QCChannelMemberDB *_instance;
...
-(NSArray<QCChannelMember*>*) getMembersWithChannel:(QCChannel*)channel {
    ...
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
       FMResultSet *resultSet = [db executeQuery:SQL_MEMBER_LIST,channel.channelId,@(channel.channelType),@(QCMemberRoleCreator),@(QCMemberRoleManager)];
...
-(NSArray<QCChannelMember*>*) getMembersWithChannel:(QCChannel*)channel limit:(NSInteger)limit {
    __block NSMutableArray *members = [NSMutableArray array];
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
       FMResultSet *resultSet = [db executeQuery:SQL_MEMBER_LIST_WKIT,channel.channelId,@(channel.channelType),@(QCMemberRoleCreator),@(QCMemberRoleManager),@(limit)];
        while (resultSet.next) {
            [members addObject:[self toMemberModel:resultSet]];
        }
        [resultSet close];
...
-(NSArray<QCChannelMember*>*) getMembersWithChannel:(QCChannel*)channel keyword:(NSString*)keyword page:(NSInteger)page limit:(NSInteger)limit {
    ...
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        ...
        if(keyword && ![keyword isEqualToString:@""]) {
            resultSet = [db executeQuery:SQL_MEMBER_LIST_PAGE_WITH_KEYWORD,channel.channelId,@(channel.channelType),[NSString stringWithFormat:@"%%%@%%",keyword],[NSString stringWithFormat:@"%%%@%%",keyword],[NSString stringWithFormat:@"%%%@%%",keyword],@(QCMemberRoleCreator),@(QCMemberRoleManager),@((page-1)*limit),@(limit)];
        }else {
            resultSet = [db executeQuery:SQL_MEMBER_LIST_PAGE,channel.channelId,@(channel.channelType),@(QCMemberRoleCreator),@(QCMemberRoleManager),@((page-1)*limit),@(limit)];
        }
        while (resultSet.next) {
            [members addObject:[self toMemberModel:resultSet]];
        }
...
-(NSArray<QCChannelMember*>*) getManagerAndCreator:(QCChannel*)channel {
    __block NSMutableArray *members = [NSMutableArray array];
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
       FMResultSet *resultSet = [db executeQuery:SQL_MEMBER_MANAGER_CREATOR,channel.channelId,@(channel.channelType),@(QCMemberRoleCreator),@(QCMemberRoleManager),@(QCMemberRoleCreator),@(QCMemberRoleManager)];
        while (resultSet.next) {
            [members addObject:[self toMemberModel:resultSet]];
        }
        [resultSet close];
...
-(BOOL) isManager:(QCChannel*)channel memberUID:(NSString*)uid {
    __block BOOL exist;
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        FMResultSet *resultSet = [db executeQuery:SQL_MEMBER_ISMANAGER,channel.channelId?:@"",@(channel.channelType),uid?:@"",@(QCMemberRoleCreator),@(QCMemberRoleManager)];
        if(resultSet.next) {
...
- (BOOL)isCreator:(QCChannel*)channel  memberUID:(NSString *)uid {
    __block BOOL exist;
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        FMResultSet *resultSet = [db executeQuery:SQL_MEMBER_ISCREATOR,channel.channelId?:@"",@(channel.channelType),uid?:@"",@(QCMemberRoleCreator)];
        if(resultSet.next) {
            exist = [resultSet intForColumn:@"cn"];
        }
        [resultSet close];
...
- (QCChannelMember*)get:(QCChannel*)channel  memberUID:(NSString *)uid {
    __block QCChannelMember *member;
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        FMResultSet *resultSet = [db executeQuery:SQL_MEMBER_WITH_UID,channel.channelId?:@"",@(channel.channelType),uid?:@"",@(QCMemberRoleCreator)];
        if(resultSet.next) {
...
-(QCChannelMember*) toMemberModel:(FMResultSet*) resultSet {
    QCChannelMember *member = [QCChannelMember new];
