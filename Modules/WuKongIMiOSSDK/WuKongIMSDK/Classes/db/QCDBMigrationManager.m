//
//  QCDBMigrationManager.m
//  QCDBMigrationManager
//
...
// Public Constants
NSString *const QCDBMigrationManagerErrorDomain = @"com.layer.QCDBMigrationManager.errors";
NSString *const QCDBMigrationManagerProgressVersionUserInfoKey = @"version";
NSString *const QCDBMigrationManagerProgressMigrationUserInfoKey = @"migration";

// Private Constants
static NSString *const QCDBMigrationFilenameRegexString = @"^(\\d+)_?((?<=_)[\\w\\s-]+)?(?<!_)\\.sql$";
...
BOOL FMDBIsMigrationAtPath(NSString *path)
{
    ...
    dispatch_once(&onceToken, ^{
        migrationRegex = [NSRegularExpression regularExpressionWithPattern:QCDBMigrationFilenameRegexString options:0 error:nil];
    });
...
@interface QCDBMigrationManager ()
...
@implementation QCDBMigrationManager
...
- (void)addMigration:(id<QCDBMigrating>)migration
{
...
- (void)addMigrations:(NSArray *)migrations
{
    ...
    for (id<NSObject> migration in migrations) {
        if (![migration conformsToProtocol:@protocol(QCDBMigrating)]) {
            @throw [NSException exceptionWithName:NSInvalidArgumentException reason:@"Failed to add migrations because an object in `migrations` array doesn't conform to the `QCDBMigrating` protocol." userInfo:nil];
        }
    }
...
- (NSArray *)migrations
{
  ...
  //  if (_migrations) return _migrations;
    ...
    NSRegularExpression *migrationRegex = [NSRegularExpression regularExpressionWithPattern:QCDBMigrationFilenameRegexString options:0 error:nil];
    ...
    for (NSString *path in migrationPaths) {
        ...
        if ([migrationRegex rangeOfFirstMatchInString:filename options:0 range:NSMakeRange(0, [filename length])].location != NSNotFound) {
            QCDBFileMigration *migration = [QCDBFileMigration migrationWithPath:path];
            [migrations addObject:migration];
        }
    }
    
    // Find all classes implementing QCDBMigrating
    if (self.dynamicMigrationsEnabled) {
        NSArray *conformingClasses = FMDBClassesConformingToProtocol(@protocol(QCDBMigrating));
        for (Class migrationClass in conformingClasses) {
            if ([migrationClass isSubclassOfClass:[QCDBFileMigration class]]) continue;
            id<QCDBMigrating> migration = [migrationClass new];
            [migrations addObject:migration];
        }
    }
...
- (id<QCDBMigrating>)migrationForVersion:(uint64_t)version
{
    for (id<QCDBMigrating>migration in [self migrations]) {
        if (migration.version == version) return migration;
    }
    return nil;
}

- (id<QCDBMigrating>)migrationForName:(NSString *)name
{
    for (id<QCDBMigrating>migration in [self migrations]) {
...
- (BOOL)migrateDatabaseToVersion:(uint64_t)version bundle:(NSBundle*)migrationsBundle progress:(void (^)(NSProgress *progress))progressBlock error:(NSError **)error
{
    ...
    for (NSNumber *migrationVersionNumber in pendingVersions) {
        ...
        id<QCDBMigrating> migration = [self migrationForVersion:migrationVersion];
        ...
        if (progressBlock) {
            [progress setUserInfoObject:@(migrationVersion) forKey:QCDBMigrationManagerProgressVersionUserInfoKey];
            [progress setUserInfoObject:migration forKey:QCDBMigrationManagerProgressMigrationUserInfoKey];
            progressBlock(progress);
            if (progress.cancelled) {
                success = NO;
                
                NSDictionary *userInfo = @{ NSLocalizedDescriptionKey: @"Migration was halted due to cancellation." };
                if (error) *error = [NSError errorWithDomain:QCDBMigrationManagerErrorDomain code:QCDBMigrationManagerErrorMigrationCancelled userInfo:userInfo];
                [self.database rollback];
                break;
            }
        }
...
static BOOL QCDBMigrationScanMetadataFromPath(NSString *path, uint64_t *version, NSString **name)
{
    NSError *error = nil;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:QCDBMigrationFilenameRegexString options:0 error:&error];
    if (!regex) {
        NSLog(@"[QCDBMigration] Failed constructing regex: %@", error);
        return NO;
    }
...
@interface QCDBFileMigration ()
@property (nonatomic, readwrite) NSString *name;
@property (nonatomic, readwrite) uint64_t version;
@end

@implementation QCDBFileMigration
...
- (id)initWithPath:(NSString *)path
{
    NSString *name;
    uint64_t version;
    if (!QCDBMigrationScanMetadataFromPath(path, &version, &name)) return nil;
