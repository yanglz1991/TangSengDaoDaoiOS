...
@implementation QCMessageExtraDB

static QCMessageExtraDB *_instance;
...
-(QCMessageExtra*) getMessageExtraWithMessageID:(uint64_t)messageID {
    __block QCMessageExtra *messageExtra;
...
-(NSArray<QCMessageExtra*>*) getContentEditWaitUpload {
    ...
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        FMResultSet *resultSet = [db executeQuery:SQL_CONTENT_EDIT_UPLOAD_WITH_STATUS,@(QCContentEditUploadStatusWait)];
...
-(void) updateContentEditUploadStatusToFailStatus{
    [[QCDB sharedDB].dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        [db executeUpdate:SQL_UPDATE_CONTENT_EDIT_UPLOADSTATUS_WITH_UPLOADSTATUS,@(QCContentEditUploadStatusError),@(QCContentEditUploadStatusWait)];
    }];
}
...
-(QCMessageExtra*) toMessageExtra:(FMResultSet*)resultSet db:(FMDatabase*)db{
    QCMessageExtra *messageExtra = [QCMessageExtra new];
    ...
    if(contentEditData&& [contentEditData length]>0) {
        ...
        if(!error && contentEditDict) {
            ...
            messageExtra.contentEdit = [[QCMessageDB shared] decodeContent:contentType data:contentEditData db:db];
        }
    }
