//
//  QCMergeForwardDetailVideoCell.m
//  QCVideo
//
...
@implementation QCMergeForwardDetailVideoModel


- (Class)cell {
    return QCMergeForwardDetailVideoCell.class;
}

@end

@interface QCMergeForwardDetailVideoCell ()

@property(nonatomic,strong) QCImageView *coverImgView;

@property(nonatomic,strong) QCSmallVideoContent *smallVideoContent;
...
@implementation QCMergeForwardDetailVideoCell

+ (CGFloat)contentHeightForModel:(QCMergeForwardDetailVideoModel *)model maxWidth:(CGFloat)maxWidth{
    QCSmallVideoContent *content = (QCSmallVideoContent*)model.message.content;
...
- (void)refresh:(QCMergeForwardDetailVideoModel *)model {
    [super refresh:model];
    self.message = model.message;
    self.smallVideoContent = (QCSmallVideoContent*)model.message.content;
    ...
    if([[NSFileManager defaultManager] fileExistsAtPath:[self.smallVideoContent coverLocalPath]]) {
        [self.coverImgView lim_setImageWithURL:[NSURL fileURLWithPath:[self.smallVideoContent coverLocalPath]]];
         ...
         [self.coverImgView loadImage:[[QCApp shared] getImageFullUrl:self.smallVideoContent.cover] placeholderImage:[self imageName:@"DefaultVideo"]];
    }
}
...
-(void) onTap {
    ...
    QCBrowserToolbar *toolbar = QCBrowserToolbar.new;
    ...
    QCVideoData *data = [QCVideoData new];
    data.autoPlayCount = 1;
    data.thumbImage = self.coverImgView.image;
//    data.extraData = @{@"message":self.message};
    
    QCMessageFileDownloadTask *task =  [[QCSDK shared] getMessageDownloadTask:self.message];
    if(!task) {
        task =  [[QCSDK shared].mediaManager download:weakSelf.message];
    }
...
- (QCImageView *)coverImgView {
    if(!_coverImgView) {
        _coverImgView = [[QCImageView alloc] init];
...
- (UILabel *)durationLbl {
    if(!_durationLbl) {
        ...
        _durationLbl.font = [[QCApp shared].config appFontOfSize:10.0f];
