...
@implementation WKGroupApprovalModel

- (Class)cell {
    return WKGroupApprovalCell.class;
}

@end

@interface WKGroupApprovalCell ()
...
@implementation WKGroupApprovalCell

+ (CGSize)sizeForModel:(WKGroupApprovalModel *)model {
    CGFloat maxWidth = WKScreenWidth - 30.0f;
    CGFloat height = 12.0f + 40.0f; // top padding + avatar
    CGSize contentSize = CGSizeZero;
    if (model.content && model.content.length > 0) {
        NSAttributedString *str = [[NSAttributedString alloc] initWithString:model.content attributes:@{NSFontAttributeName: [[WKApp shared].config appFontOfSize:14.0f]}];
    ...
    if (model.remark && model.remark.length > 0) {
        NSAttributedString *str = [[NSAttributedString alloc] initWithString:model.remark attributes:@{NSFontAttributeName: [[WKApp shared].config appFontOfSize:13.0f]}];
...
- (void)setupUI {
    ...
    self.nameLbl.font = [[WKApp shared].config appFontOfSizeMedium:16.0f];
    [self addSubview:self.nameLbl];

    self.timeLbl = [[UILabel alloc] init];
    self.timeLbl.font = [[WKApp shared].config appFontOfSize:12.0f];
...
- (void)refresh:(WKGroupApprovalModel *)model {
    ...
    if (model.avatarURL && model.avatarURL.length > 0) {
        ...
        [self.avatarImgView setImage:[WKApp shared].config.defaultAvatar];
    }
