...
-(void) initTasks {
    ...
    // ---- 如果是声音文件，则上传转码后的副本也就是amr文件 ----
     if([media isKindOfClass:[QCVoiceContent class]]) {
...
-(void) createAndAddUploadTask:(NSString*)path sourceFileURL:(NSString*)fileURL {
    ...
    [self getUploadURL:path].then(^(NSDictionary *result){
        ...
        NSURLSessionDataTask *task = [[QCAPIClient sharedClient] createFileUploadTask:uploadUrl fileURL:fileURL progress:^(NSProgress * _Nullable uploadProgress) {
...
-(void) uploadVideoCoverImage:(void(^)(void))successCallback {
    ...
    if(!coverFileURL) {
        QCLogDebug(@"上传视频，没有设置封面图,请在extra字段内设置video_cover_file");
        return;
    }
     NSString *randomFileName = [[[NSUUID UUID] UUIDString] stringByReplacingOccurrencesOfString:@"-" withString:@""];
    __weak typeof(self) weakSelf = self;
    ...
    [self getUploadURL:path].then(^(NSDictionary*result){
        NSString *uploadUrl = result[@"url"];
        NSURLSessionDataTask *task = [[QCAPIClient sharedClient] createFileUploadTask:uploadUrl fileURL:[NSString stringWithFormat:@"file://%@",coverFileURL] progress:^(NSProgress * _Nullable uploadProgress) {
        } completeCallback:^(id  _Nullable responseObj, NSError * _Nullable error) {
            if(error) {
                weakSelf.status = QCTaskStatusError;
                weakSelf.error = error;
